...
class Buff:
    ...
    def start(self, timestamp: int, duration: Optional[int] = None, stacks: int = 0):
        """
        激活 Buff (初始化状态)
    ...
    def refresh(self, timestamp: int, duration: Optional[int] = None, stacks_to_add: int = 0):
        """
        刷新 Buff 状态 (叠层或续时)
        :param timestamp: 当前时间戳
    ...
    def cleanup_expired_stacks(self, current_time: int):
        """
    ...
    def _add_independent_stack(self, start: int, duration: int, count: int = 1):
        """内部方法：处理独立堆叠"""
        end = start + duration
    ...
    def _update_timers_bounds(self):
        """内部方法：根据堆叠情况更新起止时间显示"""
