...
GENERATED_DIR = Path(
    "zsim/sim_progress/BuffGraph/generated_specs/runtime-command-scheduled-producers"
)
ORACLE_ROOT = Path("tests/fixtures/buff_graph/generated-spec-legacy-oracles")


def test_runtime_scheduled_generated_spec_legacy_oracle_batch_is_complete_and_consumable() -> None:
    wrappers = _generated_specs()
    registry = build_default_block_registry()
    adapters = _candidate_harness_adapters()

    assert len(wrappers) == 26
    ...
    for spec_path, wrapper in wrappers:
        ...
        assert fixture["migration_wave"] == "runtime-command-scheduled-producers"
