...
GENERATED_ROOT = Path("zsim/sim_progress/BuffGraph/generated_specs")
...
def test_low_risk_generated_spec_legacy_oracle_batch_is_complete_and_consumable() -> None:
    wrappers = _low_risk_generated_specs()
...
def _low_risk_generated_specs() -> list[tuple[Path, dict]]:
    specs: list[tuple[Path, dict]] = []
    for dirname in LOW_RISK_DIRS:
        for path in sorted((GENERATED_ROOT / dirname).glob("*.buffgraph.json")):
            specs.append((path, _read_json(path)))
    return specs
