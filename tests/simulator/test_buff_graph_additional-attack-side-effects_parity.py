...
FIXTURE_ROOT = Path(__file__).parents[1] / "fixtures" / "buff_graph" / "additional-attack-side-effects"


def test_additional_attack_side_effect_xlogic_requires_controlled_effect_blocks() -> None:
...
def test_additional_attack_side_effect_contracts_are_anchored_to_current_xlogic() -> None:
    fixture = json.loads((FIXTURE_ROOT / "side-effect-contracts.json").read_text(encoding="utf-8"))
...
def test_additional_attack_wave_preserves_future_effect_vocabulary() -> None:
    cases = json.loads((FIXTURE_ROOT / "unsupported-cases.json").read_text(encoding="utf-8"))
    contracts = json.loads((FIXTURE_ROOT / "side-effect-contracts.json").read_text(encoding="utf-8"))
