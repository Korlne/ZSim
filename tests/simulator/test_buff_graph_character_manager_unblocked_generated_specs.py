...
def test_character_manager_unblocked_generated_specs_validate_compile_and_mirror() -> None:
    ...
    assert manifest["generated_spec_count"] == 17
    assert manifest["blocked_case_count"] == 18
...
def test_character_manager_generated_sources_are_removed_from_remaining_unsupported_manifest() -> None:
    character_manifest = json.loads((SPEC_ROOT / "manifest.json").read_text(encoding="utf-8"))
    ...
    generated_sources = {
        ...
        for spec_file in character_manifest["generated_specs"]
    }
    remaining_sources = {
