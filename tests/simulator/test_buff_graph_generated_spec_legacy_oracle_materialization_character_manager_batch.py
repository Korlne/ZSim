...
GENERATED_DIR = Path(
    "zsim/sim_progress/BuffGraph/generated_specs/character-manager-side-effect-cases"
)
ORACLE_ROOT = Path("tests/fixtures/buff_graph/generated-spec-legacy-oracles")


def test_character_manager_generated_spec_legacy_oracle_batch_is_complete_and_consumable() -> None:
    wrappers = _generated_specs()
    registry = build_default_block_registry()
    adapters = _candidate_harness_adapters()

    assert len(wrappers) == 17
    ...
    for spec_path, wrapper in wrappers:
        ...
        if spec.graph_id == "alice-cinema-6-trigger":
            assert "migration_wave" not in fixture
        else:
            assert fixture["migration_wave"] == "character-manager-side-effect-cases"
...
def _generated_specs() -> list[tuple[Path, dict]]:
    return [
        (path, _read_json(path)) for path in sorted(GENERATED_DIR.glob("*.buffgraph.json"))
    ]
