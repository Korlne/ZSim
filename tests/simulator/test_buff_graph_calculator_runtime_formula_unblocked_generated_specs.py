...
def test_calculator_runtime_formula_sources_are_removed_from_remaining_unsupported_manifest() -> None:
    ...
    assert remaining_manifest["generated_spec_source_count"] == 149
    assert remaining_manifest["unsupported_case_count"] == 1
    assert generated_sources.isdisjoint(remaining_sources)
    assert remaining_sources == {"zsim/sim_progress/Buff/BuffXLogic/YuzuhaCinema2Trigger.py"}
