...
@pytest.mark.parametrize(
    "fixture_name",
    [
        "astral-voice-candidate.json",
        "hellfire-gears-sp-r-bonus-candidate.json",
        "ice-jade-teapot-extra-dmg-bonus-candidate.json",
        "zanshin-herb-case-candidate.json",
    ],
)
def test_dynamic_owner_generated_spec_candidate_harness_matches_fixture(
    fixture_name: str,
) -> None:
    ...
    result = run_buff_graph_candidate_parity(
        ...
        adapters=_dynamic_owner_candidate_adapters(),
        tick=case["tick"],
...
def _dynamic_owner_candidate_adapters() -> dict[str, object]:
