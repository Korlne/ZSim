...
FIXTURE_ROOT = Path(__file__).parents[1] / "fixtures" / "buff_graph" / "anomaly-dot-adjacent-cases"


def test_anomaly_dot_xlogic_requires_controlled_anomaly_dot_blocks() -> None:
...
def test_anomaly_dot_contracts_are_anchored_to_current_xlogic() -> None:
    fixture = json.loads((FIXTURE_ROOT / "dot-anomaly-contracts.json").read_text(encoding="utf-8"))
...
def test_anomaly_dot_wave_preserves_future_block_vocabulary() -> None:
    cases = json.loads((FIXTURE_ROOT / "unsupported-cases.json").read_text(encoding="utf-8"))
    contracts = json.loads((FIXTURE_ROOT / "dot-anomaly-contracts.json").read_text(encoding="utf-8"))
