...
- [x] Phase 3 RegularMul scalar helper-family bounded implementation 已完成 US-010 handoff：`Calculator.RegularMul.cal_dmg_bonus(data)`、resistance / vulnerability / special group、defense helper group 与 `cal_crit_expect(data)` 已通过 module-local helper seams 实现 / no-op verified；non-sheer `cal_base_dmg(data)` / `cal_base_attr(base_attr in 0..3, data)` 因 direct `base_attr=2/3` oracle 缺口保持 No-Go，`base_attr=4` / `cal_sheer_dmg_bonus(data)` 仍是 retained-only sheer follow-up。Verifier evidence：focused RegularMul pytest `28 passed`、scoped mypy `Success: no issues found in 9 source files`、serial `formula-parity` focused `154 passed` / mypy `9 source files` clean、serial `calculator-reads` focused `254 passed` / mypy `22 source files` clean；US-010 handoff 追加 docs scoped mypy、JSON sanity、UTF-8 / mojibake scan、focused `git diff --check`、checkpoint 和 reviewer / invariant verdict。下一默认 PRD 返回 Phase-3 same-phase candidate selection / bounded proposal，不折叠为单一旧 callsite；same-phase pool 继续保留 registered sample eligibility、remaining RegularMul / retained-only sheer、future Stun evidence、P2 guarded maintenance、retained compatibility 与 blocker-only reopen rules。
...
- [x] Current RegularMul scalar helper-family PRD US-010 final closure：current-root docs、checkpoint、Ralph progress、campaign dashboard 和 evidence ledger 已同步 implemented / No-Go 状态。Implemented / no-op verified surfaces 为 damage bonus、resistance / vulnerability / special group、defense helper group 和 crit expectation helper；non-sheer base damage / base attr 与 retained-only sheer仍按 evidence 保持 No-Go / follow-up。下一默认 PRD 是 Phase-3 same-phase candidate selection / bounded proposal：先选择一个 exact bounded surface，再写 source audit、deterministic oracle / gap review、registered-sample 条件、rollback anchors、focused tests、scoped mypy、retained validation gates、reviewer verdict 和 handoff docs；不得重开 current scalar helper-family batch或把路线折叠到最后一个 callsite。
