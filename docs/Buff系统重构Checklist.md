...
- [ ] 下一轮默认 PRD 应沿 [Buff重构方案.md](./Buff重构方案.md) 从阶段 2 guarded-maintenance 状态转入 phase-3 formula parity suite design / characterization：先命名 candidate files、focused pytest targets、scoped mypy targets、behavior-sample 条件、rollback plan、validation entrypoints 和 non-goals，再评估是否允许 production formula replacement。
