...
- [x] Final Go / No-Go：production formula replacement 下一 PRD 仍 No-Go。下一默认 PRD 继续 phase-3 formula oracle gap closure / deterministic parity matrix，先补齐 `RegularMul` / `AnomalyMul` / `StunMul` 表驱动 oracle、`CalDisorder` / `CalPolarityDisorder` / `CalAbloom` deterministic oracle、copied-output report payload parity、`AnomalyBar.current_ndarray` 字段矩阵、注册队伍行为样本触发条件与 rollback plan；通过前不得替换 `Calculator.py` / `CalAnomaly.py` 生产公式。
