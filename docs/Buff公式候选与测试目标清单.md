...
| `Calculator.RegularMul.cal_personal_crit_rate()` | `regular-crit-received-boundary`、P2-B full / personal crit parity tests、crit family boundary test；current proposal-readiness PRD confirmed existing personal/full rows are sufficient for one later bounded helper-seam implementation. | No current deterministic row blocker for the bounded helper-seam proposal；future implementation must keep focused personal/full crit tests and may add only story-local rows if a regression appears. | Snapshot-compatible，必须保留 personal crit 排除 received bonus；不得扩展 `_CalculatorReadSnapshot` public contract。 | Conditional No-Go until a future live semantic diff exists and a real registered direct-crit route proves nonzero selected personal-crit-rate relevance. | `cal_personal_crit_rate()`、optional `_calculate_personal_crit_rate(...)`、reader `read_personal_crit_rate()` parity tests、`formula-parity`、conditional `calculator-reads`。 | Proposal-ready；Conditional Go for one later bounded implementation PRD only. |
...
## Current personal crit rate proposal-readiness PRD US-008 final handoff

本 handoff 属于当前 RegularMul personal crit rate bounded proposal-readiness PRD。结论：Conditional Go for one later bounded implementation PRD only。允许的 future diff 只限 `Calculator.RegularMul.cal_personal_crit_rate(data)` 与可选 module-local `_calculate_personal_crit_rate(static_statement, dynamic_statement)` helper；helper 必须保持 `static.crit_rate + dynamic.crit_rate + dynamic.field_crit_rate` 并继续排除 `crit_rate_received_increase`。

| Contract area | US-008 final handoff |
| --- | --- |
| Selected candidate outcome | `Calculator.RegularMul.cal_personal_crit_rate(data)` is the only selected implementation candidate. Full `Calculator.RegularMul.cal_crit_rate(data)` / `_calculate_full_crit_rate(...)` remains the received-crit contrast branch and is not reopened. |
| Retained gates | Required retained gate already passed in US-006: serial `formula-parity` exited `0` with base simulator `2 passed`, isolated teams `3 passed`, focused formula suite `145 passed`, and scoped mypy success on `9 source files`. `calculator-reads` remains conditional on reader/snapshot/guardrail changes; `implicit-events` remains conditional on copied-output, event, dispatch, runtime, listener, dot-runtime, lifecycle, validation-runner, old-container, or retained compatibility surfaces. |
...
| Rollback anchors | Current `Calculator.RegularMul.cal_personal_crit_rate(data)` body, optional future `_calculate_personal_crit_rate(...)` helper if added, `CalculatorBuffAttributeReader.read_personal_crit_rate(context)`, `_CalculatorReadSnapshot`, full/personal crit focused tests, formula candidate matrix row, retained Buff docs, `formula-parity`, `calculator-reads`, old containers, and layer-separation invariants. |
| Reviewer verdict | PASS from US-007: changed-file review found no production/source boundary diffs in `Calculator.py`, `CalAnomaly.py`, BuffXLogic, dispatch/runtime/facade, event handler, or `UpdateAnomaly` paths. Event queue semantics, synchronous listener broadcasts, same-tick runtime writes, old containers, validation-runner behavior, registered routes, and retained compatibility remain unchanged. |
| Go / No-Go verdict | Conditional Go for one bounded implementation PRD only. Stop and split if implementation needs received-crit inclusion, `_CalculatorReadSnapshot` public expansion, full/personal crit bundling, `cal_crit_expect()` semantics, broad formula branch work, registered-team fixture creation, validation-runner rewrite, old-container deletion, event/runtime/listener layer merge, or retained compatibility deletion. |
...
| Next default route | Phase-3 bounded implementation PRD for `Calculator.RegularMul.cal_personal_crit_rate(data)` only. After that PRD completes or records No-Go, reselect from the same-phase pool instead of continuing with another personal-crit-rate follow-up. |
