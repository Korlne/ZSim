...
| `test_formula_oracle_table_cases_drive_expected_fields_and_reader_parity[regular-base-dmg-*]`、`test_p2b_parity_fixture_matches_old_full_and_personal_crit_rate_helpers()`、`test_p2b_full_crit_rate_includes_received_bonus_but_personal_excludes()`、`test_p2b_parity_fixture_matches_old_personal_crit_damage_helper()`、`test_p2b_personal_crit_damage_excludes_received_crit_damage_bonus()`、`test_calculator_regular_mul_crit_formula_families_preserve_received_boundaries()`、`test_calculator_attribute_formula_boundaries_remain_retained_compatibility()` | `Calculator.RegularMul.cal_base_dmg()`、`cal_base_attr()`、`cal_crit_rate()`、`cal_personal_crit_rate()`、`cal_personal_crit_dmg()`、neutral / static-field / dynamic-buff base damage inputs、received crit inclusion / exclusion。 | base damage / base attribute 只作为 retained Calculator oracle 与 reader-snapshot compatibility evidence；full crit 保留 received crit rate；personal crit rate / damage 保留排除 received bonus；不覆盖 damage bonus、defense、resistance、vulnerability 全矩阵。 |
...
| --- | --- | --- |
| `Calculator.RegularMul` | `cal_base_dmg()` / `cal_base_attr()` 已有 neutral、static-field、dynamic-buff 表驱动 oracle；crit / personal crit reader seam 与部分 received-boundary 已有 focused tests；`cal_dmg_bonus()`、`cal_defense_mul()`、`cal_recipient_def()` / `cal_pen_ratio()` / `cal_res_mul()`、`cal_dmg_vulnerability()`、`cal_stun_vulnerability()`、`cal_special_mul()`、`cal_sheer_dmg_bonus()`、`cal_crit_dmg()`、`cal_crit_expect()` 和 array outputs 仍缺表驱动 oracle。 | 继续拆成 damage bonus / defense / resistance / vulnerability、crit expectation 两组；每组包含 neutral、static-field、dynamic-buff、enemy-side / received 字段组合。 |
