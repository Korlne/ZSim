...
## US-002 Remaining Formula Surface Matrix

本矩阵属于当前 registered sample / formula candidate selection PRD。它只把剩余 formula surface 绑定到现有 oracle、缺口、reader-snapshot 依赖、registered-route 条件和 rollback anchors；不新增生产公式替换授权。保留边界继续包括 `MultiplierData`、`MulData`、`DynamicStatement`、`AnomalyBar.current_ndarray`、copied-output constructors、old Buff containers、`buff_add()`、`KickOutBuff()`、`ScheduleDispatchPort`、`RuntimeCommandPort`、`LegacyRuntimeCommandAdapter` 和 `LegacyBuffRuntimeFacade`。

| Candidate | Current oracle evidence | Missing deterministic oracle rows | Reader-snapshot dependency | Registered-route eligibility | Rollback anchors | Status |
| --- | --- | --- | --- | --- | --- | --- |
| `Calculator.RegularMul.cal_base_dmg()` | `test_formula_oracle_table_cases_drive_expected_fields_and_reader_parity[regular-base-dmg-neutral-atk]`、`[regular-base-attr-static-hp]`、`[regular-base-dmg-dynamic-atk]`；`test_calculator_regular_mul_branch_matrix_characterizes_selected_methods[fire-stunned-reader-snapshot]` / `[sheer-retained-only]`。 | Production proposal 前仍需补 `base_attr=2` defense、`base_attr=3` AP、以及非 ATK/HP 的 `extra_damage_ratio` / base damage increase 组合；sheer path 需与 `base_attr=4` runtime dependency 分开审。 | ATK/HP/普通直伤 snapshot-compatible；`base_attr=4` 会进入 `char_instance.sheer_attack_conversion_rate`，当前 `_CalculatorReadSnapshot` 不携带 `char_instance`。 | 仅 live production semantic diff 且真实 registered route 命中 direct damage 时运行；docs-only matrix 不运行样本。 | `Calculator.py::RegularMul.cal_base_dmg()` / `cal_base_attr()`、`test_buff_attribute_reader.py` formula table、branch matrix、`formula-parity` / `calculator-reads`。 | Characterized for retained oracle；production proposal blocked until exact branch + route + snapshot plan named. |
| `Calculator.RegularMul.cal_base_attr()` | `regular-base-dmg-neutral-atk` 覆盖 `base_attr=0`，`regular-base-attr-static-hp` 覆盖 `base_attr=1`，`test_regular_mul_retained_sheer_base_attr_requires_char_instance_conversion_rate()` 覆盖 `base_attr=4` retained runtime dependency。 | 缺 `base_attr=2` defense 和 `base_attr=3` AP deterministic rows；`base_attr=4` 还缺 reader-contract solution，不是只补数值行能关闭。 | `base_attr=0/1/2/3` 可由 snapshot 字段表达；`base_attr=4` 依赖 runtime `char_instance` conversion map。 | Direct damage route 条件同上；sheer conversion 当前没有可用 registered `仪玄` / Yixuan route，禁止创建 validation-only team。 | `cal_base_attr(...)`、`_regular_mul_base_attr()` helper、retained sheer test、`_CalculatorReadSnapshot` field set。 | Partly characterized；`base_attr=4` blocked by reader-snapshot contract and registered-route gap. |
| `Calculator.RegularMul.cal_dmg_bonus()` | `regular-multipliers-neutral-zero-boundary`、`regular-dmg-bonus-character-field-stack`、branch matrix rows覆盖 element / trigger / label / all damage bonus 的当前代表组合。 | 若选为 production candidate，需要补 physical / ice / electric / ether element rows、主要 trigger 类型和 label branch 的最小 cross-product；不要一次展开成全组合爆炸。 | Snapshot-compatible，依赖 `judge_node.element_type`、`trigger_buff_level`、skill labels 和 dynamic bonus fields。 | 只在 production diff 改变 live damage multiplier 且已注册队伍命中对应 element / trigger route 时追加 main-loop sample。 | `cal_dmg_bonus()`、formula table cases、branch matrix、`CalculatorBuffAttributeReader` read context。 | Characterized as retained oracle；proposal still needs exact branch selection. |
| `Calculator.RegularMul.cal_crit_rate()` | `regular-crit-received-boundary`、`test_p2b_parity_fixture_matches_old_full_and_personal_crit_rate_helpers()`、`test_p2b_full_crit_rate_includes_received_bonus_but_personal_excludes()`、`test_calculator_regular_mul_crit_formula_families_preserve_received_boundaries()`。 | Production proposal 前需决定是否需要 over-100% / crit-balancing caller rows；`cal_crit_expect()` 不在本行授权范围内。 | Snapshot-compatible，必须保留 full crit 包含 `crit_rate_received_increase`。 | Direct damage / crit route 条件成立才运行；样本不得替代 focused oracle。 | `cal_crit_rate()`、P2-B crit reader tests、formula table `regular-crit-received-boundary`。 | Characterized；blocked from production Go until crit candidate explicitly选择。 |
...
| `Calculator.RegularMul.cal_sheer_dmg_bonus()` | Branch matrix `[sheer-retained-only]` expects `1.45` for `diff_multiplier=4`；`test_regular_mul_retained_sheer_base_attr_requires_char_instance_conversion_rate()` proves `cal_sheer_dmg_bonus()` itself remains snapshot-compatible while `cal_base_attr(..., 4)` does not。 | Production proposal 仍缺 reader-contract solution for `char_instance.sheer_attack_conversion_rate` and real registered sheer route；do not close by adding only another numeric row。 | `cal_sheer_dmg_bonus()` snapshot-compatible；the surrounding sheer damage path is retained-only because base attr conversion requires runtime `char_instance`。 | 当前 No-Go：无真实 registered `仪玄` / Yixuan route evidence；禁止 validation-only team。 | `cal_sheer_dmg_bonus()`、`cal_base_attr(..., 4)`、retained sheer test、branch matrix、reader snapshot field set。 | Characterized but blocked for production proposal. |
| `Calculator.StunMul.get_stun_array()` | `test_stun_array_output_contract_preserves_field_order_dtype_and_product()` locks order `imp`, `stun_ratio`, `stun_res`, `stun_bonus`, `stun_received`、shape `(5,)`、dtype `np.float64`；individual stun scalar rows cover retained / reader-snapshot formulas。 | 若作为 proposal candidate，补 reader-snapshot construction row for full array or explicitly state retained array-only scope；also add neutral array row only if production diff changes defaults。 | Current direct array test uses retained `MultiplierData`; scalar components have reader-snapshot evidence, but whole-array reader-snapshot row is not separately locked。 | Stun / impact route only；registered sample requires real route with nonzero stun / impact observability and cannot be inferred from direct-damage samples。 | `StunMul.get_stun_array()`、stun array focused test、scalar stun oracle rows、`formula-parity`。 | Characterized array contract；no production replacement authorization. |
| `Calculator.cal_stun()` array output path (`Calculator.StunMul.cal_stun()` in PRD label) | `test_stun_array_output_contract_preserves_field_order_dtype_and_product()` wraps `get_stun_array()` and proves one-call product consumption via `np.prod(expected_array)` returning `np.float64`。 | If production diff touches consumer semantics, add alternate zero / negative resistance or neutral product row; otherwise current one-call product path is enough for characterization。 | Depends on `self.stun_multipliers.get_stun_array()` output; no separate reader API。 | Same stun registered-route condition as `get_stun_array()`；docs-only matrix skips sample。 | `Calculator.cal_stun()`、`StunMul.get_stun_array()`、focused product test。 | Characterized consumer path；blocked from production Go until exact candidate chosen. |

## US-003 registered behavior sample eligibility audit

本审计属于当前 candidate-selection PRD 的 docs-only / Guardrail-first slice。US-004 尚未选择唯一 proposal candidate，因此本节只给出当前 registered team / APL evidence、No-Go / conditional No-Go、以及未来 main-loop sample 的必要条件；不新增 validation-only team、fake APL、fixture-only route，也不运行 retained-vs-retained main-loop sample。
...
| Direct damage / crit / defense / resistance / vulnerability RegularMul branches | Registered fire/electric/physical APLs exist and can be candidate route seeds, but US-003 did not select an element / trigger / label branch or run behavior output. | Future route must bind the exact candidate branch to a real APL skill route and relevant nonzero event or formula count inside an explicit stop tick. | Conditional No-Go until the selected candidate, route, nonzero count, and rollback anchors are named. |
