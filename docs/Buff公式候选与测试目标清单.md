...
| enemy dynamic reads | `Calculator.MultiplierData.__new__()` 读取 `enemy.dynamic.dynamic_debuff_list` / `dynamic_dot_list` 参与缓存；`_calculate_dynamic_statement()` / `MultiplierData.get_buff_bonus()` 读取 `enemy.dynamic.dynamic_debuff_list`；`CalculatorBuffAttributeReader._build_statements()` 复用同一聚合入口；`CalAnomaly.py` 通过 `MulData` 读取 enemy dynamic 与结算快照；`AnomalyBar.__get_duration_enemy_buffs()` 读取 runtime-view enemy Buffs 并保留 legacy dynamic fallback。 | `tests/simulator/test_buff_attribute_reader.py`：`test_enemy_dynamic_debuff_reads_feed_old_and_reader_formula_snapshots()` 覆盖空敌方状态、单个 enemy debuff、堆叠 enemy debuff 和 enemy dot 非聚合边界；`test_cal_anomaly_uses_settled_snapshot_mul_data_and_retained_damage_ratios()` 记录异常结算路径携带 enemy dynamic lists。`tests/simulator/test_anomaly_handler_runtime_view.py` now covers 13 `test_anomaly_bar_duration_*` rows for runtime-view precedence, legacy fallback, missing enemy, no-buff / nonmatching Buffs, duration-affecting variants and clamp behavior。另有 `test_enemy_context_direct_helpers.py`、`test_buff_attribute_state_sync.py`。 | 公式级 enemy debuff 聚合与 anomaly-state read family 已有 focused characterization；anomaly-duration runtime-view / legacy dynamic 专项矩阵已完成并保持 retained behavior：runtime view 优先、legacy `dynamic_buff_dict["enemy"]` fallback 保留、missing `"enemy"` 仍为 `TypeError`、`dynamic_buff_dict is None` 为 empty enemy sequence、duration formula 保持 `basic_max_duration * (1 + summed_pct) + summed_fixed` and final `max(..., 0)`。dot/freez-like Load/Schedule continuation 的事件路由专项仍不是本 PRD 替换入口。 | `calculator-reads` profile；当前 duration PRD 也串行通过 `implicit-events`。若触及 direct enemy context，再追加 `test_enemy_context_direct_helpers.py`；若触及 event/runtime/listener 边界，再追加 `implicit-events`。 |
...
## US-003 verifier coverage matrix

本矩阵用于后续 PRD 选择 verifier，不授权本 PRD 改生产代码、focused tests、validation runner、registered routes、performance-sensitive path、old containers 或 retained compatibility。Buff validation profiles 必须串行执行，禁止并发跑 `calculator-reads`、`formula-parity`、`implicit-events` 或默认 profile。

| Boundary family | Retained / completed surface | Required future gate | Current US-003 decision |
| --- | --- | --- | --- |
| Completed Phase-3 formula / helper surfaces | RegularMul helper family、`Calculator.AnomalyMul.cal_res_pen()`、AM/AP/impact、selected Stun、CalAnomaly helper family、copied-output handler/report、`AnomalyBar.current_ndarray`、`UpdateAnomaly.py`、anomaly-duration runtime-view matrix。 | Touched focused pytest first, then `uv run python scripts/run_buff_refactor_validation.py --typecheck-profile formula-parity`; add `uv run python scripts/run_buff_refactor_validation.py --typecheck-profile calculator-reads` when reader seams or migrated XLogic reads are touched. | Docs-only matrix; no formula source, oracle rows, focused tests, or reader/snapshot contracts changed, so broad Buff validation is skipped. Completed surfaces stay no-reopen unless future focused regression, validation failure, current-root source evidence, guardrail, or reviewer evidence names exact file / symbol / behavior and rollback anchor. |
| P2-A through P2-G guarded-maintenance buckets | P2-A AM/AP reader, P2-B impact / crit readers, P2-C trigger-state gates, P2-D scheduled publish ordering, P2-E dot runtime-state, P2-F BuffAddStrategy facade-write, P2-G direct simulator context helpers. | Use the bucket's focused guardrail / pytest first; run `uv run python scripts/run_buff_refactor_validation.py --typecheck-profile calculator-reads` for reader/helper buckets and `uv run python scripts/run_buff_refactor_validation.py --typecheck-profile implicit-events` for trigger, publish, dot runtime, facade-write, listener, or same-tick boundary buckets. | Guarded-maintenance only; this PRD does not touch production source, tests, runner wiring, or retained behavior, so no P2 bucket is reopened. |
| Old containers / lifecycle compatibility | Old Buff containers, lifecycle fallback paths, `LegacyBuffRuntimeFacade`, legacy `buff_add()` / `KickOutBuff()`, retained compatibility write/read paths. | `uv run python scripts/run_buff_refactor_validation.py` only when lifecycle container behavior, runtime write path, validation-runner behavior, or retained compatibility behavior changes; include focused lifecycle/runtime tests and rollback anchors first. | Skipped now because no lifecycle/runtime write, old-container, runner, focused-test, or retained behavior changed. Old containers remain retained compatibility, not deletion authority. |
| Event-runtime-listener layers | `ScheduleDispatchPort`, `RuntimeCommandPort`, `LegacyRuntimeCommandAdapter`, listener broadcasts, dot runtime registration, scheduled publish order, same-tick writes. | `uv run python scripts/run_buff_refactor_validation.py --typecheck-profile implicit-events` is retained required evidence for future event/runtime/listener implementation PRDs; focused order/fan-out tests must precede profile validation. | No event/runtime doc boundary is changed beyond this matrix; future relevant PRDs must keep `implicit-events` as a required retained gate. Event queue semantics, synchronous listener broadcasts, and same-tick runtime writes remain separate layers. |
| Formula snapshot / reader contracts | `MultiplierData`, `MulData`, `DynamicStatement`, private `_CalculatorReadSnapshot`, `CalculatorBuffAttributeReader`, retained Calculator / CalAnomaly formulas. | `uv run python scripts/run_buff_refactor_validation.py --typecheck-profile formula-parity`; add `uv run python scripts/run_buff_refactor_validation.py --typecheck-profile calculator-reads` when public/private reader seams, migrated XLogic reads, or snapshot contracts are touched. | No public reader/snapshot expansion or formula snapshot deletion. Broad validation is skipped; scoped docs/Ralph typecheck covers this docs-only change. |
| Registered behavior / performance gates | Real registered-team main-loop consistency and runtime benchmark samples. | Main-loop JSON only after a live production semantic diff plus real registered route/APL, explicit stop tick, nonzero relevant formula/event counts, total damage, event counts, Buff timeline comparison, differences, and rollback anchor: `uv run python scripts/run_buff_main_loop_consistency.py --team <team> --stop-tick <n> --legacy-runtime <label> --candidate-runtime <label> --json`. Runtime benchmark JSON only after a performance-sensitive runtime/lifecycle/dispatch/formula/container behavior change with real route evidence: `uv run python scripts/run_buff_runtime_benchmark.py --team <team> --stop-tick <n> --legacy-runtime <label> --candidate-runtime <label> --json`. | Both live consistency and runtime benchmark samples are skipped now because this PRD changes no production source, focused tests, validation-runner behavior, registered route/APL, performance-sensitive path, retained behavior, or live semantic surface. Do not create validation-only teams, fake APLs, fixture-only routes, retained-vs-retained samples, or treat runtime labels as mode switches. |

## Current same-phase candidate selection PRD US-011 handoff

结论：US-010 proposal packet 的 `Calculator.RegularMul.cal_dmg_bonus(data)` Conditional Go / No-Go 仍是下一轮实现锚点，但下一轮 PRD 生成规则已从单点 `cal_dmg_bonus()` implementation 扩展为 RegularMul scalar helper-family bounded implementation batch。PRD 应一次性纳入同文件、同 validation profile、同 rollback anchor、无 runtime/event/listener/registered-route 新契约的相邻标量公式：必做 `cal_dmg_bonus(data)`、`cal_res_mul(...)`、`cal_dmg_vulnerability(...)`、`cal_stun_vulnerability(data)`、`cal_special_mul(data)`；条件纳入 `cal_defense_mul(data)` / `cal_k_attacker()` / `cal_pen_ratio()` / `cal_recipient_def(...)`、`cal_crit_expect(data)`、非 sheer `cal_base_dmg(data)` / `cal_base_attr(base_attr in 0..3, data)`。当前 US-011 只同步 handoff docs、Ralph evidence 与 completion bookkeeping，不替换 production formula、reader source、focused test source、validation-runner behavior、registered teams/APLs、old containers、event/runtime/listener layers 或 retained compatibility。

Selected files / symbols：

- Required batch anchors: `zsim/sim_progress/ScheduledEvent/Calculator.py::Calculator.RegularMul.cal_dmg_bonus(data)`, `cal_res_mul(...)`, `cal_dmg_vulnerability(...)`, `cal_stun_vulnerability(data)`, `cal_special_mul(data)`。
- Conditional batch anchors: `cal_defense_mul(data)` / `cal_k_attacker()` / `cal_pen_ratio()` / `cal_recipient_def(...)`, `cal_crit_expect(data)`, non-sheer `cal_base_dmg(data)` / `cal_base_attr(base_attr in 0..3, data)`; include only after an early PRD story proves focused oracle coverage and no new public reader / snapshot contract.
- Helper seams must be module-local and behavior-preserving; public method signatures, current `SkillNode` assumptions, element branch, trigger-level branch, `aftershock_attack` label branch, all-damage / all-res / all-vulnerability additions, enemy dynamic reads and rollback anchors remain intact.
- Focused evidence anchors start with `test_formula_oracle_table_cases_drive_expected_fields_and_reader_parity[regular-dmg-bonus-character-field-stack]` and `test_calculator_regular_mul_branch_matrix_characterizes_selected_methods[fire-stunned-reader-snapshot]`; missing sibling rows should be added inside the batch PRD rather than spawning another proposal-readiness PRD.

Retained boundaries / same-phase pool：

- Retained gates：selected focused pytest nodeids and scoped mypy first; serial `formula-parity` and `calculator-reads` remain required for formula / reader implementation work; `implicit-events`, default validation, and main-loop consistency stay conditional on touched surface and registered-route evidence. Validation profiles must run serially.
...
- Excluded from this batch: `base_attr=4`, `cal_sheer_dmg_bonus(data)`, registered behavior sample eligibility, event/runtime/listener paths, old containers, retained compatibility cleanup, completed copied-output / `cal_res_pen()` / AM/AP/impact / selected Stun / crit helper seams, and broad `Calculator.py` / `CalAnomaly.py` rewrite.
...
| Contract area | Current RegularMul scalar helper-family handoff |
| --- | --- |
| Implemented / no-op verified surfaces | `Calculator.RegularMul.cal_dmg_bonus(data)` -> `_calculate_damage_bonus(...)`; `cal_res_mul(...)` -> `_calculate_resistance_multiplier(...)`; `cal_dmg_vulnerability(...)` -> `_calculate_damage_vulnerability(...)`; `cal_stun_vulnerability(data)` -> `_calculate_stun_vulnerability(...)`; `cal_special_mul(data)` -> `_calculate_special_multiplier(...)`; `cal_defense_mul(data)` / `cal_recipient_def(...)` / `cal_pen_ratio(...)` / `cal_k_attacker(...)` -> defense helper group; `cal_crit_expect(data)` -> `_calculate_crit_expectation(...)`. |
| No-Go / retained-only surfaces | `Calculator.RegularMul.cal_base_dmg(data)` and non-sheer `cal_base_attr(base_attr in 0..3, data)` remain No-Go because direct deterministic non-sheer `base_attr=2/3` oracle rows are still missing. `base_attr=4` and `cal_sheer_dmg_bonus(data)` remain retained-only sheer follow-up and do not authorize public snapshot expansion. |
...
2026-06-15 13:42 +08:00 US-002 refresh：本矩阵从 current root 重新复核 `zsim/sim_progress/ScheduledEvent/Calculator.py`、`tests/simulator/test_buff_attribute_reader.py`、long-lived Buff docs、registered-team / APL evidence，并继续排除 `.codex_worktrees/`、`scripts/ralph/archive/**`、generated logs 和历史 worktree 证据。它只把 retained candidate pool 绑定到现有 oracle、缺口、reader-snapshot 依赖、registered-route 条件、rollback anchors、validation triggers 和 Go / No-Go blockers；不新增 production formula replacement 授权。

已完成 / no-reopen surfaces 继续不作为默认实现 backlog：copied-output handler/report implementation、`Calculator.AnomalyMul.cal_res_pen()` selector extraction、AM/AP/impact helper implementation、selected `Calculator.StunMul.get_stun_array()` implementation、`Calculator.RegularMul.cal_crit_rate(data)`、`Calculator.RegularMul.cal_personal_crit_rate(data)`、`Calculator.RegularMul.cal_personal_crit_dmg(data)` 和 `Calculator.RegularMul.cal_crit_dmg(data)`。后续 US-003 必须从下表重新选择一个 exact branch 或记录 No-Go，不能继承历史 selected surface。

Registered-route nuance：当前存在 `zsim/data/APLData/仪玄-耀嘉音-扳机.toml` 和 API session fixture，但 `tests/teams` 的 `TeamRegistry` 只注册物理 / 火 / 雷代表队，未注册 `仪玄` / Yixuan team。因此 retained-only sheer 仍缺 real registered sheer route evidence；APL 文件存在本身不关闭 `cal_base_attr(..., base_attr=4)` 的 production proposal blocker。

### RegularMul scalar candidate matrix

| Candidate | Current oracle evidence | Missing oracle rows | Reader-snapshot dependency | Registered-route eligibility | Rollback anchors | Validation triggers | Go / No-Go blockers |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `Calculator.RegularMul.cal_base_dmg()` | Formula table rows `regular-base-dmg-neutral-atk`、`regular-base-attr-static-hp`、`regular-base-dmg-dynamic-atk`；branch matrix `fire-stunned-reader-snapshot`。 | `base_attr=2` defense、`base_attr=3` AP、non-ATK / non-HP `extra_damage_ratio` and base damage increase combinations；`base_attr=4` sheer path is retained-only and must stay separate. | Snapshot-compatible for normal direct damage through static / dynamic / judge node / enemy fields. | Only after a live production semantic diff and real registered direct-damage route proves relevant nonzero counts; docs-only matrix does not run samples. | `Calculator.py::RegularMul.cal_base_dmg()` / `cal_base_attr()`、formula table、branch matrix、`formula-parity` / conditional `calculator-reads`。 | Future source/helper diff, focused oracle gap, or formula-parity failure. | Conditional Go only after exact base-attr branch and route are named; No-Go for bundled base_attr=4 / sheer replacement. |
| `Calculator.RegularMul.cal_base_attr()` for `base_attr=0/1/2/3` | Formula table covers ATK / HP representatives; source anchor `Calculator.py:864`; branch matrix exercises retained values through `_regular_mul_branch_matrix_values(...)` when reader snapshot is supported. | Defense/AP rows still missing; selected production proposal must name exactly which base attr closes first. | Snapshot-compatible for `0/1/2/3`; `_CalculatorReadSnapshot` must not gain runtime `char_instance` only to close this row. | Same as direct-damage route above. | `cal_base_attr(...)`、`_regular_mul_base_attr()` test helper、formula table、reader snapshot field set. | Future branch-specific source diff, focused base-attr regression, or `calculator-reads` failure. | Go only for one snapshot-compatible base attr at a time; No-Go if closure requires runtime character conversion fields. |
| `Calculator.RegularMul.cal_dmg_bonus()` | `regular-multipliers-neutral-zero-boundary`、`regular-dmg-bonus-character-field-stack`、branch matrix rows for element / trigger / label / all-damage representative combinations. | Physical / ice / electric / ether element rows plus minimal trigger / label cross-product for the selected route. | Snapshot-compatible; depends on `judge_node.element_type`, `trigger_buff_level`, skill labels, and dynamic bonus fields. | Only when production diff changes live damage multiplier and registered route hits the selected element / trigger. | `cal_dmg_bonus()` source, formula table cases, branch matrix, `CalculatorBuffAttributeReader` read context. | Future source/helper diff, selected element regression, formula-parity failure. | Conditional Go for one named element / trigger branch; No-Go for all-element branch explosion. |
| `Calculator.RegularMul.cal_crit_expect()` | `test_regular_mul_array_outputs_preserve_field_order_dtype_and_crit_split` records `crit_expect == 1.90` through `RegularMul.__init__` / `get_array_expect()`; source uses current `self.crit_rate` / `self.crit_dmg` at `Calculator.py:1009`. `_regular_mul_branch_matrix_values(...)` does not currently include a scalar `cal_crit_expect` key. | Dedicated rows for crit-rate clamp, full-vs-personal received-boundary interaction, over-100% crit expectation, and balancing/no-balancing same-expression behavior are still missing. | Snapshot-compatible only through the already computed `RegularMul` instance state; do not alter public reader snapshot to shortcut `self.crit_rate` / `self.crit_dmg`. | Direct crit route only after future live semantic diff proves the expectation term changes observable damage. | `cal_crit_expect()` source, `cal_crit_rate()` / `cal_crit_dmg()` no-reopen anchors, array-output oracle row. | Focused `cal_crit_expect` oracle gap, crit-family regression, or formula-parity failure. | No-Go until expectation-specific scalar rows exist; cannot reopen implemented crit-rate / crit-damage helper seams as part of this candidate. |
| `Calculator.RegularMul.cal_defense_mul()` | `regular-multipliers-neutral-zero-boundary`、`regular-defense-res-vulnerability-received-stack`、branch matrix rows for normal defense / pen / reduction; sheer `diff_multiplier=4` remains separate. | Attacker-level clamp / representative level row; `cal_recipient_def()` addon parameters only if a future slice selects them explicitly. | Snapshot-compatible for non-sheer; sheer route skips defense multiplier but still couples through retained-only base attr elsewhere. | Direct damage route must observe defense-affecting damage. | `cal_defense_mul()`、`cal_recipient_def()` source anchor、formula table defense stack row、branch matrix. | Source/helper diff, level/clamp regression, formula-parity failure. | Conditional Go only for exact defense branch; No-Go if proposal needs broad recipient-defense API rewrite. |
| `Calculator.RegularMul.cal_res_mul()` | `regular-multipliers-neutral-zero-boundary`、`regular-defense-res-vulnerability-received-stack`、branch matrix fire / ether representative resistance and all decrease / all pen rows. | Physical / ice / electric rows and `snapshot_res_pen` override row; frost / element aliasing must not be inferred from anomaly `cal_res_pen()`. | Snapshot-compatible through enemy element resistance and dynamic res decrease / pen fields. | Direct damage route must bind target element and prove route relevance. | `cal_res_mul()` source, formula table res stack rows, branch matrix, registered-route decision matrix. | Source/helper diff, element-specific oracle gap, formula-parity failure. | Conditional Go for one named element / override branch; No-Go for broad resistance replacement. |
| `Calculator.RegularMul.cal_dmg_vulnerability()` | `regular-multipliers-neutral-zero-boundary`、`regular-defense-res-vulnerability-received-stack`、branch matrix element vulnerability + all vulnerability rows. | Physical / ice / electric deterministic rows; if one element is selected, only that element row should be added. | Snapshot-compatible through `dynamic.<element>_vulnerability` and `all_vulnerability`. | Direct damage route must bind element / skill route. | `cal_dmg_vulnerability()` source, formula table vulnerability rows, branch matrix. | Source/helper diff, element-specific vulnerability regression, formula-parity failure. | Conditional Go only for named element route; No-Go for bundled all-vulnerability migration. |
| `Calculator.RegularMul.cal_stun_vulnerability()` | Branch matrix records retained values for stunned / all-time representative cases; source anchor `Calculator.py:1186`. | Dedicated neutral, non-stunned, stunned-only, all-time-only, and combined rows are still missing. | Snapshot-compatible through dynamic stun vulnerability fields and enemy stun state; must stay separate from `StunMul.get_stun_array()`. | Only after a live semantic diff and real registered route prove observable stun-vulnerability damage relevance. | `cal_stun_vulnerability()` source, branch matrix, Stun no-reopen anchors. | Source/helper diff, focused stun-vulnerability oracle gap, formula-parity failure. | No-Go for production proposal until oracle rows separate stunned vs all-time semantics; cannot reopen selected Stun array implementation without named evidence. |
| `Calculator.RegularMul.cal_special_mul()` | Branch matrix records retained `special_multiplier_zone` representatives; source returns `1 + data.dynamic.special_multiplier_zone` at `Calculator.py:1204`. | Neutral / no-zone and selected real-zone rows are still needed before proposal. | Snapshot-compatible through `dynamic.special_multiplier_zone`; no reader public API expansion required. | Only when registered route enters a relevant special multiplier zone after a production diff. | `cal_special_mul()` source, branch matrix, dynamic statement field anchor. | Source/helper diff, focused special-zone oracle gap, formula-parity failure. | Conditional Go for one named special-zone branch; No-Go for broad multiplier-zone rewrite. |

### Retained-only sheer follow-up status

| Candidate | Current oracle evidence | Missing oracle rows | Reader-snapshot dependency | Registered-route eligibility | Rollback anchors | Validation triggers | Go / No-Go blockers |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `Calculator.RegularMul.cal_base_attr(..., base_attr=4)` | `test_regular_mul_retained_sheer_base_attr_requires_char_instance_conversion_rate()` proves retained runtime dependency on `char_instance.sheer_attack_conversion_rate`; branch matrix keeps `[sheer-retained-only]` separate from reader-snapshot-supported rows. | Architecture-approved reader-contract plan for runtime `char_instance.sheer_attack_conversion_rate`; real registered sheer route; additional numeric rows alone are insufficient. | Not snapshot-compatible today because `_CalculatorReadSnapshot` does not carry runtime `char_instance`. | Current No-Go: no real registered `仪玄` / Yixuan route in `tests/teams`; do not create validation-only team. | `cal_base_attr(..., 4)` source, retained sheer test, `_CalculatorReadSnapshot` field set, registered-route audit docs. | Future reader-contract proposal, real route evidence, focused retained sheer regression, or `calculator-reads` failure. | No-Go until both reader-contract and registered-route blockers close. |
| `Calculator.RegularMul.cal_sheer_dmg_bonus(data)` | Branch matrix expects `1.45` for `diff_multiplier=4`; retained sheer test proves reader snapshot can still compute the simple multiplier while companion base attr remains runtime-dependent. | If selected alone, add neutral `diff_multiplier != 4` and selected `diff_multiplier == 4` rows tied to the chosen route; still cannot close the full sheer path without base_attr=4 plan. | Snapshot-compatible alone via `dynamic.sheer_dmg_bonus`, but not proposal-ready for the whole sheer damage path. | Same Yixuan / sheer-route No-Go as above. | `cal_sheer_dmg_bonus()` source, branch matrix, retained sheer test, docs rollback anchors. | Future source/helper diff, focused sheer multiplier regression, formula-parity failure. | Conditional Go only for documenting/guarding the multiplier; No-Go for production sheer replacement without base_attr=4 solution. |

## Current retained-only sheer blocker closure US-008 final handoff

结论：helper seam completed / retained-blocked。This PRD only added private module-local helper seams and durable evidence; it did not authorize a live production replacement, public reader/snapshot expansion, registered fixture, validation-runner rewrite, event/runtime/listener edit, old-container deletion, or retained compatibility cleanup.

| Evidence area | Final retained-only sheer state |
| --- | --- |
| Oracle evidence | `test_regular_mul_retained_sheer_base_attr_requires_char_instance_conversion_rate()` locks runtime `char_instance.sheer_attack_conversion_rate`, recursive non-sheer base-attr conversion, `dynamic.sheer_atk`, and reader-snapshot `AttributeError`; branch matrix row `sheer-retained-only` preserves selected branch values; `_FORMULA_ORACLE_TABLE_CASES` row `regular-sheer-dmg-bonus-retained-snapshot` locks retained / reader-snapshot `dynamic.sheer_atk=5.0`, `dynamic.sheer_dmg_bonus=0.45`, and `cal_sheer_dmg_bonus(data)=1.45`. |
| Helper evidence | `Calculator.RegularMul.cal_base_attr(..., base_attr=4)` delegates to `_calculate_sheer_base_attribute(...)`; `Calculator.RegularMul.cal_sheer_dmg_bonus(data)` delegates to `_calculate_sheer_damage_bonus(...)`; public `RegularMul` signatures and current `SkillNode` / `MultiplierData` assumptions stay unchanged. |
| Reader / snapshot No-Go | `CalculatorBuffAttributeReader` still exposes only anomaly mastery/proficiency, impact, full crit rate, personal crit rate, and personal crit damage readers; `_CalculatorReadSnapshot` remains private with `static`, `dynamic`, `judge_node`, `enemy_obj`, and `char_level`; no `char_instance` or sheer/base-attr public reader was added. |
| Registered-route No-Go | Real Yixuan / sheer source data exists, but current registered consistency teams/APLs do not include a real route that reaches the selected `diff_multiplier=4` sheer path; no validation-only team, fake APL, fixture-only route, retained-vs-retained sample, or production semantic change was created. |
| Validation evidence | Focused retained sheer pytest exited `0` with `32 passed, 130 deselected`; scoped mypy over `Calculator.py` and focused tests exited `0` with `Success: no issues found in 2 source files`; serial `formula-parity` exited `0` with focused formula slice `162 passed` and mypy clean for `9 source files`; serial `calculator-reads` exited `0` with focused reader / guardrail slice `262 passed` and mypy clean for `22 source files`. |
| Next-intake signal | Retained-only sheer is helper-complete but remains blocked for public reader/snapshot expansion and real registered-route sample. Next default is Phase-3 same-phase candidate-pool closure / exact bounded candidate selection; keep retained-only sheer as pool evidence, not active default. |

## Current US-003 Direct-Damage Branch Audit

2026-06-15 14:01 +08:00 US-003 audit：current-root `Calculator.RegularMul` non-sheer direct-damage branches were re-inspected with targeted `rg` and CodeGraph. This story records coverage / gaps only; it does not select a production formula candidate and does not authorize any `Calculator.py`, reader, test, registered-route, validation-runner, old-container, event/runtime/listener, copied-output, or retained compatibility change.

| Surface | Already deterministic in `tests/simulator/test_buff_attribute_reader.py` | Needs focused rows before production proposal | US-003 verdict |
| --- | --- | --- | --- |
| `cal_base_dmg()` / non-sheer `cal_base_attr(0/1/2/3)` | `regular-base-dmg-neutral-atk`, `regular-base-attr-static-hp`, `regular-base-dmg-dynamic-atk`, branch matrix `fire-stunned-reader-snapshot`, and array `base_dmg`. | `base_attr=2`, `base_attr=3`, and exact selected non-ATK / non-HP route combinations. | Conditional candidate only after one exact non-sheer base attr is named; `base_attr=4` remains retained-only sheer contrast. |
| `cal_dmg_bonus()` | `regular-multipliers-neutral-zero-boundary`, `regular-dmg-bonus-character-field-stack`, `regular-defense-res-vulnerability-received-stack`, branch matrix representatives, and array `dmg_bonus`. | Physical / ice / electric / ether selected element row plus minimal selected trigger / label cross-product. | Conditional candidate for one element / trigger / label branch; No-Go for all-element branch explosion. |
| `cal_crit_expect()` | Array-output oracle records `crit_expect == 1.90` through `RegularMul.__init__` / `get_array_expect()`. | Scalar rows for clamp, over-100% full crit, full-vs-personal received-boundary interaction, and balancing/no-balancing same-expression behavior. | No-Go until expectation-specific scalar rows exist; completed crit-rate / crit-damage helper seams stay contrast-only. |
| `cal_defense_mul()` | Neutral and `regular-defense-res-vulnerability-received-stack` formula rows, branch matrix representative, and array `defense_mul`. | Attacker-level clamp and selected recipient-defense / pen route row. | Conditional candidate only for one exact non-sheer defense route. |
| `cal_res_mul()` | Neutral and defense/res/vulnerability stack formula rows, branch matrix fire / ether representatives, and array `res_mul`. | Physical / ice / electric selected element row and `snapshot_res_pen` row if selected. | Conditional candidate for one named element or override; do not infer from anomaly `cal_res_pen()`. |
| `cal_dmg_vulnerability()` | Neutral and defense/res/vulnerability stack formula rows, branch matrix fire / ether representatives, and array `dmg_vulnerability`. | Physical / ice / electric selected element row. | Conditional candidate for one named element route only. |
| `cal_stun_vulnerability()` | Branch matrix stunned / retained-only representatives and array non-stunned all-time representative. | Dedicated neutral, non-stunned, stunned-only, all-time-only, and combined rows. | No-Go until scalar stun-vulnerability rows separate stunned vs all-time semantics; do not reopen Stun array implementation. |
| `cal_special_mul()` | Branch matrix nonzero representatives and array `special_multiplier_zone`. | Dedicated neutral/no-zone and selected real-zone rows. | Conditional candidate after exact special-zone row pair; no broad multiplier-zone rewrite. |

Next slice signal：continue `US-004` retained-only sheer No-Go revalidation first. Later `US-006` should select one exact surface or record No-Go; if the chosen surface still lacks rows, `US-007` must define / add the focused oracle and typecheck contract before any production proposal.

### Retained pool entries

| Candidate | Current oracle evidence | Missing oracle rows | Reader-snapshot dependency | Registered-route eligibility | Rollback anchors | Validation triggers | Go / No-Go blockers |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Registered behavior sample eligibility | US-003 registered-sample audits and current docs define conditional No-Go policy; current registered teams are physical / fire / thunder representatives, not a blanket formula oracle substitute. | Candidate-specific nonzero route count, explicit stop tick, selected formula relevance, and live production semantic diff. | Candidate-specific; samples must not replace deterministic reader / formula rows. | Run only after exact candidate diff and real registered route prove relevance; no validation-only teams. | `tests/teams`, `zsim/data/APLData`, `zsim/utils/main_loop_consistency.py`, registered-sample investigations, this matrix. | Future live semantic diff plus route preflight; main-loop consistency failure. | Retained pool entry only; No-Go as default implementation work in this docs slice. |
| P2-A through P2-G guarded maintenance | Checklist and guardrail tests record P2-A / P2-B reader parity, P2-G direct-context guardrails, and completed guarded buckets. | Only concrete guardrail, focused test, source preflight, or validation failure can name new rows. | Not a RegularMul reader-snapshot candidate; retained as compatibility / regression guard. | Not registered-route work by default. | P2 guardrail tests, `implicit-events`, old Buff coupling review, checklist, replacement notes. | Guardrail failure, focused pytest failure, scoped mypy failure, or new source evidence naming a specific P2 bucket. | Retained pool entry only; No-Go for reopening P2-A through P2-G as default implementation backlog. |
| Future `Calculator.StunMul.get_stun_array()` follow-up | Current source delegates to `_build_stun_multiplier_array(...)`; `test_stun_array_output_contract_preserves_field_order_dtype_and_product()` characterizes field order, dtype, single read, and `Calculator.cal_stun()` product consumer; current handoff marks selected implementation implemented / no-op verified. | None for default backlog; new rows only if named evidence, focused regression, or validation failure identifies a Stun array gap. | Stun array contract is separate from RegularMul scalar reader snapshots. | Reopen only with future live Stun semantic diff and real registered stun / impact route proving nonzero relevance. | `Calculator.StunMul.get_stun_array()`、`Calculator.cal_stun()`、Stun array focused test、current Stun implementation handoff docs. | Named source evidence, focused Stun array regression, formula-parity / calculator-reads / implicit-events failure. | Reopenable only; No-Go for default implementation work in this PRD absent named evidence. |
| Retained compatibility / blocker-only reopen rules | Long-lived Buff docs keep `MultiplierData`, `MulData`, `DynamicStatement`, old containers, copied-output constructors, `ScheduleDispatchPort`, `RuntimeCommandPort`, `LegacyRuntimeCommandAdapter`, `BuffRuntimeReadPort`, and `LegacyBuffRuntimeFacade` as retained anchors. | Only exact file / symbol / failure evidence can name a blocker-specific row. | Candidate-specific; no blanket reader snapshot expansion. | Candidate-specific; registered sample is conditional evidence, not a route generator. | `docs/Buff重构方案.md`, `docs/旧Buff系统耦合审查结果.md`, checklist, replacement notes, formula matrix, Ralph evidence ledger. | Validation failure, source preflight naming a new coupling, or reviewer invariant failure. | Retained pool entry only; No-Go for broad cleanup, old-container deletion, layer merge, or validation-runner rewrite from this docs matrix. |

## Current same-phase candidate selection PRD US-006 exact bounded decision

2026-06-15 14:46 +08:00 结论：选择 `Calculator.RegularMul.cal_dmg_bonus(data)` 的 fire / normal attack / `aftershock_attack` / all-damage stack 作为唯一 exact bounded proposal candidate；本 story 只选择下一步 oracle / typecheck / proposal contract 面，不授权 production source diff 或 live main-loop sample。记录位置：`scripts/ralph/investigations/2026-06-15-US-006-select-one-exact-bounded-candidate.md`。

| Contract area | US-006 decision |
| --- | --- |
| Exact files / symbols | `zsim/sim_progress/ScheduledEvent/Calculator.py::Calculator.RegularMul.cal_dmg_bonus(data)`；supporting test file `tests/simulator/test_buff_attribute_reader.py`。 |
| Exact branch | `element_type=1` fire、`trigger_buff_level=0` normal attack、`skill.labels["aftershock_attack"] == 1`、`dynamic.all_dmg_bonus`，对应 `regular-dmg-bonus-character-field-stack`。 |
| Focused tests | `tests/simulator/test_buff_attribute_reader.py::test_formula_oracle_table_cases_drive_expected_fields_and_reader_parity[regular-dmg-bonus-character-field-stack]`；supporting reader-snapshot matrix node `::test_calculator_regular_mul_branch_matrix_characterizes_selected_methods[fire-stunned-reader-snapshot]`。 |
| Scoped mypy target | `uv run python -m mypy zsim/sim_progress/ScheduledEvent/Calculator.py tests/simulator/test_buff_attribute_reader.py --follow-imports skip --ignore-missing-imports`。 |
| Retained validation gates | This docs selection runs focused pytest + scoped mypy；later formula source diff must run `formula-parity`；later reader / snapshot diff must also run `calculator-reads`；event / dispatch-runtime / listener / copied-output diff must add `implicit-events`。 |
| Registered sample condition | Current slice keeps main-loop consistency No-Go。Future live semantic diff may use only a real registered route with explicit stop tick、runtime labels、total damage comparison、full event counts、Buff timeline comparison、differences、rollback anchors and nonzero selected `cal_dmg_bonus()` relevance in both snapshots；no validation-only team、fake APL、fixture-only route or retained-vs-retained sample。 |
...
结论：选择 `Calculator.RegularMul.cal_personal_crit_dmg(data)` 作为当前唯一 exact candidate，供后续 oracle-gap / proposal-boundary slice 继续推进。本结论只选择候选，不授权生产公式替换；已实现 / no-op verified 的 `Calculator.RegularMul.cal_crit_rate(data)` 保持关闭，`cal_personal_crit_rate()`、`cal_crit_dmg()`、`cal_crit_expect()`、damage bonus / defense / resistance / vulnerability / base damage / retained-only sheer / array output 都保持分离边界。

| Contract area | Current US-003 decision |
| --- | --- |
| Exact source symbols | Selected source: `zsim/sim_progress/ScheduledEvent/Calculator.py::Calculator.RegularMul.cal_personal_crit_dmg(data)`；reader anchor: `CalculatorBuffAttributeReader.read_personal_crit_damage(context)`；snapshot anchor: `_CalculatorReadSnapshot` carrying static / dynamic / judge / enemy / level fields through `MultiplierData`-compatible fields. |
| Full vs personal crit boundary | Personal crit damage must exclude `received_crit_dmg_bonus`。Full `cal_crit_dmg(data)` remains the contrast branch that includes received crit damage；implemented `cal_crit_rate(data)` / `_calculate_full_crit_rate(...)` is not reopened. |
| Focused tests | Retain `tests/simulator/test_buff_attribute_reader.py::test_p2b_personal_crit_damage_excludes_received_crit_damage_bonus` and `::test_calculator_regular_mul_crit_formula_families_preserve_received_boundaries` as the focused oracle / reader proof set. |
| Retained validation gates | Focused pytest plus focused mypy are sufficient for this docs-only selection. `formula-parity` remains the retained formula gate for later formula work；`calculator-reads` is required only if reader seams or snapshot construction change；`implicit-events`, lifecycle, and main-loop consistency remain conditional. |
| Rollback anchors | Current bodies of `Calculator.RegularMul.cal_personal_crit_dmg(data)`, `CalculatorBuffAttributeReader.read_personal_crit_damage(context)`, `_CalculatorReadSnapshot`, focused crit-family boundary tests, and this section. |
| Registered sample conditions | Do not run a sample in US-003. A later live sample is allowed only after a production semantic diff exists and a real registered direct-damage route proves selected-branch relevance with explicit stop tick, runtime labels, total damage comparison, relevant nonzero count, event counts, and Buff timeline comparison. No validation-only team, fake APL, fixture-only route, or retained-vs-retained sample. |
| Non-selected same-phase pool | `cal_base_dmg()` / `cal_base_attr()`, `cal_dmg_bonus()`, implemented `cal_crit_rate()`, `cal_personal_crit_rate()`, `cal_crit_dmg()`, `cal_defense_mul()`, `cal_res_mul()`, `cal_dmg_vulnerability()`, retained-only sheer, arrays, registered behavior sample eligibility, P2 guarded maintenance, and retained compatibility remain follow-up pool, not deleted scope. |
| Stop conditions | Stop and record No-Go if follow-up work requires `_CalculatorReadSnapshot` public contract expansion, validation-runner rewiring, retained compatibility deletion, `cal_crit_rate()` reopening, full `cal_crit_dmg()` bundling, `cal_crit_expect()` semantics, damage bonus / defense / resistance / vulnerability cross-product expansion, retained-only sheer route work, old Buff container deletion, or event/runtime/listener layer merge. |

## Current US-005 selected RegularMul proposal boundary / rollback anchors

结论：future bounded implementation PRD 的 production proposal boundary 只限 `zsim/sim_progress/ScheduledEvent/Calculator.py::Calculator.RegularMul.cal_personal_crit_dmg(data)`。当前 US-005 只记录 boundary / rollback contract，不改 production formula、reader source、focused tests、validation runner、registered teams/APLs、copied-output constructors、old Buff containers 或 event/runtime/listener/dot/lifecycle 分层。

| Contract area | Current US-005 boundary |
| --- | --- |
| Selected file / symbol | `zsim/sim_progress/ScheduledEvent/Calculator.py::Calculator.RegularMul.cal_personal_crit_dmg(data)`；current root line is `998` and the retained formula remains `static.crit_damage + dynamic.crit_dmg + dynamic.field_crit_dmg`。 |
| Helper under consideration | A later implementation may introduce only a module-local scalar helper such as `_calculate_personal_crit_damage(static_statement, dynamic_statement)` if it preserves the exact expression above. That helper must not accept `received_crit_dmg_bonus`, `judge_node`, `enemy`, `char_instance`, runtime views, arrays, or copied-output payloads. |
| Reader / snapshot anchors | `CalculatorBuffAttributeReader.read_personal_crit_damage(context)` builds `_CalculatorReadSnapshot` through `_build_formula_snapshot(context)` and delegates to the selected retained formula. `_CalculatorReadSnapshot` stays private and limited to static / dynamic / judge_node / enemy_obj / char_level; no `char_instance` or public field expansion is authorized. |
| Focused test rows | Story-local exact nodes: `test_p2b_parity_fixture_matches_old_personal_crit_damage_helper`, `test_p2b_personal_crit_damage_excludes_received_crit_damage_bonus`, `test_calculator_regular_mul_crit_formula_families_preserve_received_boundaries`, `test_calculator_regular_mul_branch_matrix_characterizes_selected_methods`, and `test_calculator_attribute_formula_boundaries_remain_retained_compatibility` in `tests/simulator/test_buff_attribute_reader.py`。US-004 verifier nodes also keep `test_formula_oracle_table_cases_drive_expected_fields_and_reader_parity` and the conditional retained-only sheer node as contrast evidence, not selected implementation scope. |
| Retained profiles / focused pytest targets | `formula-parity` keeps `FORMULA_PARITY_FOCUSED_TEST_TARGETS = ["tests/simulator/test_buff_attribute_reader.py"]`。`calculator-reads` keeps `test_buff_attribute_reader.py`, `test_buff_raw_container_guardrail.py`, `test_migrated_am_ap_reader_guardrail.py`, `test_migrated_p2b_reader_guardrail.py`, `test_buff_attribute_state_sync.py`, and `test_full_crit_event_adjacent_reader.py` as focused pytest / focused mypy targets. |
| Scoped mypy targets | `formula-parity`: `Calculator.py`, `CalAnomaly.py`, `anomaly_bar/__init__.py`, `AnomalyBarClass.py`, `CopyAnomalyForOutput.py`, `BranchBladeSongCritDamageBonus.py`, `TimeweaverDisorderDmgMul.py`, `scripts/run_buff_refactor_validation.py`, plus `test_buff_attribute_reader.py` through focused mypy. `calculator-reads`: `Calculator.py`, AM/AP/P2-B reader source files (`AliceAdditionalAbilityApBonus.py`, `BranchBladeSongCritDamageBonus.py`, `LighterAdditionalAbility_IceFireBonus.py`, `MiyabiCoreSkill_IceFire.py`, `QingYiAdditionalAbilityStunConvertToATK.py`, `CannonRotor.py`, `TriggerAdditionalAbilityStunBonus.py`, `WoodpeckerElectroSet4_NA.py`, `WoodpeckerElectroSet4_E_EX.py`, `WoodpeckerElectroSet4_CA.py`, `YuzuhaAdditionalAbilityAnomalyDmgBonus.py`, `YuzuhaAdditionalAbilityAnomalyBuildupBonus.py`, `Soldier0AnbyCoreSkillCritDMGBonus.py`, `TimeweaverDisorderDmgMul.py`), `scripts/run_buff_refactor_validation.py`, plus the focused test files listed above. |
| Rollback anchors | Selected method body, proposed helper if added later, `_regular_mul_branch_matrix_values(...)[\"cal_personal_crit_dmg\"]`, the full `cal_crit_dmg(data)` received-damage contrast row, personal crit damage reader parity rows, `CalculatorBuffAttributeReader.read_personal_crit_damage(context)`, `_build_formula_snapshot(context)`, `_CalculatorReadSnapshot`, this doc, `docs/Buff重构替换说明.md`, `docs/Buff系统重构Checklist.md`, `docs/Buff重构下阶段计划草稿.md`, `docs/旧Buff系统耦合审查结果.md`, the Ralph investigation/checkpoint for US-005, `formula-parity`, and `calculator-reads`. |
| Registered sample conditions | Do not run a main-loop sample in this docs-only slice. A future sample is allowed only for a live production semantic diff with a real registered direct-damage route, explicit stop tick, legacy/candidate labels, total damage comparison, relevant nonzero selected formula or event count, and Buff timeline comparison. No validation-only team, fake APL, fixture-only route, retained-vs-retained sample, or sample whose selected formula/event count is zero. |
| Retained boundaries | Keep `ScheduleDispatchPort`, `RuntimeCommandPort`, `LegacyRuntimeCommandAdapter`, `BuffRuntimeReadPort`, `LegacyBuffRuntimeFacade`, copied-output constructors, old Buff containers, `buff_add()`, `KickOutBuff()`, listener broadcast, dot runtime registration/removal, `MultiplierData`, `MulData`, `DynamicStatement`, `Calculator.RegularMul.cal_crit_dmg(data)`, implemented `cal_crit_rate(data)` / `_calculate_full_crit_rate(...)`, retained-only sheer, and all non-selected `RegularMul` branches as retained boundaries. |
| No-Go triggers | Stop and record No-Go if implementation requires broad interface change, phase boundary crossing, deletion/removal boundary, performance-sensitive rewrite, `_CalculatorReadSnapshot` expansion, public reader/runtime contract change, validation-runner rewrite, registered-team/APL creation, old-container deletion, event/runtime/listener layer merge, full `cal_crit_dmg(data)` bundling, `received_crit_dmg_bonus` entering the personal formula/helper, retained-only sheer shortcut, or a failed retained gate without a smaller rollback slice. |
...
## Current proposal-readiness PRD US-008 final handoff

本 handoff 属于当前 RegularMul remaining-branch bounded proposal-readiness PRD。US-008 不改 production source；它把 US-003 至 US-007 的 selected candidate、focused oracle、proposal boundary、retained gates、registered-sample decision、rollback anchors 和 reviewer verdict 同步到 long-lived Buff docs。结论：Conditional Go for one later bounded implementation PRD limited to `Calculator.RegularMul.cal_personal_crit_dmg(data)` / `CalculatorBuffAttributeReader.read_personal_crit_damage(context)`；该 Go 不授权 broad `Calculator.py` / `CalAnomaly.py` rewrite、RegularMul branch bundling、full crit damage replacement、`cal_crit_rate(data)` reopening、retained-only sheer shortcut、registered-team fixture creation、validation-runner rewrite、old-container deletion、layer merge 或 retained compatibility deletion。

| Contract area | US-008 final handoff |
| --- | --- |
| Selected candidate outcome | `Calculator.RegularMul.cal_personal_crit_dmg(data)` is the only selected implementation candidate. The selected formula must remain `static.crit_damage + dynamic.crit_dmg + dynamic.field_crit_dmg` and must continue to exclude `received_crit_dmg_bonus`; full `Calculator.RegularMul.cal_crit_dmg(data)` remains the received-damage contrast branch. |
| Refreshed matrix status | `Calculator.RegularMul.cal_crit_rate(data)` / `_calculate_full_crit_rate(...)` remains implemented / no-op verified evidence only. Non-selected rows in the current matrix remain characterized or blocked as documented; retained-only sheer remains No-Go without reader-contract and real registered Yixuan route evidence. |
| Retained gates | Focused selected personal-crit-damage pytest remains the first proof layer. US-006 retained validation gates passed serially: focused reader pytest `143 passed`, `formula-parity` exited `0` with scoped mypy success on `9 source files`, and `calculator-reads` exited `0` with scoped mypy success on `22 source files`. `implicit-events` is conditional on copied-output / event / dispatch / runtime / listener surfaces; default validation is conditional on lifecycle / runtime-write / validation-runner behavior changes. |
| Registered-sample decision | Conditional No-Go. No live production semantic diff exists in this docs-only handoff, and current main-loop output lacks formula-call counts. A future sample must use a real registered direct-crit route, explicit stop tick, runtime labels, total damage comparison, nonzero selected-branch relevance, and Buff timeline comparison; no validation-only team, fake APL, fixture-only route, or retained-vs-retained sample. |
| Rollback anchors | `Calculator.RegularMul.cal_personal_crit_dmg(data)`, optional future local scalar helper if authorized, `CalculatorBuffAttributeReader.read_personal_crit_damage(context)`, `_CalculatorReadSnapshot`, full-vs-personal crit damage focused tests, branch matrix row, retained Buff docs, `formula-parity`, `calculator-reads`, old containers, and layer-separation invariants. |
| Reviewer verdict | PASS from US-007: changed files stayed in the proposal / evidence boundary, event queue semantics, synchronous listener broadcasts, and same-tick runtime writes remain separate layers, and no new raw queue or container passthrough was introduced. |
| Go / No-Go verdict | Conditional Go for one bounded implementation PRD only. Stop and split if implementation needs `_CalculatorReadSnapshot` public contract expansion, received-crit inclusion, `cal_crit_expect()` semantics, full crit damage / crit rate / damage bonus / defense / resistance / vulnerability bundling, retained-only sheer route work, validation-runner rewrite, registered-team fixture creation, old-container deletion, event/runtime/listener layer merge, or retained compatibility deletion. |
...
## Current personal crit damage implementation PRD US-008 final handoff

本 handoff 属于当前 RegularMul personal crit damage bounded implementation PRD。结论：Implemented / no-op verified at handoff。`Calculator.RegularMul.cal_personal_crit_dmg(data)` 已委托 `_calculate_personal_crit_damage(data.static, data.dynamic)`，helper 只计算 `static.crit_damage + dynamic.crit_dmg + dynamic.field_crit_dmg`；`CalculatorBuffAttributeReader.read_personal_crit_damage(context)` 仍通过 reader-built formula snapshot 委托该 public formula path。full `Calculator.RegularMul.cal_crit_dmg(data)` 仍是包含 `received_crit_dmg_bonus` 的 contrast branch。

| Contract area | US-008 implementation handoff |
| --- | --- |
| Implemented / no-op verified surface | `zsim/sim_progress/ScheduledEvent/Calculator.py::Calculator.RegularMul.cal_personal_crit_dmg(data)` and module-local `_calculate_personal_crit_damage(static_statement, dynamic_statement)` only. |
| Exact validation commands | `uv run pytest tests/simulator/test_buff_attribute_reader.py -q -k "personal_crit_damage or regular_mul_branch_matrix or crit_formula_families"`；`uv run python -m mypy zsim/sim_progress/ScheduledEvent/Calculator.py tests/simulator/test_buff_attribute_reader.py --follow-imports skip --ignore-missing-imports`；focused docs `git diff --check -- docs/Buff重构下阶段计划草稿.md docs/Buff系统重构Checklist.md docs/Buff公式候选与测试目标清单.md docs/Buff重构替换说明.md`；Ralph JSON sanity；UTF-8 / U+FFFD / mojibake scan. |
...
本 handoff 属于当前 RegularMul personal crit rate bounded implementation PRD。结论：Implemented / no-op verified。`Calculator.RegularMul.cal_personal_crit_rate(data)` now delegates to module-local `_calculate_personal_crit_rate(static_statement, dynamic_statement)`；helper 必须保持 `static.crit_rate + dynamic.crit_rate + dynamic.field_crit_rate` 并继续排除 `crit_rate_received_increase`。

| Contract area | US-008 final handoff |
| --- | --- |
| Selected candidate outcome | `Calculator.RegularMul.cal_personal_crit_rate(data)` is implemented through `_calculate_personal_crit_rate(...)` and is no longer the next default backlog item. Full `Calculator.RegularMul.cal_crit_rate(data)` / `_calculate_full_crit_rate(...)` remains the received-crit contrast branch and is not reopened. |
| Retained gates | Current handoff focused pytest exited `0` with `10 passed, 135 deselected`; scoped mypy over `Calculator.py` and focused tests exited `0` with `Success: no issues found in 2 source files`. Prior retained `formula-parity` remains rollback evidence; `calculator-reads` remains conditional on reader/snapshot/guardrail changes; `implicit-events` remains conditional on copied-output, event, dispatch, runtime, listener, dot-runtime, lifecycle, validation-runner, old-container, or retained compatibility surfaces. |
| Registered-sample decision | Conditional No-Go. There is no live production semantic diff in this docs-only handoff, and no real registered route currently proves nonzero selected personal-crit-rate relevance inside an explicit stop tick. No validation-only team, fake APL, fixture-only route, or retained-vs-retained sample is allowed. |
| Rollback anchors | Current `Calculator.RegularMul.cal_personal_crit_rate(data)` body, `_calculate_personal_crit_rate(...)`, `CalculatorBuffAttributeReader.read_personal_crit_rate(context)`, `_CalculatorReadSnapshot`, full/personal crit focused tests, formula candidate matrix row, retained Buff docs, `formula-parity`, `calculator-reads`, old containers, and layer-separation invariants. |
...
## Current CalAnomaly copied-output formula helper-family PRD US-008 final handoff

- Implemented / no-op verified surfaces:
  - `CalAnomaly.set_final_multipliers(...)` / `cal_anomaly_dmg()` 已通过 `_assemble_final_multiplier_vector(...)` 与 `_calculate_anomaly_damage_expectation(...)` helper seams 保留 final multiplier order、snapshot impact / stun bonus division 和 `AnomalyBar.scaling_factor` 行为。
  - `CalDisorder.cal_disorder_base_dmg(...)`、`cal_disorder_extra_mul()`、`cal_disorder_stun()`、`CalPolarityDisorder.cal_polarity_disorder_base_dmg(...)` 与 `CalAbloom` ratio multiplication 已收口为 module-local helper seams，public class/method anchors 和 copied payload formula inputs 保持不变。
- Validation evidence:
  - Focused CalAnomaly helper-family pytest exited `0` with `18 passed`.
  - Scoped mypy over `CalAnomaly.py` and `test_buff_attribute_reader.py` exited `0` with `Success: no issues found in 2 source files`.
  - Serial `formula-parity` exited `0` with focused formula regression and scoped mypy success on `9 source files`; serial `calculator-reads` exited `0` with focused reader regression and scoped mypy success on `22 source files`.
  - `implicit-events` and full validation were skipped by condition because this PRD did not change copied-output constructors, `UpdateAnomaly.py` production behavior, handler/report payloads, event/runtime/listener layers, lifecycle containers, validation-runner behavior, old containers, or retained compatibility.
- Rollback anchors:
  - `zsim/sim_progress/ScheduledEvent/CalAnomaly.py` helper seams and retained public methods, focused CalAnomaly / CalDisorder / CalPolarityDisorder / CalAbloom nodeids in `tests/simulator/test_buff_attribute_reader.py`, retained `formula-parity` / `calculator-reads` profiles, and handoff docs.
- Retained boundaries:
  - `AnomalyBar.current_ndarray` write-path field matrix, `UpdateAnomaly.py` mode / publish / listener / reset paths, copied-output constructors, handler report payloads, `ScheduleDispatchPort`, `RuntimeCommandPort`, listener broadcast, dot runtime registration, old containers and retained compatibility paths remain unchanged.
- Next-step routing:
  - Default next PRD should build the `AnomalyBar.current_ndarray` / `UpdateAnomaly.py` write-path field matrix as bounded characterization or same-PRD Go / No-Go closure.
  - Same-phase pool remains: enemy dynamic / duration reads, retained-only sheer blocker package if reader-contract and real route evidence appear, registered behavior sample eligibility only for live semantic diffs, P2-A through P2-G guarded maintenance only on concrete blockers, retained compatibility, and blocker-only phase-1 reopen rules.

## Current AnomalyBar / UpdateAnomaly write-path field matrix PRD US-007 final handoff

- Implemented / characterized surfaces:
  - `AnomalyBar.current_ndarray` lifecycle / settlement / reset / copy matrix and `UpdateAnomaly.py` mode 0 / 1 / 2 write-path field matrix are retained-behavior characterized.
  - `UpdateAnomaly.update_anomaly(...)`, `spawn_output(...)`, `anomaly_effect_active(...)`, and `remove_dots_cause_disorder(...)` remain layer-separated: copied-output construction, listener broadcast, scheduled publish, dot runtime registration/removal, and same-tick Buff / Debuff writes are not merged.
- Same-PRD No-Go:
  - US-006 records No-Go for additional production work inside this PRD. No production source, focused test source, validation-runner behavior, copied-output constructor, handler/report payload, public reader/snapshot API, registered team/APL, old-container path, event/runtime/listener layer merge, or retained compatibility cleanup is authorized by this handoff.
- Validation evidence:
  - Focused pytest over selected `test_buff_attribute_reader.py`, `test_update_anomaly_dispatch.py`, and `test_anomaly_handler_runtime_view.py` nodeids exited `0` with `50 passed`.
  - Scoped mypy over `AnomalyBarClass.py`, `UpdateAnomaly.py`, `CalAnomaly.py`, `Calculator.py`, and focused tests exited `0` with `Success: no issues found in 7 source files`.
  - Serial `formula-parity`, `implicit-events`, and `calculator-reads` profiles exited `0`; full validation and registered main-loop consistency were skipped by unchanged lifecycle/runtime/old-container/runner surfaces and no live semantic diff / real registered route evidence.
  - US-007 adds docs / Ralph evidence gates: focused diff check, Ralph JSON sanity, UTF-8 / mojibake scan, scoped Ralph tooling typecheck, checkpoint, reviewer / invariant verdict, and completion bookkeeping.
- Rollback anchors:
  - `AnomalyBarClass.py` current-ndarray / duration-read anchors, `UpdateAnomaly.py` write-path anchors, copied-output constructors, focused tests in the three simulator files, retained validation profiles, `scripts/ralph/investigations/2026-06-16-US-006-serial-validation-go-no-go.md`, and long-lived Buff handoff docs.
- Reviewer / invariant verdict:
  - PASS. Changed files stay inside docs / Ralph evidence / bookkeeping. Event queue semantics, synchronous listener broadcasts, same-tick runtime writes, explicit ports/adapters, old containers, validation-runner behavior, registered routes, formula/read compatibility, and no-reopen rules remain unchanged.
  - No new Buff coupling or coupling-classification evidence was found; `docs/旧Buff系统耦合审查结果.md` remains unchanged.
- Next-step routing:
  - Default next PRD: enemy dynamic / `AnomalyBar.__get_duration_enemy_buffs()` anomaly-duration read matrix bounded characterization or same-PRD Go / No-Go closure.
  - Same-phase pool remains: retained-only sheer blocker package if reader-contract and real route evidence appear, registered behavior sample eligibility only for live semantic diffs, future Stun evidence only when named by regression / oracle / validation evidence, P2-A through P2-G guarded maintenance only on concrete blockers, retained compatibility, and blocker-only phase-1 reopen rules.

## Current Anomaly Duration Runtime-View Matrix PRD US-007 final handoff

- Characterized / retained surfaces:
  - `AnomalyBar.__get_duration_enemy_buffs(...)` reads runtime-view enemy Buffs through `BuffRuntimeReadPort.get_active_buffs("enemy")` when available and retains legacy `dynamic_buff_dict["enemy"]` fallback when no runtime view is supplied.
  - Missing legacy `"enemy"` still raises the existing `TypeError`; `dynamic_buff_dict is None` preserves the empty enemy sequence / base duration behavior.
  - No duration metadata, empty enemy Buff lists, inactive matching Buffs, active unrelated Buffs and unknown duration effect keys preserve base duration.
  - Duration-affecting Buffs cover fixed deltas, percentage deltas, `Buff.dy.count` scaling, multi-Buff order-independent accumulation and negative-effect zero clamp.
- Validation evidence / retained gates:
  - Focused duration matrix ran all 13 `test_anomaly_bar_duration_*` nodeids with `13 passed`.
  - Scoped mypy over `AnomalyBarClass.py`, `buff_runtime.py` and `test_anomaly_handler_runtime_view.py` exited `0` with `Success: no issues found in 3 source files`.
  - Serial `calculator-reads` exited `0` with base simulator `2 passed`, isolated teams `3 passed`, focused slice `260 passed` and mypy clean for `22 source files`.
  - Serial `implicit-events` exited `0` with base simulator `2 passed`, isolated teams `3 passed`, focused slice `259 passed` and mypy clean for `90 source files`.
- Rollback anchors:
  - `AnomalyBar.change_info_cause_active(...)`, `AnomalyBar.__get_max_duration(...)`, `AnomalyBar.__get_duration_enemy_buffs(...)`, `BuffRuntimeReadPort.get_active_buffs("enemy")`, `LegacyBuffRuntimeReadAdapter.get_active_buffs(...)`, the 13 focused duration nodeids, retained `calculator-reads`, retained `implicit-events`, and handoff docs.
- Retained compatibility:
  - No production source, focused test source, formula path, public reader/snapshot API, validation-runner behavior, registered team/APL, old-container path, copied-output behavior, event/runtime/listener layer or retained compatibility path changed in US-007.
  - No new Buff coupling or changed coupling-classification evidence was found, so `docs/旧Buff系统耦合审查结果.md` remains unchanged.
- Next-step routing:
  - Default next PRD: remaining `Calculator.RegularMul` / retained-only sheer blocker package, bounded to `base_attr=4`, `cal_sheer_dmg_bonus(data)`, reader-contract eligibility, deterministic oracle evidence and real registered-route evidence.
  - Same-phase pool remains: registered behavior sample eligibility only for future live semantic diffs, future `Calculator.StunMul.get_stun_array()` follow-up only if named evidence appears, P2-A through P2-G guarded maintenance only on concrete blockers, retained compatibility, and blocker-only phase-1 reopen rules.
