...
| `Calculator.RegularMul.cal_crit_rate()` | `regular-crit-received-boundary`、`test_p2b_parity_fixture_matches_old_full_and_personal_crit_rate_helpers[over-one-full-crit-received-boundary]`、`test_p2b_full_crit_rate_includes_received_bonus_but_personal_excludes()`、`test_calculator_regular_mul_crit_formula_families_preserve_received_boundaries()`。 | US-004 已补 over-100% full crit focused row；`cal_crit_expect()` / crit-balancing caller semantics 仍不在本行授权范围内。 | Snapshot-compatible，必须保留 full crit 包含 `crit_rate_received_increase`。 | Direct damage / crit route 条件成立才运行；样本不得替代 focused oracle。 | `cal_crit_rate()`、P2-B crit reader tests、formula table `regular-crit-received-boundary`。 | Selected candidate proposal packet complete; Conditional Go for one later bounded implementation PRD limited to `cal_crit_rate(data)`. Live sample remains conditional No-Go until real route evidence proves nonzero selected full-crit formula relevance and nonzero `crit_rate_received_increase`. |
