...
| `Calculator.StunMul` | `cal_imp()` 已有 reader seam / family parity 与表驱动 impact reader case；US-013 已把 impact 从 readiness-only 推进为 bounded production proposal-ready，US-007 已给出 scalar impact helper extraction Go。`cal_stun_ratio()`、`cal_stun_res()`、`cal_stun_bonus()`、`cal_stun_received()` 已有 retained / reader-snapshot deterministic oracle；`get_stun_array()` 仍缺 vector assembly oracle。 | 下一步可在 AM/AP/impact bounded implementation 中新增 scalar `_calculate_impact(...)` 并让 `cal_imp()` 委托该 helper；不得把 P2-B impact reader evidence 外推成完整 StunMul production replacement；`get_stun_array()` / array output 和注册队伍行为样本条件仍是后续候选。 |
