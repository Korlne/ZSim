...
| `Calculator.RegularMul.cal_personal_crit_rate()` | `regular-crit-received-boundary`、P2-B full / personal crit parity tests、crit family boundary test；current implementation PRD confirmed helper-seam implementation keeps existing personal/full rows sufficient. | No current deterministic row blocker. Future work should add only story-local rows if a focused regression or named oracle gap appears. | Snapshot-compatible，必须保留 personal crit 排除 received bonus；不得扩展 `_CalculatorReadSnapshot` public contract。 | Conditional No-Go until a future live semantic diff exists and a real registered direct-crit route proves nonzero selected personal-crit-rate relevance. | `cal_personal_crit_rate()`、`_calculate_personal_crit_rate(...)`、reader `read_personal_crit_rate()` parity tests、`formula-parity`、conditional `calculator-reads`。 | Implemented / no-op verified at current handoff; no longer default implementation backlog. |
