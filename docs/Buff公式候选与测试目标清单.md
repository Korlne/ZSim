...
| `Calculator.RegularMul.cal_crit_expect()` | Branch matrix now records expected retained values via `_regular_mul_branch_matrix_values(...)`; source uses current `self.crit_rate` / `self.crit_dmg` at `Calculator.py:1009`. | Dedicated rows for crit-rate clamp, full-vs-personal received-boundary interaction, and over-100% crit expectation are still missing. | Snapshot-compatible only through the already computed `RegularMul` instance state; do not alter public reader snapshot to shortcut `self.crit_rate` / `self.crit_dmg`. | Direct crit route only after future live semantic diff proves the expectation term changes observable damage. | `cal_crit_expect()` source, `cal_crit_rate()` / `cal_crit_dmg()` no-reopen anchors, branch matrix. | Focused `cal_crit_expect` oracle gap, crit-family regression, or formula-parity failure. | No-Go until expectation-specific oracle rows exist; cannot reopen implemented crit-rate / crit-damage helper seams as part of this candidate. |
