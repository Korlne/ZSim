...
| `CalAnomaly` | settled snapshot、shape guard、enemy dynamic lists、`CalAnomaly.cal_active_crit()`、`cal_def_mul()`、res / vulnerability / stun / special multiplier 组合、`CalAnomaly.cal_anomaly_dmg()` retained ratio sample 已有。 | 继续覆盖 `cal_k_level()` clamps、`set_final_multipliers()` vector order、snapshot impact / stun ratio treatment 与 `scaling_factor`。 |
