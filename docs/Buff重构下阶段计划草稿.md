...
Phase-3 same-phase candidate selection / bounded proposal PRD（当前默认）：最新 RegularMul full-crit-damage bounded implementation US-008 已完成 `Calculator.RegularMul.cal_crit_dmg(data)` 的 behavior-preserving helper seam，下一轮不得继续围绕 full crit damage 生成第二个 implementation follow-up，除非 focused regression、validation failure、root-workspace source evidence 或 reviewer-named evidence 明确重开。下一轮应从 same-phase pool 中选择一个 exact bounded surface，先记录 source audit、oracle / gap review、registered-sample eligibility、rollback anchors、retained validation gates、reviewer invariant 和 handoff docs，再决定 Go / No-Go 或 bounded implementation。当前默认候选池继续保留 registered behavior sample eligibility、remaining `Calculator.RegularMul` branches / retained-only sheer follow-up、future `Calculator.StunMul.get_stun_array()` follow-up if named evidence appears、P2-A through P2-G guarded maintenance、retained compatibility 与 blocker-only reopen rules；不得把后续 PRD 生成折叠成 full crit damage 或任何单一 immediate successor。
