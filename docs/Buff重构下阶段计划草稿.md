...
Phase-3 AM/AP/impact bounded production implementation PRD：本轮 proposal 最终 Go，但只允许一个窄实现包，并且下一 PRD 仍应沿 [Buff重构方案.md](./Buff重构方案.md) 的阶段路线推进。默认 scope 是 `Calculator.py` 内的 scalar helper-family：`Calculator.AnomalyMul.cal_am()` 作为已收敛 baseline 保持不变，除非 focused validation 暴露回归；`Calculator.AnomalyMul.cal_ap()` 保留 public signature 与 `@lru_cache(maxsize=16)` 并委托 `_calculate_anomaly_proficiency(...)`；新增 scalar `_calculate_impact(...)` 后让 `Calculator.StunMul.cal_imp()` 委托该 helper。下一 PRD 必须带 focused pytest、scoped mypy targets、rollback anchors、registered behavior sample 条件、retained `formula-parity` / `calculator-reads` gates、触达 event/runtime 时的 `implicit-events` gate，以及明确 non-goals。
...
- Formula snapshots、CalAnomaly internals、old containers、legacy `buff_add()` / `KickOutBuff()` 和 deleted raw queue discovery surfaces 仍是 retained compatibility / phase-3 / blocker-only 项；下一轮默认只进入 AM/AP/impact bounded production proposal，不直接删除 retained compatibility。
- US-013 决定：AM/AP/impact 可以进入 production proposal PRD，但不是直接 implementation。下一轮必须先选择 `cal_am`、`cal_ap`、`cal_imp` 或一个明确 AM/AP/impact helper set，列出 rollback、focused tests、scoped mypy、registered behavior sample 条件、retained `formula-parity` / `calculator-reads` gates 和 non-goals，不能直接替换 `Calculator` / `CalAnomaly` 整体公式。
...
#### 候选块 Phase-3：AM/AP/impact bounded production implementation（当前默认）

- 候选文件 / 符号：`zsim/sim_progress/ScheduledEvent/Calculator.py` 中的 `Calculator.AnomalyMul.cal_am()`, `Calculator.AnomalyMul.cal_ap()`, `Calculator.StunMul.cal_imp()`, `_calculate_anomaly_mastery(...)`, `_calculate_anomaly_proficiency(...)`, proposed scalar `_calculate_impact(...)`, `CalculatorBuffAttributeReader.read_anomaly_mastery(...)`, `read_anomaly_proficiency(...)`, `read_impact(...)`；focused tests 入口为 `tests/simulator/test_buff_attribute_reader.py` 中 `test_cal_am_retained_multiplier_data_oracle_rows`, `test_cal_ap_retained_multiplier_data_oracle_rows`, `test_cal_imp_retained_multiplier_data_oracle_rows`, `test_calculator_am_ap_impact_formula_boundaries_remain_retained_compatibility`。
- 当前耦合：AM/AP/impact 仍由 retained `MultiplierData` / `DynamicStatement` formula snapshot 和 reader-built snapshot 共同证明；reader seam 是 compatibility evidence，不等价于删除 retained formulas。US-003 through US-009 已补齐 retained oracle、reader snapshot parity 和 boundary split；US-010 through US-013 已补齐 validation wiring、registered-sample 条件、serial gates 与 final handoff。
- 当前默认 Ralph-sized 工作方向：生成 bounded production implementation PRD。实现范围只限 scalar helper-family：AM 保持 helper-backed baseline，AP 收敛到 `_calculate_anomaly_proficiency(...)`，impact 新增 scalar `_calculate_impact(...)` 并让 `cal_imp()` 委托该 helper。下一 PRD 第一批 story 必须先用 focused characterization / guardrail 锁定 public signature、`@lru_cache(maxsize=16)`、reader parity、rollback anchors、registered behavior sample 条件、retained validation gates 和 non-goals，再做 production diff。
- 后续同阶段候选块：
  ...
  - `Calculator.StunMul.get_stun_array()` / array output：当前只作为 same-phase formula oracle candidate，不因 `cal_imp()` readiness 自动获得 production replacement 授权。
  - `Calculator.RegularMul` remaining branches：crit expectation、stun vulnerability、special / sheer multiplier 与 array outputs 仍是 oracle candidate，不混入 AM/AP/impact proposal 默认 scope。
...
- 必须保留：`Calculator.AnomalyMul.cal_res_pen()` completed selector extraction 不重开；`MultiplierData` / `MulData` / `DynamicStatement` formula snapshots、`AnomalyBar.current_ndarray`、Calculator / CalAnomaly formulas、copied-output constructors、old containers、legacy `buff_add()` / `KickOutBuff()`、`RuntimeCommandPort`、`LegacyRuntimeCommandAdapter`、`LegacyBuffRuntimeFacade` 继续作为 retained compatibility / rollback anchors。
- 验证入口：AM/AP/impact implementation 必跑 relevant focused pytest targets、`uv run python scripts/run_buff_refactor_validation.py --typecheck-profile formula-parity`、`uv run python scripts/run_buff_refactor_validation.py --typecheck-profile calculator-reads`；触达事件 / runtime / dispatch boundary 时追加 `implicit-events`，触达 lifecycle container / runtime write path 时追加默认 profile。若 implementation 需要验证 runner 改动，先跑 `--help` 和 focused runner tests。
- 行为样本规则：US-013 final handoff 未改 live semantics，因此本轮不跑 main-loop sample；后续只有真实 production semantic diff 且真实注册队伍能触达目标 route 时才跑 `scripts/run_buff_main_loop_consistency.py`，否则记录注册队伍缺口并用 focused parity tests 收口。
- 回滚计划：AM/AP/impact implementation 失败时回退对应 helper / profile / formula diff，继续走 retained `Calculator.AnomalyMul.cal_am()`, `Calculator.AnomalyMul.cal_ap()`, `Calculator.StunMul.cal_imp()`, `MultiplierData`, `DynamicStatement`, and `CalculatorBuffAttributeReader` compatibility evidence；不得用回滚删除 `CalAnomaly.py`、copied-output constructors、old containers 或 runtime compatibility paths。
- 非目标：不在 implementation story 中实现 broad `Calculator.py` / `CalAnomaly.py` rewrite；不删除 old containers、legacy `buff_add()` / `KickOutBuff()`、`RuntimeCommandPort`、`LegacyRuntimeCommandAdapter` 或 `LegacyBuffRuntimeFacade`；不把 CLI label 当 live runtime switch；不把 P2-A / P2-B reader migration guardrails 重开成默认 backlog。
...
- 当前默认 Phase-3 AM/AP/impact bounded production implementation PRD 必跑：新增或触达的 focused pytest targets、`uv run python scripts/run_buff_refactor_validation.py --typecheck-profile formula-parity`，以及 retained gate `uv run python scripts/run_buff_refactor_validation.py --typecheck-profile calculator-reads`；不得默认重开已完成的 `Calculator.AnomalyMul.cal_res_pen()` selector extraction，也不得把 bounded scalar helper diff 扩大为 broad formula rewrite。
...
- US-026 后的 validation decision：`formula-parity`、`calculator-reads` 与 `implicit-events` serial gates 已通过，但当时 production replacement 仍 No-Go。US-013 已把 AM/AP/impact 更新为 bounded production proposal-ready；Current US-007 已把该 proposal 判定为 bounded implementation Go。下一轮必须从本文件的 Phase-3 AM/AP/impact 候选块和 `docs/Buff公式候选与测试目标清单.md` 的 oracle/readiness/proposal 证据取材，命名 exact formula helper scope、focused tests、scoped mypy targets、registered behavior sample 条件、rollback plan、retained `formula-parity` / `calculator-reads` / conditional `implicit-events` gate 与 remaining blockers。
