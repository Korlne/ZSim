...
- 当前总路线已重置；最新权威 PRD 入口以本文 `## 当前默认下一步` 为准，下面保留的 2026-06-07 到 2026-06-15 handoff bullets 是历史 ledger，不得覆盖当前默认入口。
...
Phase-3 下一默认 Ralph PRD 形状：implementation batch 或同一 PRD 内 Go / No-Go closure（当前默认 / anti-selection-loop）。当前 non-sheer `Calculator.RegularMul.cal_base_dmg(data)` / `Calculator.RegularMul.cal_base_attr(base_attr in 0..3, data)` batch 已在本 PRD 完成 implementation、serial validation 与 handoff；下一轮不得再围绕这个已完成微表面生成 full-cycle same-phase candidate-selection / proposal-readiness PRD。

下一轮 PRD generator 应从 `docs/Buff公式候选与测试目标清单.md`、本 checklist、旧耦合审查和当前 focused tests 中选择一个有 current-root source / oracle / retained-gate 证据的同族实现块；如果证据只够 No-Go，则在同一 PRD 内写 blocker / No-Go closure 并转向下一个有证据的 sibling block。不得把 registered behavior sample eligibility、retained-only sheer、future Stun、P2 guarded maintenance、retained compatibility 或 blocker-only reopen pool 提升为 active default，除非 focused regression、validation failure、root-workspace source evidence、guardrail 或 reviewer-named evidence 明确命名具体 blocker。
...
- 当前默认 PRD 改为 Phase-3 `Calculator.RegularMul` non-sheer base damage / base-attribute bounded implementation batch：最新 RegularMul scalar helper-family bounded implementation 已完成并验证，下一轮不得再生成 generic same-phase candidate selection / bounded proposal。PRD 应复用当前 retained evidence 与 focused oracle gap 结果，围绕 `cal_base_dmg(data)`、`cal_base_attr(base_attr in 0..3, data)` 组织 implementation / focused-test stories；若某个 non-sheer branch 缺少 oracle 或触发 public-contract 风险，应在同一 PRD 内 No-Go / skip 该 branch，并继续处理满足同类条件的 sibling surfaces。不得默认重开已完成的 copied-payload handler/report implementation、`Calculator.AnomalyMul.cal_res_pen()` selector extraction、AM/AP/impact helper implementation、selected Stun implementation、current `Calculator.RegularMul.cal_crit_rate(data)` / `cal_personal_crit_rate(data)` / `cal_personal_crit_dmg(data)` / `cal_crit_dmg(data)` implementations、current RegularMul scalar helper-family batch、retained-only sheer No-Go 或 P2 guarded buckets，也不得把下一轮扩大为 broad formula rewrite。
