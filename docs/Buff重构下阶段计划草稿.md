...
Phase-3 selected Stun array output bounded proposal / implementation PRD（当前默认）：registered sample / formula candidate-selection PRD 已完成，唯一 selected surface 是 `Calculator.StunMul.get_stun_array()` / `Calculator.cal_stun()` array output。下一轮默认只生成一个围绕该 surface 的 bounded proposal / implementation PRD：候选文件 / 符号限定在 `zsim/sim_progress/ScheduledEvent/Calculator.py` 的 `get_stun_array()` 与 `cal_stun()`、`tests/simulator/test_buff_attribute_reader.py::test_stun_array_output_contract_preserves_field_order_dtype_and_product` 和必要 docs / Ralph evidence；必须保留 focused pytest、`formula-parity`、`calculator-reads`、conditional `implicit-events`、registered sample 条件、rollback anchors、stop conditions、retained gates、reviewer questions 和 non-goals。若未来 diff 触达 live semantic behavior，只有真实 registered stun / impact route 且 explicit stop tick 内有 nonzero relevant counts 时才运行 main-loop consistency；不得创建 validation-only team。未来 PRD generation 可以产出 Markdown PRD，但不得在同一步直接改 `scripts/ralph/prd.json`；Ralph JSON conversion 留给后续显式转换步骤。不得扩大为 broad `Calculator.py` / `CalAnomaly.py` rewrite、validation-runner rewrite、registered-team fixture creation、old-container deletion、listener / scheduled publish / dot runtime / same-tick runtime write layer merge、RegularMul branch bundle、retained-only sheer shortcut 或 retained compatibility 删除。
...
- Current candidate-selection PRD 已选择 `Calculator.StunMul.get_stun_array()` / `Calculator.cal_stun()` array output 作为唯一 next bounded surface，并给出 Go for one later bounded proposal / implementation PRD only。该选择只确定下一默认 PRD，不删除 same-phase candidate pool；registered behavior sample eligibility、remaining `Calculator.RegularMul` branches / retained-only sheer follow-up、Stun array future follow-up、P2-A through P2-G guarded maintenance、retained compatibility 和 blocker-only reopen rules 仍保留为后续候选。
