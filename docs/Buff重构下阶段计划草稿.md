...
Phase-3 copied-output handler/report bounded implementation PRD（conditional Go）：当前 bounded proposal PRD 已给出 one later bounded implementation PRD 的 conditional Go，但下一 PRD 仍必须先选择 exact coherent copied-output handler/report slice，不得把 Go 当成 immediate broad production authorization。默认 scope 只能从 `zsim/sim_progress/anomaly_bar/CopyAnomalyForOutput.py` copied payload constructors、`zsim/sim_progress/Update/UpdateAnomaly.py::spawn_output(...)` mode 0 / 1 / 2 creation and missing-`polarity_ratio` failure path、以及 anomaly / disorder / polarity / abloom handler report payload paths 中取材；`UpdateAnomaly.update_anomaly(...)` scheduled publish order 只有在 proposal 明确证明 order parity 时才可触达。PRD 生成器必须命名 exact touched files / symbols、focused pytest、scoped mypy targets、registered behavior sample 条件、rollback anchors、stop conditions、retained `formula-parity` / `calculator-reads` gates 和 copied-output/event-runtime `implicit-events` gate。same-phase pool 必须继续保留 registered behavior sample eligibility、`Calculator.RegularMul` remaining branches / retained-only sheer follow-up、`Calculator.StunMul.get_stun_array()` / array output follow-up、P2-A through P2-G guarded maintenance、retained compatibility 与 blocker-only reopen rules；不得扩大为 broad `Calculator.py` / `CalAnomaly.py` rewrite、validation-runner rewrite、registered-team fixture creation、old-container deletion、layer merge 或 retained compatibility 删除。
...
- Copied-output constructor / listener / handler-report parity 已完成 characterization、proposal-readiness decision 和 bounded proposal package；当前剩余默认工作是 conditional later implementation PRD，但只能做 one coherent copied-output handler/report slice。该 implementation PRD 完成后，same-phase pool 仍应继续保留 registered behavior sample eligibility、RegularMul / retained-only sheer、Stun array、P2 guarded maintenance 和 retained compatibility 候选，不得只沿 copied-output 生成单一路径。
...
#### 候选块 Phase-3：copied-output handler/report bounded implementation PRD（当前默认 / conditional Go）
...
- 当前已知耦合：constructor field matrix、listener broadcast vs scheduled publish separation、handler report payload parity、registered-sample eligibility、rollback anchors、stop conditions 和 reviewer questions 已由 bounded proposal PRD 覆盖；当前结论是 conditional Go with named blockers for one later bounded copied-output handler/report implementation PRD。copied-output payload、listener broadcast、scheduled queue publish、dot runtime registration 与 same-tick runtime writes 仍是分层 retained boundaries。
- Ralph-sized 工作方向：生成 implementation PRD 时只选择 one coherent copied-output handler/report slice，并在 PRD 中列出 exact production diff candidates、focused tests、scoped mypy targets、serial retained gates、registered-route sample conditions、rollback anchors、stop conditions and reviewer questions；如果 exact slice 需要跨入 `Calculator.py` / `CalAnomaly.py`、validation-runner、registered-team fixture、old-container deletion、dispatch/runtime port implementation 或 layer merge，则停止并拆新 PRD。
...
- 同阶段候选池：registered behavior sample eligibility、`Calculator.RegularMul` remaining branches / retained-only sheer follow-up、`Calculator.StunMul.get_stun_array()` / array output follow-up、P2-A through P2-G guarded maintenance、retained compatibility、blocker-only phase-1 reopen rules。若 copied-output implementation 完成，下一轮应从这些候选中重新选择 exact bounded slice，而不是继续自动生成 copied-output follow-up。
- 验证入口：implementation PRD 至少跑 focused copied-output pytest、scoped mypy、JSON sanity、UTF-8 / mojibake scan，并串行保留 `formula-parity`、`calculator-reads` 与 `implicit-events`；live semantic diff 只有在 real registered route 且 nonzero relevant event count 时才追加 main-loop consistency sample。
...
- 当前默认 Phase-3 copied-output handler/report bounded implementation PRD 必跑：focused copied-output pytest、focused typecheck / scoped mypy target、JSON sanity、UTF-8 / mojibake scan，以及串行 `formula-parity`、`calculator-reads`、`implicit-events` retained gates。不得默认重开已完成的 `Calculator.AnomalyMul.cal_res_pen()` selector extraction、AM/AP/impact helper implementation、Stun / RegularMul array contract、selected RegularMul branch matrix 或 retained-only sheer No-Go，也不得把 implementation diff 扩大为 broad formula rewrite。
