...
- 当前已知耦合：`get_stun_array()` 仍由 `cal_stun()` 使用，CodeGraph 导航显示它不应因 `cal_imp()` helper extraction 自动获得 production replacement 授权；`RegularMul` branch matrix 已锁定 direct damage、crit、defense/resistance、damage vulnerability、stun vulnerability、special multiplier 和 sheer damage bonus retained values，但 sheer 仍是 runtime-only / retained-only follow-up。
- 本轮结果：US-002 至 US-004 已补 Stun array、RegularMul arrays 和 selected RegularMul branch characterization；US-005 判定 production proposal No-Go；US-006 串行验证通过；US-007 handoff 只同步 docs / Ralph evidence，不改 production / validation-runner / event-runtime source。
...
- 当前已知耦合：current array / RegularMul evidence is characterization-only；production proposal 缺 registered-route behavior sample 条件、remaining branch / copied-output gap closure 和 exact rollback plan；P2-A through P2-G 是 guarded maintenance，不是默认实现 backlog。
