...
Phase-3 `Calculator.RegularMul` remaining branch bounded proposal-readiness PRD（当前默认）：selected Stun array implementation 已完成，状态为 implemented / no-op verified at handoff；`Calculator.StunMul.get_stun_array()` 已委托 `_build_stun_multiplier_array(...)`，`Calculator.cal_stun()` 仍是既有 product consumer，五字段顺序与 `np.float64` dtype 保持不变。下一轮默认不再生成 Stun implementation follow-up，而是刷新剩余 `Calculator.RegularMul` branch matrix，选择一个 exact bounded candidate 或记录 No-Go，并为该 candidate 命名 focused pytest、scoped mypy、rollback anchors、registered-sample 条件、retained gates、reviewer questions 和 non-goals。copied-output handler/report implementation、`Calculator.AnomalyMul.cal_res_pen()` selector extraction、AM/AP/impact helper implementation、selected Stun implementation、Stun / RegularMul characterization、retained-only sheer No-Go 与 P2-A through P2-G guarded buckets 都是 completed evidence / retained pool，不是默认 implementation backlog；只有 focused regression、guardrail failure、validation failure 或 root-workspace source evidence 命名具体文件 / 符号 / gate 时才可重开。未来 PRD generation 可以产出 Markdown PRD，但不得在同一步直接改 `scripts/ralph/prd.json`；Ralph JSON conversion 留给后续显式转换步骤。不得扩大为 broad `Calculator.py` / `CalAnomaly.py` rewrite、validation-runner rewrite、registered-team fixture creation、old-container deletion、listener / scheduled publish / dot runtime / same-tick runtime write layer merge、retained-only sheer shortcut 或 retained compatibility 删除。
...
- Current Stun array bounded implementation PRD 已完成且 handoff 为 implemented / no-op verified；`Calculator.StunMul.get_stun_array()` future follow-up 只在新证据命名 focused oracle gap、validation failure、root-workspace source regression 或 proposal-readiness packet 时重开。当前默认 PRD 改为 `Calculator.RegularMul` remaining branch bounded proposal-readiness；registered behavior sample eligibility、remaining `Calculator.RegularMul` branches / retained-only sheer follow-up、future Stun follow-up with named evidence、P2-A through P2-G guarded maintenance、retained compatibility 和 blocker-only reopen rules 仍保留为后续候选。
...
#### 候选块 Phase-3：array outputs / remaining formula oracle closure（Stun implementation complete / remaining candidates retained）
...
- 本轮结果：US-002 至 US-004 已补 Stun array、RegularMul arrays 和 selected RegularMul branch characterization；array / RegularMul US-005 判定 production proposal No-Go；sheer reader-snapshot readiness US-003 / US-005 也判定 production proposal No-Go；后续 bounded implementation PRD 已把 `Calculator.StunMul.get_stun_array()` 的五字段数组构造收敛到 `_build_stun_multiplier_array(...)`，并保持 `Calculator.cal_stun()` 的 `np.prod(...)` consumer 不变。
- 当前 implementation handoff：selected Stun array implementation 状态为 implemented / no-op verified at handoff。US-003 已完成生产 helper extraction；US-008 只同步 handoff docs / Ralph evidence，不再制造额外 source churn。已记录 verifier evidence：focused reader pytest `141 passed`、`formula-parity` focused reader `141 passed` / mypy `9 source files` clean、`calculator-reads` focused reader `241 passed` / mypy `22 source files` clean。
- 后续工作方向：不要把 Stun helper implementation 自动扩展为 broad production replacement。若后续仍选择 formula surface，必须命名一个剩余 exact branch 或 follow-up gap，补 deterministic oracle、rollback anchors 和 registered behavior sample 条件后，再重新评估 proposal readiness。
...
- 同阶段候选池：
  ...
  - remaining `Calculator.RegularMul` branches / retained-only sheer follow-up：继续只按 exact branch、deterministic oracle、rollback anchors 和 registered-sample 条件重开；不得把 Stun helper implementation 当成 RegularMul 打包授权。
  - future `Calculator.StunMul.get_stun_array()` follow-up：当前 helper extraction 已完成；只有 named regression、focused oracle gap 或新的 proposal-readiness packet 才能重开。
...
- 当前默认 Phase-3 same-phase candidate selection / bounded proposal PRD 必跑：targeted source/docs preflight、focused docs / JSON / UTF-8 checks、active story scoped typecheck gate，并在选择 exact bounded slice 后声明对应 retained gate。不得默认重开已完成的 copied-payload handler/report implementation、`Calculator.AnomalyMul.cal_res_pen()` selector extraction、AM/AP/impact helper implementation、Stun / RegularMul array contract、selected RegularMul branch matrix 或 retained-only sheer No-Go，也不得把下一轮直接扩大为 broad formula rewrite。
