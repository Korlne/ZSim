...
- 2026-06-18 enemy-state helper handoff 已更新：`read_enemy_anomaly_active(...)`、`read_enemy_shock_active(...)` / `read_enemy_stun_active(...)`、`read_enemy_frozen_edge_state(...)` / `read_enemy_stun_edge_state(...)` / `read_enemy_frost_frostbite_edge_state(...)` 均已作为 BuffXLogic-local exact helper subsets 落地或完成 handoff。Anomaly helper `implicit-events` serial gate 通过 focused `284 passed` / mypy `93 source files` clean；shock/stun helper serial gate 通过 focused `285 passed` / mypy `93 source files` clean；edge-state read-family serial gate 通过 focused bundle `40 passed`、scoped mypy `11 source files` clean、`implicit-events` focused `311 passed` / mypy `93 source files` clean。所有 completed helper subsets 均为 implemented / no-reopen；当前默认下一 PRD 改为 `BuffXLogic Remaining Enemy-State Copied-Output And Dot/Debuff Runtime-State Exact Intake`。必须保留下方 same-phase candidate pool，不得把文档折叠成 anomaly / shock / stun / edge-state helper follow-up。
...
Phase 1-4 当前默认路由：BuffXLogic remaining enemy-state copied-output / formula parity and dot/debuff runtime-state route review / exact-intake PRD, while preserving the broader same-phase candidate pool.

- 决策基线：Phase 5 暂停；当前已把 `read_enemy_anomaly_active(...)`、`read_enemy_shock_active(...)` / `read_enemy_stun_active(...)`、`read_enemy_frozen_edge_state(...)` / `read_enemy_stun_edge_state(...)` / `read_enemy_frost_frostbite_edge_state(...)` marked implemented / no-reopen for their exact approved file sets. These helpers remain BuffXLogic-local and narrow: current-state reads can move behind helpers, but caller-owned prerequisites, previous-state fields, edge comparisons, branch order, record/template identity, report-state writes, `JudgeTools.detect_edge(...)`, `simple_start(...)`, anomaly-bar identity and record mutation stay in owning files. Previous selected impact / crit / stun owner-family evidence is completed / guarded-maintenance and is not the current default route.
- 当前默认下一 PRD：generate `BuffXLogic Remaining Enemy-State Copied-Output And Dot/Debuff Runtime-State Exact Intake`. The PRD should start with compact current-root preflight and then choose one coherent same-risk subfamily instead of stopping at one safe callsite. It must name exact files / symbols / behavior, focused verifier, rollback anchors, retained boundaries and non-goals, then produce one of:
  - bounded production helper / reader-adapter delegation plus focused characterization and guardrail updates for copied-output / formula-parity-adjacent reads or dot/debuff runtime-state reads;
  - a blocker-only No-Go packet with retained boundary and rollback evidence, only after no implementable sibling in the chosen subfamily can be named;
  - a stop-for-human route review if no exact implementation candidate can be named across the current remaining enemy-state read-family pool.
- Completed / no-reopen helper subset:
  - Helper: `read_enemy_anomaly_active(...)` in `zsim/sim_progress/Buff/BuffXLogic/enemy_anomaly_read.py`; implemented users are `ElectroLipGlossAtkAndDmgBonus`, `JaneAdditionalAbilityPhyBuildupBonus`, `MarcatoDesireAtkBonus`, and `TimeweaverApBonus`.
  - Helpers: `read_enemy_shock_active(...)` / `read_enemy_stun_active(...)` in `zsim/sim_progress/Buff/BuffXLogic/enemy_state_read.py`; implemented users are `LinaAdditionalSkillEleDMGBonus` for shock and `Soldier11AdditionalSkillExtraFireDMGBonus`, `YixuanAdditionalAbilityDmgBonus`, `YixuanCinema2StunTimeLimitBonus`, and `YuzuhaCinema2Trigger` for stun.
  ...
  - Rollback anchors: helper modules, delegated BuffXLogic files, `tests/simulator/test_enemy_anomaly_read_helper.py`, `tests/simulator/test_enemy_state_read_helper.py`, `tests/simulator/test_enemy_edge_state_read_helper.py`, `tests/simulator/test_simple_anomaly_gate_characterization.py`, `tests/simulator/test_simple_enemy_state_gate_characterization.py`, `tests/simulator/test_enemy_context_direct_helpers.py`, `tests/simulator/test_yuzuha_direct_context_helpers.py`, `tests/simulator/test_freeze_stun_edge_detection_characterization.py`, `tests/simulator/test_enemy_dynamic_read_guardrail.py`, focused replacement notes and this next-stage draft.
- Candidate block A - copied-output / formula parity adjacent reads:
  - Candidate files / symbols: `HugoCorePassiveTotalizeTrigger.py`, `VivianCorePassiveTrigger.py`, `VivianCinema6Trigger.py`, `YanagiPolarityDisorderTrigger.py`, copied-output constructors, `UpdateAnomaly.spawn_output(...)`, `CalAnomaly`, `Calculator.py`, `MultiplierData`, `MulData`, `DynamicStatement`, `_CalculatorReadSnapshot`, and `CalculatorBuffAttributeReader`.
  - Known coupling: these paths can mix enemy dynamic reads with copied anomaly / disorder payload construction, report payload parity, anomaly formula snapshots, AP / AM / impact / stun formula state and validation-runner profiles.
  - Ralph-sized work directions: characterize exact current dynamic read and payload/formula dependency first; delegate only a read predicate or reader-backed snapshot when parity evidence proves no formula / copied-output semantic change; otherwise emit No-Go with rollback anchors.
  - Retained boundaries: copied-output payload fields, retained public formula methods, `MultiplierData` / `MulData` / `DynamicStatement`, old containers and event/runtime/listener layers remain separate.
...
- Candidate block B - dot/debuff runtime-state and anomaly-map reads:
  ...
  - Ralph-sized work directions: characterize read-only versus runtime mutation first; only delegate a pure read when the test proves no dot registration, debuff mutation, scheduled publish, listener broadcast or same-tick write is moved; otherwise emit blocker / No-Go.
