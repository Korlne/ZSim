...
Phase-3 array-output / remaining formula oracle closure PRD：AM/AP/impact bounded implementation 已完成，下一默认 PRD 仍应沿 [Buff重构方案.md](./Buff重构方案.md) 的阶段路线推进，但不重开已完成的 AM/AP/impact helper-family。默认候选是 `Calculator.StunMul.get_stun_array()` / array output oracle work 与 `Calculator.RegularMul` remaining branches 的 bounded characterization / proposal-readiness package；PRD 生成器应先选择 exact formula surface、focused pytest、scoped mypy targets、rollback anchors、registered behavior sample 条件、retained `formula-parity` / `calculator-reads` gates 和 non-goals。same-phase pool 必须继续保留 copied-output handler/report payload parity、registered-team behavior sample eligibility，以及 P2-A through P2-G guarded maintenance；触达 event/runtime 时追加 `implicit-events`，不得扩大为 broad `Calculator.py` / `CalAnomaly.py` rewrite 或 retained compatibility 删除。
...
- AM/AP/impact bounded implementation 已完成：后续不再把 `cal_am()`、`cal_ap()`、`cal_imp()` helper-family 当默认 backlog。remaining same-phase pool 应从 `Calculator.StunMul.get_stun_array()` / array outputs、`Calculator.RegularMul` remaining branches、copied-output handler/report payload parity、registered-team behavior sample eligibility 与 P2-A through P2-G guarded maintenance 中选择 exact bounded slice。
...
#### 候选块 Phase-3：array outputs / remaining formula oracle closure（当前默认）
...
- 当前已知耦合：`get_stun_array()` 仍由 `cal_stun()` 使用，CodeGraph 导航显示它不应因 `cal_imp()` helper extraction 自动获得 production replacement 授权；`RegularMul` remaining branches 仍混合 direct damage / crit / vulnerability / special multiplier 公式与 retained snapshot 语义。
- Ralph-sized 工作方向：先做 deterministic oracle / boundary characterization，命名 exact formula branch、focused pytest、scoped mypy、rollback anchors、registered behavior sample 条件和 retained validation gates；只有 oracle 和 rollback evidence 充足后，后续 PRD 才能讨论 bounded production proposal。
...
- 当前默认 Phase-3 array-output / remaining formula oracle closure PRD 必跑：新增或触达的 focused pytest targets、`uv run python scripts/run_buff_refactor_validation.py --typecheck-profile formula-parity`，以及 retained gate `uv run python scripts/run_buff_refactor_validation.py --typecheck-profile calculator-reads`；不得默认重开已完成的 `Calculator.AnomalyMul.cal_res_pen()` selector extraction 或 AM/AP/impact helper implementation，也不得把 oracle / characterization diff 扩大为 broad formula rewrite。
