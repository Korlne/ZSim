...
Phase-3 same-phase candidate selection / bounded proposal PRD（当前默认）：copied-payload handler/report bounded implementation PRD 已完成，不再自动生成第二个 copied-output follow-up。下一轮默认只做一个 bounded candidate-selection / proposal-readiness PRD：从 registered behavior sample eligibility、remaining `Calculator.RegularMul` branches / retained-only sheer follow-up、`Calculator.StunMul.get_stun_array()` / array-output follow-up、P2-A through P2-G guarded maintenance、retained compatibility 或 blocker-only reopen rules 中选择一个 exact bounded slice，并写清 candidate files / symbols、known coupling、focused pytest、scoped mypy targets、registered sample 条件、rollback anchors、stop conditions、retained gates 和 non-goals。未来 PRD generation 可以产出 Markdown PRD，但不得在同一步直接改 `scripts/ralph/prd.json`；Ralph JSON conversion 留给后续显式转换步骤。不得扩大为 broad `Calculator.py` / `CalAnomaly.py` rewrite、validation-runner rewrite、registered-team fixture creation、old-container deletion、listener / scheduled publish / dot runtime / same-tick runtime write layer merge 或 retained compatibility 删除。
