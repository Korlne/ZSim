...
Phase-3 RegularMul personal crit rate bounded implementation PRD（当前默认）：当前 personal crit rate proposal-readiness PRD 已完成 handoff，结论为 Conditional Go for one later bounded implementation PRD only。下一轮默认只能围绕 `Calculator.RegularMul.cal_personal_crit_rate(data)` 与可选 module-local `_calculate_personal_crit_rate(static_statement, dynamic_statement)` helper，保持 `static.crit_rate + dynamic.crit_rate + dynamic.field_crit_rate`，继续排除 `crit_rate_received_increase`，并写清 candidate files / symbols、focused personal/full crit tests、scoped mypy targets、registered-sample 条件、rollback anchors、retained `formula-parity` / conditional `calculator-reads` / conditional `implicit-events` gate、non-goals 和 stop conditions。不得默认重开已完成的 copied-payload handler/report implementation、`Calculator.AnomalyMul.cal_res_pen()` selector extraction、AM/AP/impact helper implementation、selected Stun implementation、current `Calculator.RegularMul.cal_crit_rate(data)` implementation、current `Calculator.RegularMul.cal_personal_crit_dmg(data)` implementation、retained-only sheer No-Go 或 P2 guarded buckets，也不得把下一轮直接扩大为 broad formula rewrite。该默认完成或被证据阻断后，PRD 生成仍必须回到 same-phase pool，而不是继续串联 personal crit rate follow-up；same-phase pool 必须继续保留 registered behavior sample eligibility、remaining `Calculator.RegularMul` branches / retained-only sheer follow-up、future `Calculator.StunMul.get_stun_array()` follow-up if named evidence appears、P2-A through P2-G guarded maintenance、retained compatibility 与 blocker-only reopen rules。
...
#### 候选块 Phase-3：RegularMul personal crit rate bounded implementation PRD（conditional Go / same-phase pool retained）
...
- 本轮结果：proposal-readiness PRD 已关闭 source / reader audit、deterministic oracle review、proposal boundary、registered-sample conditional No-Go、retained validation gate 与 reviewer Go / No-Go；结果为 Conditional Go for one later bounded implementation PRD only。
- 当前默认下一 PRD：Phase-3 `Calculator.RegularMul.cal_personal_crit_rate(data)` bounded implementation PRD。该 PRD 只允许等价 helper seam 或 explicit No-Go；完成后不得自动生成第二个 personal-crit-rate follow-up，必须重新从 retained same-phase pool 选择。
- 当前 verdict：Conditional Go。Registered main-loop sample 仍是 conditional No-Go，除非 future live semantic diff 且真实 registered direct crit route 在 explicit stop tick 内证明 nonzero selected personal-crit-rate relevance；不得创建 validation-only team、fake APL、fixture-only route 或 retained-vs-retained sample。
...
- 验证入口：future implementation 至少运行 focused personal/full crit pytest、scoped mypy over `Calculator.py` and focused tests、serial `formula-parity`；`calculator-reads` 只在 reader seam / snapshot / reader guardrail 变更时追加；`implicit-events` 只在 copied-output、event、dispatch、runtime、listener、dot runtime、same-tick write、lifecycle、validation-runner 或 old-container surfaces 触达时追加；live semantic diff 且 route/count 合格时才追加 main-loop consistency。
