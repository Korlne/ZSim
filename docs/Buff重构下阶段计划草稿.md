...
- 当前有效阶段是 Phase 3 implementation：阶段 1 基础设施解耦已由 `PRD-12 US-024` 关闭，阶段 2 全量分类、P2-A AM/AP reader + computed count state-sync、P2-B crit / impact reader、P2-C trigger-state read-only gates、P2-D scheduled publish ordering / adapter parity、P2-E dot runtime-state / initialization、P2-F BuffAddStrategy caller / facade-write design 与 P2-G direct simulator context helpers 均已完成 guarded scope；下一轮不再回到 phase-2 closure 或 same-phase candidate-selection 循环，除非 focused regression、validation failure、root-workspace source evidence、guardrail 或 reviewer-named evidence 明确命名 blocker。
...
Phase-4 下一默认 Ralph PRD 形状：旧 Buff 残余删除 readiness / phase-boundary intake（当前默认 / no-immediate-deletion）。当前 candidate-pool closure / exact-intake PRD 已记录 Phase-3 closure / No-Go：没有 current-root source、focused regression、guardrail、validation failure 或 reviewer-named evidence 支撑新的 exact bounded production candidate。下一 PRD 应先盘点阶段 4 的删除候选、证明 retained compatibility 是否仍被 live route / focused tests / validation profiles 使用，并在同 PRD 内输出 one deletion-readiness Go / No-Go packet；不得直接删除旧容器、旧 adapter、`MultiplierData` / `DynamicStatement` / `MulData`、legacy `buff_add()` / `KickOutBuff()`、public reader/snapshot contract 或 validation-runner wiring。

下一轮 PRD generator 应从 `docs/Buff系统重构Checklist.md` 的阶段 4 项、`docs/旧Buff系统耦合审查结果.md` 的 retained coupling、本文 retained-boundary ledger、`docs/Buff公式候选与测试目标清单.md` 的 validation entrypoints 和当前 focused tests 中生成 deletion-readiness intake；它不是新的 Phase-3 same-phase selection loop，也不是某个已完成 formula branch 的 follow-up。Phase-3 same-phase pool 仍保留为 reopen evidence：retained-only sheer helper-complete / blocked、completed / no-reopen RegularMul / CalAnomaly / Stun / AM/AP/impact / copied-output / anomaly-bar readiness、P2 guarded maintenance、registered behavior sample eligibility、retained compatibility 和 blocker-only reopen rules。只有 future focused regression、validation failure、current-root source evidence、guardrail 或 reviewer-named evidence 明确命名具体 blocker 时，才从这个 retained pool 中开窄修复 PRD。

### Current candidate-pool closure / exact-intake PRD final handoff

- Phase-3 closure / No-Go evidence:
  - `scripts/ralph/investigations/2026-06-16-US-005-phase-3-closure-no-go.md` records that no exact bounded production candidate is evidenced by current-root source, tests, prior packets and Buff docs.
  - `US-006` records docs-only serial validation / reviewer gate evidence: no production source, focused test source, formula/readiness source, event/runtime/listener/dot runtime surface, validation wiring, same-tick runtime write, lifecycle container, runtime write path or validation-runner behavior changed.
  - `US-007` final handoff keeps docs / Ralph evidence / completion bookkeeping only; no new Buff coupling or changed coupling classification was discovered, so [旧Buff系统耦合审查结果.md](./旧Buff系统耦合审查结果.md) remains unchanged.
- Current default next PRD:
  - Phase-4 deletion-readiness / phase-boundary intake, scoped to evidence collection, retained-reference census, focused guardrail checks, rollback anchors, verifier commands and one Go / No-Go decision. If deletion is not proven unused and behavior-preserving, the PRD must stop with No-Go instead of manufacturing a cleanup diff.
- Same-phase retained pool for future generation:
  - Retained-only sheer: `Calculator.RegularMul.cal_base_attr(..., base_attr=4)` / `cal_sheer_dmg_bonus(data)` are helper-complete but still blocked by public reader/snapshot and real registered-route conditions. Validation entrypoints remain focused `test_buff_attribute_reader.py`, `formula-parity`, `calculator-reads`, and conditional `implicit-events`.
  - Completed / no-reopen formula and output surfaces: RegularMul helpers, `Calculator.AnomalyMul.cal_res_pen()`, AM/AP/impact, selected Stun, CalAnomaly helper family, copied-output handler/report, `AnomalyBar.current_ndarray`, `UpdateAnomaly.py` write paths and anomaly-duration runtime-view matrix. Reopen only with current-root evidence naming file, symbol, behavior and rollback boundary.
  - Event/runtime guarded maintenance: P2-A through P2-G, `ScheduleDispatchPort`, `RuntimeCommandPort`, `LegacyRuntimeCommandAdapter`, `LegacyBuffRuntimeFacade` and `BuffRuntimeReadPort` remain guarded-maintenance only; run `implicit-events` when a future slice touches dispatch/runtime/listener/dot/same-tick write behavior.
  - Registered behavior sample eligibility: allowed only after a live production semantic diff, real registered route, explicit stop tick, nonzero relevant formula/event counts, total damage comparison, full event counts, Buff timeline comparison and rollback anchors exist; no validation-only team, fake APL, fixture-only route or retained-vs-retained sample.
- Preserved non-goals:
