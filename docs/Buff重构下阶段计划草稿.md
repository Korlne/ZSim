...
- 当前有效阶段经 2026-06-18 Phase 1-4 comprehensive replacement gap-closure PRD 收敛为 docs/control-plane route closure：US-001 至 US-008 已把 `docs/Buff系统重构Checklist.md` 的 `Phase 1-4 全面替换差异排查` 八项路由为 bounded implementation candidate、deletion-readiness candidate、long-term compatibility strategy、No-Go / blocker 或 Phase 5 pause condition；US-009 负责最终 handoff。
- 2026-06-18 checklist wording fix：`docs/Buff系统重构Checklist.md` 的 Phase 3 标题已澄清为“XLogic 切片替换与保留边界收口”。Phase 3 顶层 `[x]` 只表示已命名切片已有 implementation / characterization / No-Go / retained-boundary 结论，不表示 `BuffXLogic` 全量新架构迁移完成，也不授权删除 `JudgeTools`、`check_preparation(...)`、old containers 或 retained compatibility。
- 2026-06-18 Phase 1-4 comprehensive checklist audit 已完成：`docs/Buff系统重构Checklist.md` 现在明确 Phase 1 是基础设施隔离边界而非旧容器删除，Phase 2 是分析 / 分类而非 live migration，Phase 3 是切片替换与保留边界收口而非全面新架构迁移，Phase 4 是删除就绪审查与 retained-boundary No-Go 而非旧残余删除完成。`Phase 1-4 全面替换差异排查` 每项已写入最终 route status，但这些 `[x]` 表示路线决策完成，不表示旧 Buff 全面替换或删除完成。
- 2026-06-18 impact / crit / stun owner-family state-sync/read-gate PRD 已完成：selected family `LighterAdditionalAbility_IceFireBonus.py`、`QingYiAdditionalAbilityStunConvertToATK.py`、`TriggerAdditionalAbilityStunBonus.py`、`Soldier0AnbyCoreSkillCritDMGBonus.py` 已有 source preflight、focused characterization、state-sync/read-gate guardrails、serial `calculator-reads` / `implicit-events` verifier evidence 和 final handoff；current-root source did not require production edits. 该条现在是 completed / no-reopen evidence，不得继续沿这四个 selected files 生成 narrow follow-up，也不得恢复 Phase 5 validation PRD。
- 2026-06-18 enemy-state helper handoff 已更新：`read_enemy_anomaly_active(...)`、`read_enemy_shock_active(...)` / `read_enemy_stun_active(...)`、`read_enemy_frozen_edge_state(...)` / `read_enemy_stun_edge_state(...)` / `read_enemy_frost_frostbite_edge_state(...)` 均已作为 BuffXLogic-local exact helper subsets 落地或完成 handoff。Copied-output exact-intake PRD 也已完成：`HugoCorePassiveTotalizeTrigger.py` 的 copied-output stun predicates 已委托 `read_enemy_stun_active(...)`，`VivianCorePassiveTrigger.py` / `VivianCinema6Trigger.py` / `YanagiPolarityDisorderTrigger.py` 的 judge-time anomaly-active predicates 已委托 `read_enemy_anomaly_active(...)`；copied-output helper outcome 为 implemented / no-reopen。Dot/debuff runtime-state and anomaly-map reads remain retained future pool；当前默认下一 PRD 改为 `BuffXLogic Dot/Debuff Runtime-State And Anomaly-Map Exact Intake`。必须保留下方 same-phase candidate pool，不得把文档折叠成 anomaly / shock / stun / edge-state / copied-output helper follow-up。
- 2026-06-18 convergence cadence：当前路线允许继续按 bounded PRD 逐步推进，但每完成 2-3 个 implementation / Go-No-Go PRD，或任一 PRD 把整组候选降级为 No-Go / blocker 后，必须生成一次 route review / convergence review story 或 PRD。该 review 不得重复普通 candidate selection；它只回答 retained / blocker 是否减少、Phase 5 resume 条件是否更接近、哪些旧兼容面进入长期兼容、哪些仍需 exact implementation / deletion-readiness / human phase-boundary review。
- 最终 Buff 重构完成判据需要逐步显式化：后续 handoff 必须维护一份可收敛的 completion criteria 状态，包括 old containers / lifecycle compatibility、`BuffXLogic` owner-family replacement、`JudgeTools` service-location、event/runtime/listener layer separation、formula / effect snapshots、old residual deletion readiness、registered behavior / performance gates 和 Phase 5 resume 条件。不得只记录 retained / No-Go / blocker 而不分类其后续命运。
- Phase 5 pause/resume condition：Phase 5 一致性与性能收口继续暂停。只有全部 Phase 1-4 gap 已关闭、降级为长期兼容策略，或形成 explicit blocker with human-approved route，且 handoff 明确允许恢复时，才可把 Phase 5 作为默认 PRD；当前 US-008 / US-009 结论不满足恢复条件。
- 2026-06-18 comprehensive gap-closure PRD 未发现新 Buff coupling 或既有 coupling classification 变化；[旧Buff系统耦合审查结果.md](./旧Buff系统耦合审查结果.md) 保持不变。除非后续证据命名 exact candidate、verifier 和 rollback anchor，否则不得把 checklist 顶层状态、retained pool 或历史 handoff 句子推导为生产 diff、删除 diff、public reader expansion、validation-runner rewrite 或 retained compatibility cleanup。
- 若下方历史 handoff bullet 仍写“下一默认 PRD 返回 Phase-3 same-phase candidate selection / bounded proposal”、Phase-4 retained-boundary refresh、checklist reconciliation / recommender package，或 Phase 1-4 gap closure，一律视为当时的 no-reopen / pool-retention / completed-control-plane 记录，不得作为当前 PRD 生成入口。当前入口只能取 `## 合理 PRD 形状硬约束` 与 `## 当前默认下一步`。
...
- 2026-06-15 RegularMul full crit damage bounded implementation PRD 已完成 US-008 final handoff：`Calculator.RegularMul.cal_crit_dmg(data)` 已通过 `_calculate_full_crit_damage(static_statement, dynamic_statement, judge_node)` helper seam 实现 / no-op verified，public signature、current `SkillNode` assertion、`aftershock_attack` label branch、`received_crit_dmg_bonus` inclusion、`min(5, crit_dmg)` cap、full-vs-personal crit damage contrast、`_CalculatorReadSnapshot` 字段集、old containers、event/runtime/listener layers、validation-runner behavior、registered routes 和 retained compatibility 均保持不变。US-008 handoff 记录 focused full-crit pytest、scoped mypy、focused docs `git diff --check`、Ralph JSON sanity、UTF-8 / mojibake scan、campaign refresh、reviewer / invariant verdict 与 rollback anchors；未发现新 Buff coupling 或既有耦合分类变化，因此不更新 [旧Buff系统耦合审查结果.md](./旧Buff系统耦合审查结果.md)。下一默认 PRD 返回 Phase-3 same-phase candidate selection / bounded proposal，而不是继续跟随 full crit damage；same-phase pool 继续保留 registered behavior sample eligibility、remaining `Calculator.RegularMul` branches / retained-only sheer follow-up、future `Calculator.StunMul.get_stun_array()` follow-up if named evidence appears、P2-A through P2-G guarded maintenance、retained compatibility 与 blocker-only reopen rules。
- 2026-06-16 CalAnomaly copied-output formula helper-family PRD 已完成 US-008 final handoff：`CalAnomaly.set_final_multipliers(...)` / `cal_anomaly_dmg()`、`CalDisorder` base / extra / stun、`CalPolarityDisorder` base-damage adjustment 与 `CalAbloom` ratio helper seams 均已 implemented / no-op verified；US-006 copied-output / event boundary gate 与 US-007 serial verifier 证明本 PRD 未触达 `CopyAnomalyForOutput.py`、`UpdateAnomaly.py` production behavior、handler/report payload、dispatch/runtime ports、listener/dot/runtime layers、old containers、validation-runner behavior、registered routes 或 retained compatibility。Verifier evidence：focused CalAnomaly helper-family pytest `18 passed`、scoped mypy `Success: no issues found in 2 source files`、serial `formula-parity` mypy clean on `9 source files`、serial `calculator-reads` mypy clean on `22 source files`、handoff docs / JSON / UTF-8 checks；`implicit-events` 与 full validation 继续按条件跳过，因为没有 copied-output / event/runtime/listener / dot-runtime / lifecycle / validation-runner / retained-compatibility diff。Reviewer verdict：PASS，`scripts/ralph/architecture/invariants.md` 仍成立，changed files 只在 handoff / Ralph evidence boundary 内；未发现新 Buff coupling 或既有耦合分类变化，因此不更新 [旧Buff系统耦合审查结果.md](./旧Buff系统耦合审查结果.md)。

## 本文档的用途

- 记录“当前 PRD 完成后，下一阶段准备调查 / 实现什么”。
- 为下一轮 `scripts/ralph/prd.json` 提供吞吐更大的候选包、调查清单、实现边界和验证入口。
- 避免 PRD 方向重新漂移回“看到一个 XLogic 就改一个”的模式，也避免连续多个大 cycle 只做单点调查 / 单点 handoff。

## PRD 取材原则

- 本文档不强制规定 PRD 的固定 story 结构；`prd` skill 负责从本文档和 handoff docs 生成 `tasks/prd-*.md`，`ralph` skill 再依据该 PRD 的 `## User Stories` 转换出 `scripts/ralph/prd.json` 的 US。PRD 是大 cycle 的范围 / 吞吐单位，US 是 PRD 内的执行细分与验收单元，不设人工数量上限；只要求每个 US 仍能在一个 Ralph iteration 内完成或明确 stop-and-split。
- 本文档必须给 PRD 生成器提供足够大的候选包，而不是只描述一个最小安全下一步。每次更新“当前默认下一步”时，都要同时保留后续同阶段候选块，避免自动循环在完成一个窄切片后只能继续生成很薄的 PRD。
- 每个候选块都应写清：候选文件 / 符号、当前已知耦合、可以拆出的 Ralph-sized 工作方向、必须保留的边界、验证入口和不应触碰的非目标。若多个调查项共享相同 owner、风险面、验证 profile、rollback anchor 和 stop condition，应默认放进同一个 PRD，而不是拆成多个大 cycle。
...
- 后续自动推进必须带有收敛节奏：连续 2-3 个 bounded PRD 后，或当前同族候选块被全部实现、降级或阻塞后，下一轮应优先生成 route review / convergence review。该 review 的验收标准是更新 checklist / 本文当前状态 / 替换说明中的完成判据状态，并把每个 retained / blocker 分类为 `exact implementation candidate`、`exact deletion-readiness candidate`、`needs oracle / registered sample`、`long-term compatibility`、`explicit blocker with human-approved route` 或 `No-Go`。

## 合理 PRD 形状硬约束

- Phase 3 默认 PRD 形状是 implementation batch 或同 PRD 内 Go / No-Go closure，不再默认生成 full-cycle `same-phase candidate selection / bounded proposal`。Selection / proposal 只能作为 implementation PRD 的前置 1 个 US，或者作为某个 story 的 investigation packet；除非当前根工作区源码、focused test、validation gate 或 reviewer evidence 明确互相矛盾，不得单独占用 8-12 个 US。
- 合理的 Phase 3 implementation PRD 应关闭一个同族候选块，而不是一个微小 callsite。默认 5-8 个 US：最多 1 个 intake / preflight；至少 2-4 个 implementation + focused-test story；最多 1 个 serial validation story；最多 1 个 final handoff story。若 70% 以上 US 都是 route reconciliation、candidate table、public contract review、registered-sample eligibility、validation packet、proposal packet、reviewer packet 或 handoff，而没有生产 diff / focused oracle diff / guardrail diff，则该 PRD 形状不合格。
...
Phase 1-4 当前默认路由：BuffXLogic dot/debuff runtime-state and anomaly-map exact intake, while preserving copied-output / formula parity evidence and the broader same-phase retained candidate pool.

- 决策基线：Phase 5 暂停；current copied-output / dot-debuff exact-intake PRD 已完成。Copied-output helper outcome = implemented / no-reopen for exact predicate delegation: `HugoCorePassiveTotalizeTrigger.py` now uses `read_enemy_stun_active(...)` for the two copied-output stun predicates, and `VivianCorePassiveTrigger.py`, `VivianCinema6Trigger.py`, `YanagiPolarityDisorderTrigger.py` now use `read_enemy_anomaly_active(...)` only for judge-time anomaly-active predicates. Focused copied-output bundle passed with `36 passed`; excluded-pool guardrail bundle passed with `30 passed`; scoped mypy passed on `13 source files`; serial `implicit-events` passed with base `2 passed`, isolated teams `3 passed`, focused `323 passed`, and mypy success on `93 source files`.
- 当前默认下一 PRD：generate `BuffXLogic Dot/Debuff Runtime-State And Anomaly-Map Exact Intake`. The PRD should start with compact current-root preflight over the retained files, then choose one coherent same-risk subfamily rather than one safe callsite. It must name exact files / symbols / behavior, focused verifier, rollback anchors, retained boundaries and non-goals, then produce one of:
  - bounded production helper / reader-adapter delegation plus focused characterization and guardrail updates for a proven read-only dot/debuff runtime-state or anomaly-map predicate;
  - a blocker-only No-Go packet with retained boundary and rollback evidence, only after no implementable sibling in the chosen retained subfamily can be named;
  - a stop-for-human route review if no exact implementation candidate can be named across the retained dot/debuff and anomaly-map pool.
- Completed / no-reopen helper subset:
  - Helper: `read_enemy_anomaly_active(...)` in `zsim/sim_progress/Buff/BuffXLogic/enemy_anomaly_read.py`; implemented simple users are `ElectroLipGlossAtkAndDmgBonus`, `JaneAdditionalAbilityPhyBuildupBonus`, `MarcatoDesireAtkBonus`, and `TimeweaverApBonus`; implemented copied-output users are `VivianCorePassiveTrigger.py`, `VivianCinema6Trigger.py`, and `YanagiPolarityDisorderTrigger.py`.
  - Helpers: `read_enemy_shock_active(...)` / `read_enemy_stun_active(...)` in `zsim/sim_progress/Buff/BuffXLogic/enemy_state_read.py`; implemented simple users are `LinaAdditionalSkillEleDMGBonus` for shock and `Soldier11AdditionalSkillExtraFireDMGBonus`, `YixuanAdditionalAbilityDmgBonus`, `YixuanCinema2StunTimeLimitBonus`, and `YuzuhaCinema2Trigger` for stun; implemented copied-output user is `HugoCorePassiveTotalizeTrigger.py`.
  - Helpers: `read_enemy_frozen_edge_state(...)`, `read_enemy_stun_edge_state(...)`, and `read_enemy_frost_frostbite_edge_state(...)` in `zsim/sim_progress/Buff/BuffXLogic/enemy_edge_state_read.py`; implemented users are `BranchBladeSongCritRateBonus`, `PolarMetalFreezeBonus`, `LighterUniqueSkillStunTimeLimitBonus`, `LyconAdditionalAbilityStunVulnerability`, `QingYiCoreSkillStunDMGBonus`, `WeepingGeminiApBonus`, and `MiyabiCoreSkill_FrostBurn`.
  - Guardrail: `tests/simulator/test_enemy_dynamic_read_guardrail.py` must continue to reject helper imports / calls outside the approved helper-specific files unless a later PRD updates the approved subset with new characterization evidence.
  - Rollback anchors: helper modules, delegated BuffXLogic files, copied-output dispatch tests, helper tests, `tests/simulator/test_enemy_dynamic_read_guardrail.py`, focused replacement notes, evidence ledger and this next-stage draft.
- Candidate block A - copied-output / formula parity adjacent reads:
  - Candidate files / symbols: implemented predicate files `HugoCorePassiveTotalizeTrigger.py`, `VivianCorePassiveTrigger.py`, `VivianCinema6Trigger.py`, `YanagiPolarityDisorderTrigger.py`; retained copied-output/formula anchors `get_active_anomaly()`, copied-output constructors, `UpdateAnomaly.spawn_output(...)`, `DirgeOfDestinyAnomaly`, `CalAnomaly`, `Calculator.py`, `MultiplierData`, `MulData`, `DynamicStatement`, `_CalculatorReadSnapshot`, and `CalculatorBuffAttributeReader`.
  - Known coupling: predicate reads are implemented and exact, but copied anomaly / disorder payload construction, report payload parity, anomaly formula snapshots, AP / AM / impact / stun formula state and validation-runner profiles remain caller-owned or retained.
  - Ralph-sized work directions: do not generate another copied-output predicate helper PRD. Reopen only if current-root source, focused regression, guardrail failure, validation failure or reviewer evidence names an exact copied-output payload / formula / reader behavior and rollback path.
  - Retained boundaries: copied-output payload fields, `get_active_anomaly()`, `spawn_output(...)`, retained public formula methods, `MultiplierData` / `MulData` / `DynamicStatement`, old containers and event/runtime/listener layers remain separate.
  - Validation entrypoints: file-specific focused pytest, `test_enemy_dynamic_read_guardrail.py`, `formula-parity` for formula output changes, `calculator-reads` for reader/snapshot surfaces, and `implicit-events` for copied-output or event-adjacent behavior.
  - Non-goals: broad `Calculator.py` / `CalAnomaly.py` rewrite, payload schema rewrite, public reader/snapshot expansion, retained compatibility deletion, or validation-runner rewrite.
- Candidate block B - dot/debuff runtime-state and anomaly-map reads:
  - Candidate files / symbols: `MiyabiCoreSkill_IceFire.py`, `VivianDotTrigger.py`, `AnomalyDebuffExitJudge.py`, `HailstormShrineIceBonus.py`, `MiyabiAdditionalAbility_IgnoreIceRes.py`, `enemy.dynamic.dynamic_dot_list`, `enemy.dynamic.dynamic_debuff_list`, and `getattr(...enemy.dynamic, name)`.
  - Known coupling: these paths can be lifecycle/runtime-state reads rather than simple boolean gates; dot runtime registration, debuff mirror identity, anomaly-map lookup and scheduled publish / listener broadcast separation must stay explicit.
  - Ralph-sized work directions: this is the current default next PRD. Characterize read-only versus runtime mutation first; only delegate a pure read when focused tests prove no dot registration, debuff mutation, scheduled publish, listener broadcast or same-tick write is moved; otherwise emit blocker / No-Go.
  - Retained boundaries: dot runtime registration and removal, enemy debuff mirror identity, `RuntimeCommandPort`, `LegacyRuntimeCommandAdapter`, `ScheduleDispatchPort` publish, old containers and retained lifecycle paths remain separate.
  - Validation entrypoints: focused runtime-state pytest, guardrail scan for `getattr(...enemy.dynamic, name)`, `implicit-events` for event-adjacent branches, lifecycle/default validation only if old-container lifecycle paths are touched.
  - Non-goals: dot/debuff single-source rewrite, second runtime write facade, raw queue passthrough, listener/event/runtime layer merge, or old-container deletion.
- Candidate block C - old containers / lifecycle compatibility:
  - Candidate files / symbols: `exist_buff_dict`, `DYNAMIC_BUFF_DICT`, `LOADING_BUFF_DICT`, `BuffLoadLoop()`, legacy `buff_add()`, `KickOutBuff()`, `LegacyBuffRuntimeFacade`, `ScheduleBuffSettle(...)`, pending activation and active removal paths.
  - Known coupling: old template identity, active/pending queue identity and same-tick settle fallback remain retained compatibility, not accidental cleanup.
  - Ralph-sized work directions: only refresh evidence or open deletion-readiness work when a current-root guardrail / validation failure names exact unused or behavior-preserving scope.
  - Retained boundaries: old container identity, facade-backed writes, `RuntimeCommandPort` same-tick writes and formula/copy-output behavior remain untouched by reader PRDs.
  - Validation entrypoints: lifecycle/facade focused tests, `implicit-events`, `git diff --check`, UTF-8 scans and reviewer invariant check.
  - Non-goals: deleting old containers, rewriting lifecycle broadly, or treating retained evidence as active backlog without source proof.
- Candidate block D - `JudgeTools` helper responsibility slices:
  - Candidate files / symbols: `Buff/JudgeTools/*`, `check_preparation(...)`, lazy record initialization, template identity lookup, trigger-state helpers, enemy context, preload/action context and tick / RNG-adjacent reads.
  - Known coupling: these helpers preserve record/template identity and old context discovery; deleting them as a side effect of read-helper work risks breaking caller prerequisites.
  - Ralph-sized work directions: map one helper responsibility at a time and add focused guardrails before any cleanup; keep read-helper PRDs owner-family scoped.
  - Retained boundaries: no raw queue discovery reopen, no broad helper deletion, no hidden public contract expansion.
  - Validation entrypoints: focused JudgeTools tests, guardrail scans, `implicit-events` only when event-adjacent behavior is touched.
  - Non-goals: deleting helper responsibilities because a separate reader/state-sync helper landed.
- Candidate block E - planned-event producer and handler-requeue families:
  - Candidate files / symbols: `ScheduleDispatchPort`, `create_schedule_dispatch_port(...)`, producer families already closed through dispatch, not-yet-executable handler requeue files, listener broadcast paths, local event groups, dot runtime registration and same-tick writes.
  - Known coupling: publish semantics, scheduler requeue, synchronous listener broadcast and runtime writes are separate layers.
  - Ralph-sized work directions: open producer work only when source names exact payload, target fan-out and order; otherwise keep closed producers no-reopen.
  - Retained boundaries: dispatch adapters are created on demand and not cached across `ScheduleData.reset_myself()` queue rebinds.
  - Validation entrypoints: `implicit-events`, focused producer tests, ordering assertions and reviewer invariant check.
  - Non-goals: raw queue passthrough, generic event bus merge, cached dispatch adapters or handler-requeue rewrite inside reader PRDs.
- Candidate block F - registered behavior / performance gates:
  - Candidate files / symbols: `scripts/run_buff_main_loop_consistency.py`, `scripts/run_buff_runtime_benchmark.py`, registered teams/APLs, stop-tick samples and runtime benchmark fixtures.
  - Known coupling: these gates are meaningful only for live semantic or performance-sensitive diffs with real registered routes and nonzero relevant counts.
  - Ralph-sized work directions: require registered-route evidence only after a production semantic or performance-sensitive change; record skip rationale for docs/test-only/helper-only work.
  - Retained boundaries: no validation-only teams, fake APLs or retained-vs-retained samples.
  - Validation entrypoints: focused tests first, then main-loop / benchmark only when the touched surface warrants it.
  - Non-goals: making Phase 5 registered-route validation the default for handoff docs.
- Candidate block G - completed guarded-maintenance and blocker-only reopen rules:
  - Candidate files / symbols: completed `read_enemy_anomaly_active(...)`, `read_enemy_shock_active(...)`, `read_enemy_stun_active(...)`, edge-state helpers, P2-A through P2-G, AM/AP/impact scalar helpers, selected Stun, RegularMul helper families, CalAnomaly helper family, copied-output handler/report surfaces, old-container and lifecycle retained buckets.
  - Known coupling: completed surfaces are reviewer context and rollback anchors, not automatic backlog.
  - Ralph-sized work directions: reopen only with current-root source, focused regression, guardrail failure, validation failure or reviewer evidence naming exact file / symbol / behavior and rollback path.
  - Retained boundaries: no generic same-phase selection loop, no deletion by assertion, no public contract expansion.
  - Validation entrypoints: candidate-specific focused tests, existing validation profiles, docs diff checks, UTF-8 scans and reviewer invariant verdict.
  - Non-goals: re-generating PRDs for completed helper subsets or using blocker-only evidence to force production changes.
- 验证入口：docs/control-plane handoff 继续使用 `git diff --check -- docs scripts/ralph tasks tests zsim`、Ralph JSON sanity when Ralph JSON is changed、UTF-8 / U+FFFD / mojibake scans 和 scoped Ralph tooling typecheck。`BuffXLogic` source work must add focused runtime-state / read-gate pytest; add `calculator-reads` when attribute readers are touched, `formula-parity` only for formula output changes, and `implicit-events` for event-adjacent branches. Run profiles serially.
- 非目标：不运行 Phase 5 registered-route validation as the default; 不删除 old containers / retained compatibility without exact unused or behavior-preserving proof; 不删除 `Buff.BuffLogic` / old template identity / lazy record initialization; 不合并 event queue / listener broadcast / dot runtime / local event groups / scheduled handler requeue / runtime reads / same-tick runtime write layers; 不扩展 public reader/snapshot APIs; 不新增 validation-only registered team。
- Handoff 要求：下一 PRD 完成时必须回写 checklist、本文当前默认下一步、same-phase candidate pool 和替换说明；若发现新耦合或分类变化，才回写旧 Buff 耦合审查。只有全部 Phase 1-4 gap 被关闭、降级为长期兼容策略，或形成 explicit blocker with human-approved route 后，才允许恢复 Phase 5 默认 PRD。
...
### Current Anomaly Duration Runtime-View Matrix PRD final handoff

- Implemented / characterized surfaces:
  - `AnomalyBar.__get_duration_enemy_buffs(...)` now has focused matrix evidence for runtime-view enemy Buff precedence through `BuffRuntimeReadPort.get_active_buffs("enemy")`, legacy `dynamic_buff_dict["enemy"]` fallback, missing legacy `"enemy"` `TypeError`, `dynamic_buff_dict is None` empty sequence behavior, no duration metadata, empty enemy Buff list, inactive / unrelated / unknown-key Buffs, fixed deltas, percentage deltas, count-scaled effects, multi-Buff accumulation and negative clamp.
  - The retained duration formula remains `basic_max_duration * (1 + summed_pct) + summed_fixed`, with fixed and percentage effects scaled by `Buff.dy.count` and final `max(..., 0)` clamping.
- Same-PRD No-Go / retained branches:
  - US-006 records No-Go for additional production work inside this PRD. No production source, focused test source, validation-runner behavior, public reader/snapshot API, registered route, old-container deletion, event/runtime/listener layer merge, copied-output behavior, formula path or retained compatibility cleanup is authorized by this handoff.
- Validation evidence:
...
- Rollback anchors:
  - `AnomalyBar.change_info_cause_active(...)`, `AnomalyBar.__get_max_duration(...)`, `AnomalyBar.__get_duration_enemy_buffs(...)`, `BuffRuntimeReadPort.get_active_buffs("enemy")`, `LegacyBuffRuntimeReadAdapter.get_active_buffs(...)`, focused duration nodeids in `tests/simulator/test_anomaly_handler_runtime_view.py`, retained `calculator-reads`, retained `implicit-events`, and this handoff doc set.
- Reviewer / invariant verdict:
  ...
  - No new Buff coupling or changed anomaly-duration coupling classification was found; [旧Buff系统耦合审查结果.md](./旧Buff系统耦合审查结果.md) remains unchanged.
- Next-step routing:
  - Default next PRD: remaining `Calculator.RegularMul` / retained-only sheer blocker package, bounded to evidence review or same-PRD Go / No-Go closure for `base_attr=4`, `cal_sheer_dmg_bonus(data)`, reader-contract eligibility and real registered-route evidence.
  - Same-phase pool remains: registered behavior sample eligibility only for future live semantic diffs with real route / nonzero counts, future `Calculator.StunMul.get_stun_array()` follow-up only if named regression / oracle / validation evidence appears, P2-A through P2-G guarded maintenance only on concrete blockers, retained compatibility, and blocker-only phase-1 reopen rules.

### Current AnomalyBar / UpdateAnomaly write-path field matrix PRD final handoff

- Implemented / characterized surfaces:
  - `AnomalyBar.current_ndarray` lifecycle and settlement matrix is characterized: effective-only `ndarray_box` accumulation, `(1, 1)` reset behavior, `current_effective_anomaly`, weighted-average `current_ndarray`, `settled=True`, deepcopy non-aliasing, and retained shallow-copy compatibility.
  - `UpdateAnomaly.update_anomaly(...)` / `spawn_output(...)` write paths are characterized for mode 0 `NewAnomaly`, mode 1 `Disorder`, mode 2 `PolarityDisorder`, invalid mode, missing polarity ratio, active-bar deepcopy, original bar reset, listener broadcast, scheduled publish, dot runtime, and same-tick debuff-write separation.
- Same-PRD No-Go / retained branches:
  - US-006 records No-Go for additional production work inside this PRD. No live production source, focused test source, validation-runner behavior, copied-output constructor, handler/report payload, registered team/APL, lifecycle container, old-container path, public reader/snapshot API, event/runtime/listener layer merge, or retained compatibility path is authorized by this handoff.
- Validation evidence:
  ...
  - Scoped mypy over `AnomalyBarClass.py`, `UpdateAnomaly.py`, `CalAnomaly.py`, `Calculator.py`, and the focused tests exited `0` with `Success: no issues found in 7 source files`.
  - Serial `formula-parity`, `implicit-events`, and `calculator-reads` profiles all exited `0`; full validation stayed skipped because no lifecycle container, runtime write path, old-container behavior, or validation-runner behavior changed.
  - US-007 docs gate adds focused docs diff check, Ralph JSON sanity, UTF-8 / mojibake scan, scoped Ralph tooling typecheck, checkpoint, reviewer verdict, and completion bookkeeping.
- Rollback anchors:
  - `zsim/sim_progress/anomaly_bar/AnomalyBarClass.py` current-ndarray / duration-read anchors, `zsim/sim_progress/Update/UpdateAnomaly.py` write-path anchors, copied-output constructors, focused pytest nodeids in the three simulator test files, `scripts/ralph/investigations/2026-06-16-US-006-serial-validation-go-no-go.md`, retained `formula-parity` / `implicit-events` / `calculator-reads` profiles, and this handoff doc set.
- Retained boundaries / reviewer verdict:
  - PASS against `scripts/ralph/architecture/invariants.md`: event queue semantics, synchronous listener broadcasts, same-tick runtime writes, explicit dispatch/runtime ports, old containers, validation-runner behavior, registered routes, retained formula/read compatibility, and no-reopen rules remain intact.
  - No new Buff coupling or coupling-classification evidence was found; [旧Buff系统耦合审查结果.md](./旧Buff系统耦合审查结果.md) remains unchanged for this handoff.
- Next-step routing:
  ...
  - Same-phase pool remains available after that default: retained-only sheer blocker package if reader-contract and real route evidence appear, registered behavior sample eligibility only for live semantic diffs, future Stun evidence only when named by regression / oracle / validation evidence, P2-A through P2-G guarded maintenance only on concrete blockers, retained compatibility, and blocker-only phase-1 reopen rules.

### Current CalAnomaly copied-output formula helper-family PRD final handoff

- Implemented / no-op verified surfaces:
  - `_assemble_final_multiplier_vector(...)` 与 `_calculate_anomaly_damage_expectation(...)` 保留 `CalAnomaly.set_final_multipliers(...)` / `cal_anomaly_dmg()` 的 public call anchors、final multiplier order、snapshot impact / stun bonus 处理和 `AnomalyBar.scaling_factor` 位置。
  - `_calculate_disorder_base_damage(...)`、`_calculate_disorder_extra_multiplier(...)`、`_calculate_disorder_stun_multiplier(...)`、`_calculate_polarity_disorder_base_damage(...)` 与 `_apply_abloom_anomaly_damage_ratio(...)` 只拆出同文件公式 arithmetic；copied-output payload、listener broadcast、scheduled publish、dot runtime、same-tick runtime writes 和 handler report paths 未替换。
- Same-PRD No-Go / retained branches:
  - `CopyAnomalyForOutput.py` constructors、`UpdateAnomaly.py` production behavior、anomaly / disorder / polarity / abloom handler report payloads、validation-runner behavior、registered team / APL creation、old-container deletion、retained compatibility cleanup、public reader / snapshot expansion、event/runtime/listener layer merge 均未改动。
- Validation evidence:
  - Focused CalAnomaly helper-family pytest node batch exited `0` with `18 passed`.
  - Scoped mypy for `CalAnomaly.py` and `test_buff_attribute_reader.py` exited `0` with `Success: no issues found in 2 source files`.
  - Serial `formula-parity` exited `0` with base simulator, isolated teams, focused formula regression, scoped mypy success on `9 source files`, and success marker.
  - Serial `calculator-reads` exited `0` with base simulator, isolated teams, focused reader regression, scoped mypy success on `22 source files`, and success marker.
  - `implicit-events` and full validation were skipped by condition because this PRD did not change copied-output, event/runtime/listener, dot-runtime, lifecycle, runtime-write, validation-runner, old-container, or retained compatibility surfaces.
- Rollback anchors:
  - `zsim/sim_progress/ScheduledEvent/CalAnomaly.py` helper seams and retained public methods, `tests/simulator/test_buff_attribute_reader.py` CalAnomaly / CalDisorder / CalPolarityDisorder / CalAbloom focused oracle nodeids, `scripts/ralph/investigations/2026-06-16-US-006-copied-output-event-boundary-gate.md`, `scripts/ralph/checkpoints/2026-06-16-us-006-copied-output-event-boundary-gate.md`, retained `formula-parity` / `calculator-reads` profiles, and this handoff doc set.
- Retained boundaries / reviewer verdict:
  - PASS against `scripts/ralph/architecture/invariants.md`: event queue semantics, synchronous listener broadcasts, same-tick runtime writes, explicit ports/adapters, old containers, validation-runner behavior, registered routes, retained compatibility paths and completed Buff surfaces remain intact.
  - No new Buff coupling or coupling-classification evidence was found; [旧Buff系统耦合审查结果.md](./旧Buff系统耦合审查结果.md) remains unchanged for this handoff.
- Next-step routing:
  - Default next PRD: `AnomalyBar.current_ndarray` / `UpdateAnomaly.py` write-path field matrix bounded characterization or same-PRD Go / No-Go closure.
  - Same-phase pool remains available after that default: enemy dynamic / `AnomalyBar.__get_duration_enemy_buffs()` duration reads, retained-only sheer blocker package if reader-contract and real route evidence appear, registered behavior sample eligibility only for live semantic diffs, P2-A through P2-G guarded maintenance only on concrete blockers, retained compatibility, and blocker-only phase-1 reopen rules.

### Current non-sheer base implementation PRD final handoff

- Implemented / no-op verified surfaces:
  - `Calculator.RegularMul.cal_base_attr(base_attr in 0..3, data)` 已通过 module-local `_calculate_non_sheer_base_attribute(base_attr, static_statement, dynamic_statement)` helper seam 收口；public method signature 和 call shape 不变。
  - `Calculator.RegularMul.cal_base_dmg(data)` 已通过 module-local `_calculate_base_damage(damage_ratio, attr, dynamic_statement)` helper seam 收口；`SkillNode` assertion、`diff_multiplier`、`skill.diff_multiplier` 和 `cal_base_attr(base_attr, data)` 的相对顺序保留。
- Same-PRD No-Go / retained branches:
  - `base_attr=4`、`cal_sheer_dmg_bonus(data)`、runtime `char_instance.sheer_attack_conversion_rate`、public `CalculatorBuffAttributeReader` API expansion、`_CalculatorReadSnapshot` expansion、registered team / APL creation、validation-runner rewrite、event/runtime/listener paths、old-container deletion、retained compatibility cleanup 均未改动。
- Validation evidence:
  - Focused pytest for five `regular-base-*` oracle rows plus base-damage helper formula/delegation coverage exited `0` with `8 passed`.
  - Scoped mypy for `Calculator.py` and `test_buff_attribute_reader.py` exited `0` with `Success: no issues found in 2 source files`.
  - Serial `formula-parity` exited `0` with base simulator `2 passed`, isolated teams `3 passed`, focused formula `159 passed`, mypy success on `9 source files`, and `[验证完成] 所有步骤通过`.
  - Serial `calculator-reads` exited `0` with base simulator `2 passed`, isolated teams `3 passed`, focused reader `259 passed`, mypy success on `22 source files`, and `[验证完成] 所有步骤通过`.
- Rollback anchors:
  - `zsim/sim_progress/ScheduledEvent/Calculator.py::Calculator.RegularMul.cal_base_attr(...)`, `_calculate_non_sheer_base_attribute(...)`, `Calculator.RegularMul.cal_base_dmg(data)`, `_calculate_base_damage(...)`, the five `regular-base-*` oracle rows, `test_regular_mul_base_damage_scalar_helper_preserves_current_formula`, `test_regular_mul_base_damage_delegates_to_module_helper_and_base_attr_call`, retained `formula-parity` / `calculator-reads`, and this handoff doc set.
- Retained boundaries:
  - Event queue semantics、synchronous listener broadcasts、same-tick runtime writes、`ScheduleDispatchPort` / `RuntimeCommandPort` layers、old containers、copied-output constructors、validation-runner behavior、registered routes 和 retained compatibility paths 均保持不变。
  - 本 PRD 未发现新的 Buff coupling 或既有 coupling-classification 变化，因此不更新 [旧Buff系统耦合审查结果.md](./旧Buff系统耦合审查结果.md)。

### Current retained-only sheer blocker closure PRD final handoff

- Implemented / no-op verified surfaces:
  - `Calculator.RegularMul.cal_base_attr(..., base_attr=4)` now delegates the retained sheer conversion block to module-local `_calculate_sheer_base_attribute(...)`.
  - `Calculator.RegularMul.cal_sheer_dmg_bonus(data)` now delegates the `diff_multiplier == 4` branch to module-local `_calculate_sheer_damage_bonus(...)`.
- Exact oracle / helper / No-Go evidence:
  - `test_regular_mul_retained_sheer_base_attr_requires_char_instance_conversion_rate()` locks runtime `char_instance.sheer_attack_conversion_rate`, recursive non-sheer base-attr conversion, `dynamic.sheer_atk`, and reader-snapshot `AttributeError` for missing `char_instance`.
  - `_FORMULA_ORACLE_TABLE_CASES` row `regular-sheer-dmg-bonus-retained-snapshot` locks retained / reader-snapshot `dynamic.sheer_atk=5.0`, `dynamic.sheer_dmg_bonus=0.45`, and `cal_sheer_dmg_bonus(data)=1.45` for `diff_multiplier=4`.
  - US-003 keeps `CalculatorBuffAttributeReader` unchanged and `_CalculatorReadSnapshot` limited to `static`, `dynamic`, `judge_node`, `enemy_obj`, and `char_level`; no public reader/snapshot expansion is authorized.
  - US-004 records Conditional No-Go for registered main-loop sampling: the real Yixuan / sheer data exists, but no current registered consistency team/APL reaches that route; no validation-only team, fake APL, fixture-only route, or retained-vs-retained sample was created.
- Validation evidence:
  - Focused retained sheer pytest exited `0` with `32 passed, 130 deselected`.
  - Scoped mypy over `Calculator.py` and `test_buff_attribute_reader.py` exited `0` with `Success: no issues found in 2 source files`.
  - Serial `formula-parity` exited `0` with base simulator `2 passed`, isolated teams `3 passed`, focused formula slice `162 passed`, scoped mypy success on `9 source files`, and `[验证完成] 所有步骤通过`.
  - Serial `calculator-reads` exited `0` with base simulator `2 passed`, isolated teams `3 passed`, focused reader / guardrail slice `262 passed`, scoped mypy success on `22 source files`, and `[验证完成] 所有步骤通过`.
- Retained boundaries / reviewer verdict:
  - PASS against `scripts/ralph/architecture/invariants.md`: public `RegularMul` signatures, `SkillNode` / `MultiplierData` assumptions, event queue semantics, synchronous listener broadcasts, same-tick runtime writes, explicit ports/adapters, old containers, validation-runner behavior, registered routes, lifecycle paths, and retained compatibility remain unchanged.
  - No new Buff coupling or coupling-classification evidence was found; [旧Buff系统耦合审查结果.md](./旧Buff系统耦合审查结果.md) remains unchanged for this handoff.
- Next-step routing:
  - Retained-only sheer is helper-complete but still blocked for public reader/snapshot expansion and real registered-route sampling. Do not generate another retained-only sheer implementation PRD without new evidence that closes those blockers.
  - Current default next PRD: Phase-3 same-phase candidate-pool closure / exact bounded candidate selection. Preserve the same-phase pool below rather than reducing future intake to this retained-only sheer branch.
...
- Current selected-family handoff：2026-06-18 impact / crit / stun owner-family state-sync/read-gate PRD 已完成 `LighterAdditionalAbility_IceFireBonus.py`、`QingYiAdditionalAbilityStunConvertToATK.py`、`TriggerAdditionalAbilityStunBonus.py`、`Soldier0AnbyCoreSkillCritDMGBonus.py` 的 current-root preflight、focused characterization、state-sync order assertions、guardrails 和 serial validation；current-root source already matched the intended reader/state-sync boundary, so no production diff was required.
...
#### 候选块 Phase-3：RegularMul scalar helper-family bounded implementation PRD（已完成 / same-phase pool retained）

- 候选文件 / 符号：`zsim/sim_progress/ScheduledEvent/Calculator.py` 中的 `Calculator.RegularMul.cal_dmg_bonus(data)`、`cal_res_mul(...)`、`cal_dmg_vulnerability(...)`、`cal_stun_vulnerability(data)`、`cal_special_mul(data)`；条件纳入 `cal_defense_mul(data)` / `cal_k_attacker()` / `cal_pen_ratio()` / `cal_recipient_def(...)`、`cal_crit_expect(data)`、非 sheer `cal_base_dmg(data)` / `cal_base_attr(base_attr in 0..3, data)`；每个纳入面都应使用 module-local behavior-preserving helper seam 或等价小型私有 helper family。focused oracle anchor 至少包括 `tests/simulator/test_buff_attribute_reader.py::test_formula_oracle_table_cases_drive_expected_fields_and_reader_parity[regular-dmg-bonus-character-field-stack]` 与 `test_calculator_regular_mul_branch_matrix_characterizes_selected_methods[fire-stunned-reader-snapshot]`，缺口由本 PRD 早期 story 一次性补足。
- 当前 proposal outcome：Conditional Go / No-Go for one later bounded implementation PRD, widened by PRD generation rule into one RegularMul scalar helper-family batch。允许的 diff 只可保持 public method signatures、current `SkillNode` assumptions、element / trigger / label dispatch、all-damage / all-res / all-vulnerability additions、enemy dynamic reads、existing helper outputs、focused oracle rows 与 rollback anchors；不得改变 public reader contract、`_CalculatorReadSnapshot` shape、registered fixtures、validation-runner behavior、event queue、listener broadcast、same-tick runtime writes、old containers 或 retained compatibility。
- 历史默认下一 PRD 已执行完成：damage bonus + vulnerability/resistance/special scalar group、defense helper group、crit expectation group 已 implemented / no-op verified；non-sheer base damage / base attr 后续也已完成。当前不再把本块作为 active default；只有 shared oracle / validation / invariant 失败时才以具体 blocker 重开。
- Retained gates：先跑 selected focused pytest nodeids 与 scoped mypy；formula / reader work 继续串行保留 `formula-parity` 与 `calculator-reads`；只有触达 copied-output、event、dispatch/runtime、listener、dot-runtime 或 same-tick runtime write surface 时追加 `implicit-events`；触达 lifecycle container / runtime write path 时追加默认 profile。验证 profile 必须串行执行，不并发。
- Registered-sample decision：当前仍为 Conditional No-Go。本 PRD 没有 live production semantic diff，也没有真实 registered route 在 explicit stop tick 内证明 nonzero selected `cal_dmg_bonus()` relevance；future sample 必须记录 team、APL、stop tick、runtime labels、total damage comparison、relevant formula/event counts、full event counts、Buff timeline comparison、differences 与 rollback anchors。
- 同阶段候选池：registered behavior sample eligibility、remaining `Calculator.RegularMul` branches / retained-only sheer follow-up、future `Calculator.StunMul.get_stun_array()` follow-up if named evidence appears、P2-A through P2-G guarded maintenance、retained compatibility 与 blocker-only reopen rules 均继续保留。当前 US-011 handoff 未发现新 Buff coupling 或既有耦合分类变化，因此 `docs/旧Buff系统耦合审查结果.md` 不需要更新。
- 非目标：不做 broad `Calculator.py` / `CalAnomaly.py` rewrite；不把 array outputs、retained-only sheer、`base_attr=4`、registered behavior sample、event/runtime/listener paths 或 old-container cleanup 混入本 batch；不新增 public `CalculatorBuffAttributeReader` damage-bonus reader；不扩展 `_CalculatorReadSnapshot`；不重开 completed copied-output / `cal_res_pen()` / AM/AP/impact / selected Stun / crit helper seams；不创建 validation-only registered team；不删除 old containers、legacy `buff_add()` / `KickOutBuff()` 或 retained compatibility。
...
#### 候选块 Phase-3：RegularMul full crit damage bounded implementation PRD（已完成 / same-phase pool retained）

- 候选文件 / 符号：`zsim/sim_progress/ScheduledEvent/Calculator.py` 中的 `Calculator.RegularMul.cal_crit_dmg(data)` 与 module-local `_calculate_full_crit_damage(static_statement, dynamic_statement, judge_node)`；contrast anchors 为 `Calculator.RegularMul.cal_personal_crit_dmg(data)` / `_calculate_personal_crit_damage(...)`、`CalculatorBuffAttributeReader.read_personal_crit_damage(context)`、`_CalculatorReadSnapshot`、focused full-vs-personal crit damage tests。
- 当前已知耦合：full crit damage 必须继续包含 static crit damage、dynamic crit damage、field crit damage、`aftershock_attack` label bonus、`received_crit_dmg_bonus` 和 `min(5, crit_dmg)` cap；personal crit damage 仍排除 received crit damage。`_CalculatorReadSnapshot` 保持 private static / dynamic / judge / enemy / level adapter，不新增 public full-crit-damage reader API。
- 本轮结果：bounded implementation PRD 已完成 helper-seam implementation、focused oracle / contrast confirmation、serial retained validation decision、registered-sample Conditional No-Go、reviewer invariant / rollback gate 与 final handoff。`Calculator.RegularMul.cal_crit_dmg(data)` now delegates to `_calculate_full_crit_damage(data.static, data.dynamic, data.judge_node)` while preserving the public method signature and current `SkillNode` assertion.
- 当前默认下一 PRD：Phase-3 same-phase candidate selection / bounded proposal PRD。不得自动生成第二个 full-crit-damage follow-up；future generation must reselect from the retained same-phase pool.
- 当前 verdict：Implemented / no-op verified at handoff。Registered main-loop sample 仍是 conditional No-Go，除非 future live semantic diff 且真实 registered direct-damage route 在 explicit stop tick 内证明 nonzero selected full-crit-damage relevance；不得创建 validation-only team、fake APL、fixture-only route 或 retained-vs-retained sample。
- 必须保留：full-vs-personal crit damage contrast、implemented full / personal crit rate helper seams、implemented personal crit damage helper seam、selected Stun implementation、copied-output constructors、old containers、legacy `buff_add()` / `KickOutBuff()`、dispatch/runtime ports、listener broadcast、dot runtime registration、retained formula snapshots 和 P2-A through P2-G guarded buckets。
- 同阶段候选池：registered behavior sample eligibility、remaining `Calculator.RegularMul` branches / retained-only sheer follow-up、future `Calculator.StunMul.get_stun_array()` follow-up if named evidence appears、P2-A through P2-G guarded maintenance、retained compatibility、blocker-only phase-1 reopen rules。
- 验证入口：current handoff exact commands include focused full-crit pytest `uv run pytest tests/simulator/test_buff_attribute_reader.py -q -k "crit_formula_families or full_crit_damage or reader_personal_crit_damage_anchor"` and scoped mypy over `Calculator.py` / focused tests with `Success: no issues found in 2 source files`；handoff 还记录 focused docs `git diff --check`、Ralph JSON sanity 和 UTF-8 / mojibake scan。后续 formula/read candidate 仍保留 `formula-parity` 与 conditional `calculator-reads`；触达 copied-output、event、dispatch、runtime 或 listener 边界时追加 `implicit-events`；触达 lifecycle container / runtime write path 时追加默认 profile；live semantic diff 且 route/count 合格时才追加 main-loop consistency。
- 非目标：不做 broad `Calculator.py` / `CalAnomaly.py` rewrite；不新增 public `CalculatorBuffAttributeReader.read_full_crit_damage(...)`；不扩展 `_CalculatorReadSnapshot`；不重开 personal crit damage、crit rate、damage bonus、defense/resistance/vulnerability、base damage、retained-only sheer、Stun array、copied-output、registered-team fixtures、validation-runner rewrite、old-container deletion 或 event/runtime/listener layer merge。

#### 候选块 Phase-3：RegularMul scalar helper-family bounded implementation PRD（已完成 / same-phase pool retained）

- 候选文件 / 符号：`zsim/sim_progress/ScheduledEvent/Calculator.py` 中的 `Calculator.RegularMul.cal_dmg_bonus(data)`、`cal_res_mul(...)`、`cal_dmg_vulnerability(...)`、`cal_stun_vulnerability(data)`、`cal_special_mul(data)`、`cal_defense_mul(data)`、`cal_recipient_def(...)`、`cal_pen_ratio(...)`、`cal_k_attacker(...)`、`cal_crit_expect(data)`；module-local helper anchors 为 `_calculate_damage_bonus(...)`、`_calculate_resistance_multiplier(...)`、`_calculate_damage_vulnerability(...)`、`_calculate_stun_vulnerability(...)`、`_calculate_special_multiplier(...)`、`_calculate_defense_multiplier(...)`、`_calculate_recipient_defense(...)`、`_calculate_pen_ratio(...)`、`_calculate_attacker_level_coefficient(...)`、`_calculate_crit_expectation(...)`。
- 当前已知耦合：这些 helper seam 只拆出同文件标量计算，不新增 public reader API，不扩展 private `_CalculatorReadSnapshot` 字段集，不改 registered route、validation-runner、event queue、dispatch/runtime ports、listener broadcast、dot runtime、same-tick runtime write、old containers 或 retained compatibility。`Calculator.RegularMul.cal_base_dmg(data)` / non-sheer `cal_base_attr(base_attr in 0..3, data)` 本轮为 No-Go，因为 direct non-sheer `base_attr=2/3` oracle 缺口仍未关闭；`base_attr=4` / `cal_sheer_dmg_bonus(data)` 仍是 retained-only sheer follow-up，不随本 batch 升级。
- 本轮结果：RegularMul scalar helper-family bounded implementation PRD 已完成。Implemented / no-op verified surfaces 为 damage bonus、resistance / vulnerability / special group、defense helper group 和 crit expectation helper；non-sheer base damage / base attr 为 documented No-Go；registered main-loop sample 为 Conditional No-Go，因为本 PRD 只有 behavior-preserving helper seams，没有 live production semantic diff，也没有真实 registered route 证明 nonzero selected formula/event relevance。
- 当前默认下一 PRD：Phase-3 `Calculator.RegularMul` non-sheer base damage / base-attribute bounded implementation batch，或在同一 PRD 内完成 No-Go closure。下一轮不得再从 retained pool 重新生成完整 candidate-selection / proposal-readiness cycle；应复用已完成 scalar helper-family evidence、当前 focused oracle gap 结果和 retained gates，把 selection / contract / sample / validation 判断压进 implementation preflight 与 validation / handoff stories。
- Same-phase candidate blocks retained：
  - registered behavior sample eligibility：仅在 future live production semantic diff 且真实 registered route、explicit stop tick、nonzero relevant formula/event counts、total damage、full event counts、Buff timeline 和 rollback anchors 都具备时运行 main-loop consistency；禁止 validation-only team、fake APL、fixture-only route 或 retained-vs-retained sample。
  - retained-only sheer follow-up：当前已完成 private helper seam 与 retained validation；只在 future source / validation / reviewer evidence 同时关闭 public reader/snapshot 与 real registered-route blockers 时重开，不得通过 public snapshot expansion 快捷升级。
  - future `Calculator.StunMul.get_stun_array()` follow-up：当前 implementation 已完成；只有 named regression、focused oracle gap、validation failure 或新的 proposal-readiness packet 才能重开。
  - P2-A through P2-G guarded maintenance：继续只由 guardrail、focused test、validation 或 reviewer-named concrete blocker 触发，不作为默认实现 backlog。
  - retained compatibility / blocker-only reopen：old containers、legacy Buff write paths、dispatch/runtime/listener/dot layers 和 retained formula snapshots 只能由 root-workspace source、guardrail、focused test、validation 或 real registered-route blocker evidence 命名后重开。
- 验证入口：current PRD retained gates 为 focused RegularMul pytest `28 passed`、scoped mypy `Success: no issues found in 9 source files`、serial `formula-parity` base `2 passed` / isolated `3 passed` / focused `154 passed` / mypy `9 source files` clean、serial `calculator-reads` base `2 passed` / isolated `3 passed` / focused `254 passed` / mypy `22 source files` clean。US-010 handoff 追加 scoped mypy、JSON sanity、UTF-8 / mojibake scan、focused `git diff --check` 和 checkpoint / reviewer evidence。
- 非目标：不做 broad `Calculator.py` / `CalAnomaly.py` rewrite；不新增 public `CalculatorBuffAttributeReader` damage/full-damage API；不扩展 `_CalculatorReadSnapshot`；不创建 registered fixtures；不重写 validation runner；不删除 old containers、retained compatibility、event/runtime/listener layer separation 或 P2 guarded buckets。
...
#### 候选块 Phase-3：`AnomalyBar.current_ndarray` / `UpdateAnomaly.py` write-path field matrix（已完成 / retained boundary）

- 候选文件 / 符号：`zsim/sim_progress/anomaly_bar/AnomalyBarClass.py` 中 `current_ndarray`、`anomaly_settled()`、`reset_current_info_cause_output()`、`reset_myself()`、`__deepcopy__()`；`zsim/sim_progress/Update/UpdateAnomaly.py` 中 `spawn_output(...)`、`update_anomaly(...)`、`anomaly_effect_active(...)`、`remove_dots_cause_disorder(...)`；copied-output anchors 为 `NewAnomaly`、`Disorder`、`PolarityDisorder` 与 handler report payload focused tests。
- 当前已知耦合：CodeGraph current-root evidence shows `spawn_output(mode 0)` settles the bar before `NewAnomaly`, modes 1 / 2 create disorder objects and synchronously broadcast `LBS.DISORDER_SPAWN`, `update_anomaly(...)` deep-copies `bar` into `active_bar`, publishes through `ScheduleDispatchPort`, mutates `enemy.dynamic.active_anomaly_bar_dict`, and resets the original bar at the end. `AnomalyBar.anomaly_settled()` writes `current_effective_anomaly`, divides into `current_ndarray`, and flips `settled`; these writes must remain separate from copied-output constructors, listener broadcast, scheduled publish, dot runtime registration, and same-tick runtime writes.
- 本轮结果：field matrix 已覆盖 mode 0 `NewAnomaly`, mode 1 `Disorder`, mode 2 `PolarityDisorder`, invalid mode / missing polarity ratio, active-bar deepcopy, reset behavior, current ndarray object / shape expectations, effective anomaly fields, `active_by` / `activated_by`, and no-publish / broadcast / scheduled-publish split. US-006 已记录同 PRD No-Go for additional production work；后续只在 focused regression、validation failure、root-workspace source evidence、guardrail 或 reviewer-named evidence 命名该 surface 时重开 blocker。
- 必须保留：`ScheduleDispatchPort` publish boundary, synchronous `listener_manager.broadcast_event(...)`, dot runtime / debuff side effects, `RuntimeCommandPort` same-tick write boundary, old containers, retained `MultiplierData` / `MulData` formula snapshots, copied-output payload constructors, handler report payload parity, and existing focused tests.
- 验证入口：已通过 focused `test_update_anomaly_dispatch.py`, `test_buff_attribute_reader.py` copied snapshot / CalAnomaly nodeids, `test_anomaly_handler_runtime_view.py`, scoped mypy over `UpdateAnomaly.py` / `AnomalyBarClass.py` / related formula/read anchors / focused tests, plus serial `formula-parity`, `implicit-events`, and `calculator-reads`. Future maintenance should rerun the touched focused subset first, then add `implicit-events` only for copied-output / event / dispatch / runtime / listener changes and `formula-parity` / `calculator-reads` only for formula/read surfaces.
- 非目标：不 rewrite `CalAnomaly.py` formulas, not broaden to `Calculator.py`, not create registered-team fixtures, not replace copied-output constructors or handler report payloads without a named blocker, not merge listener / scheduled publish / dot runtime / same-tick write layers, not delete old containers or retained compatibility.

#### 候选块 Phase-3：enemy dynamic / anomaly duration reads（已完成 / retained boundary）

- 候选文件 / 符号：`AnomalyBar.change_info_cause_active(...)`、`AnomalyBar.__get_max_duration(...)`、`AnomalyBar.__get_duration_enemy_buffs(...)`、`BuffRuntimeReadPort.get_active_buffs("enemy")` and legacy `dynamic_buff_dict["enemy"]` fallback；related evidence lives in `test_anomaly_handler_runtime_view.py`, `test_buff_attribute_reader.py`, `calculator-reads`, and old coupling review sections for anomaly duration reads.
- 当前已知耦合：current-root CodeGraph confirms `change_info_cause_active(...)` passes optional `buff_runtime_view` into duration calculation, and `__get_duration_enemy_buffs(...)` prefers runtime view before legacy `dynamic_buff_dict["enemy"]`. This PRD completed the missing runtime-view / legacy dynamic duration专项矩阵 and found no reason to change the known coupling classification.
- 本轮结果：runtime-view enemy buffs, legacy dynamic duration fallback, missing enemy list `TypeError`, no-buff empty tuple, no duration metadata, empty enemy sequence, inactive / unrelated / unknown-key Buffs, fixed deltas, percentage deltas, count-scaled effects, multi-Buff accumulation and negative clamp are characterized. The slice closes as retained-boundary evidence and No-Go for additional production work inside this PRD.
- 必须保留：runtime read-only boundary, old dynamic fallback compatibility, copied-output/event layers, formula snapshots, and validation-runner behavior.
- 验证入口：focused 13-nodeid duration matrix, scoped mypy over duration source/test anchors, serial `calculator-reads`, and serial `implicit-events` are the retained evidence. Future maintenance reruns the touched focused subset first and only adds broader gates if the touched surface demands them.
- 非目标：no public reader/snapshot expansion, no old-container deletion, no formula rewrite, no event queue or listener changes, no registered-route fabrication, and no retained compatibility deletion.
...
- Current RegularMul full-crit-damage bounded implementation US-008 后的 handoff decision：`Calculator.RegularMul.cal_crit_dmg(data)` 已完成 helper-seam implementation / no-op verification，`_calculate_full_crit_damage(...)` 保持 `aftershock_attack` label branch、`received_crit_dmg_bonus` inclusion、`min(5, crit_dmg)` cap、public signature 和 current `SkillNode` assumption；public full-crit-damage reader API、snapshot public expansion、full/personal crit bundling、retained-only sheer shortcut、registered-team fixture creation、validation-runner rewrite、old-container deletion、layer merge 和 retained compatibility deletion 均仍为 No-Go。下一默认 PRD 返回 Phase-3 same-phase candidate selection / bounded proposal；候选池继续保留 registered behavior sample eligibility、remaining `Calculator.RegularMul` branches / retained-only sheer follow-up、future `Calculator.StunMul.get_stun_array()` follow-up if named evidence appears、P2-A through P2-G guarded maintenance、retained compatibility 与 blocker-only reopen rules。当前 handoff 未发现新 Buff 耦合或既有耦合分类变化，因此不更新 [旧Buff系统耦合审查结果.md](./旧Buff系统耦合审查结果.md)。
- Current route reconciliation US-001 后的 handoff decision：current-root docs 与 controller state 均以 Phase-3 same-phase candidate selection / bounded proposal 为当前默认，不继续沿 copied-output、`cal_res_pen()`、AM/AP/impact、selected Stun array、full crit rate、personal crit rate、personal crit damage 或 full crit damage 生成实现 backlog；这些 surface 均是 completed / no-reopen evidence，除非 future focused regression、validation failure、root-workspace source evidence、guardrail 或 reviewer-named evidence 明确重开。当前结论排除 `.codex_worktrees/`、`scripts/ralph/archive/`、`scripts/ralph/run-logs/`、logs、generated history 与 historical worktree evidence。
