...
Phase-3 characterization / proposal-readiness continuation PRD：array-output / RegularMul oracle readiness 与 RegularMul sheer reader-snapshot readiness 均已完成，但 production proposal 仍为 No-Go。下一默认 PRD 仍应沿 [Buff重构方案.md](./Buff重构方案.md) 的阶段路线推进，继续从 phase-3 candidate pool 中选择一个 exact bounded characterization slice，而不是生成 production replacement。默认候选应优先补齐当前 No-Go 后仍缺的 proposal prerequisites：copied-output handler/report payload parity、registered-team behavior sample eligibility、`Calculator.RegularMul` remaining branches / retained-only sheer follow-up、或 `Calculator.StunMul.get_stun_array()` / array output follow-up 中的一个具体缺口；PRD 生成器必须先命名 focused pytest、scoped mypy targets、rollback anchors、registered behavior sample 条件、retained `formula-parity` / `calculator-reads` gates 和 non-goals。same-phase pool 必须继续保留 P2-A through P2-G guarded maintenance；触达 event/runtime 时追加 `implicit-events`，不得扩大为 broad `Calculator.py` / `CalAnomaly.py` rewrite 或 retained compatibility 删除。
...
#### 候选块 Phase-3：post-array proposal-readiness continuation（当前默认）

- 候选文件 / 符号：`CopyAnomalyForOutput.py`、`UpdateAnomaly.spawn_output(...)`、anomaly handler report payload、listener-facing copied-output fields、registered-route evidence packets、`Calculator.RegularMul` retained-only / remaining branch follow-ups、`Calculator.StunMul.get_stun_array()` follow-up gaps，以及 P2-A through P2-G guardrail files。
- 当前已知耦合：current array / RegularMul / sheer evidence is characterization-only；production proposal 缺 registered-route behavior sample 条件、reader-snapshot contract approval、remaining branch / copied-output gap closure 和 exact rollback plan；P2-A through P2-G 是 guarded maintenance，不是默认实现 backlog。
- Ralph-sized 工作方向：优先选择一个 exact bounded gap，写 focused pytest / scoped mypy / retained gates / rollback anchors / registered-sample 条件；如果候选触达 copied-output、event、dispatch、runtime 或 listener 分层，追加 `implicit-events` 并保持 listener broadcast、scheduled publish、dot runtime registration 和 same-tick runtime writes 分层。
- 必须保留：`Calculator.py` / `CalAnomaly.py` retained formula snapshots、`MultiplierData` / `MulData` / `DynamicStatement`、`AnomalyBar.current_ndarray`、old containers、legacy `buff_add()` / `KickOutBuff()`、`ScheduleDispatchPort`、`RuntimeCommandPort`、`LegacyRuntimeCommandAdapter`、`LegacyBuffRuntimeFacade` 与已完成 AM/AP/impact / cal_res_pen bounded implementations。
- 验证入口：docs / decision-only story 至少跑 JSON sanity、focused mypy 或 active-story typecheck gate、UTF-8 / mojibake scan；formula / reader changes 跑 `formula-parity` 和 `calculator-reads`；event/runtime/copied-output changes 加 `implicit-events`；live semantic diff with real registered route 加 main-loop consistency sample。
- 非目标：不从当前 No-Go 直接生成 production formula replacement，不创建 validation-only registered team，不把 `.codex_worktrees/` 命中当 current source blocker，不删除 retained compatibility。
...
- 当前默认 Phase-3 characterization / proposal-readiness continuation PRD 必跑：新增或触达的 focused pytest targets、`uv run python scripts/run_buff_refactor_validation.py --typecheck-profile formula-parity`，以及 retained gate `uv run python scripts/run_buff_refactor_validation.py --typecheck-profile calculator-reads`；docs / decision-only story 可记录跳过原因但必须至少跑 JSON sanity、focused typecheck 或 active-story verifier和 UTF-8 / mojibake scan。不得默认重开已完成的 `Calculator.AnomalyMul.cal_res_pen()` selector extraction、AM/AP/impact helper implementation、Stun / RegularMul array contract 或 selected RegularMul branch matrix，也不得把 characterization diff 扩大为 broad formula rewrite。
