...
Phase-3 bounded `Calculator.RegularMul.cal_crit_rate(data)` production implementation PRD（当前默认）：当前 RegularMul remaining-branch PRD 已选择 `Calculator.RegularMul.cal_crit_rate(data)`，关闭 over-100% full crit focused oracle gap，完成 registered-sample eligibility audit，产出 bounded proposal packet，并串行通过 retained gates。下一轮默认可以生成 one bounded implementation PRD，但 production diff 必须限于 `Calculator.RegularMul.cal_crit_rate(data)` / full-crit reader seam 所需的最小 behavior-preserving implementation；不得顺手实现 `cal_crit_expect()`、personal crit、crit damage、damage bonus、defense、resistance、vulnerability、base damage、retained-only sheer、array outputs、Stun follow-up、copied-output、CalAnomaly 或 validation-runner rewrite。注册样本仍是 conditional No-Go：只有 future live semantic diff 且真实 registered route 证明 nonzero full-crit formula relevance 与 nonzero `crit_rate_received_increase`，才可运行 main-loop consistency；禁止 validation-only team / fake APL / retained-vs-retained sample。未来 PRD generation 可以产出 Markdown PRD，但不得在同一步直接改 `scripts/ralph/prd.json`；Ralph JSON conversion 留给后续显式转换步骤。same-phase pool 继续保留 registered behavior sample eligibility、remaining `Calculator.RegularMul` branches / retained-only sheer follow-up、future `Calculator.StunMul.get_stun_array()` follow-up if new evidence names one、P2-A through P2-G guarded maintenance、retained compatibility 与 blocker-only reopen rules。
...
- Current RegularMul remaining-branch PRD 已完成 handoff：`Calculator.RegularMul.cal_crit_rate(data)` 是唯一 selected bounded implementation candidate；full crit 保留 `crit_rate_received_increase`，personal crit 保留排除 received crit 的 contrast boundary。当前默认 PRD 改为 bounded `Calculator.RegularMul.cal_crit_rate(data)` implementation PRD；registered behavior sample 仍为 conditional No-Go，直到真实 registered route 证明 nonzero selected formula relevance 与 nonzero received-crit 输入。remaining `Calculator.RegularMul` branches / retained-only sheer、future `Calculator.StunMul.get_stun_array()` follow-up with named evidence、P2-A through P2-G guarded maintenance、retained compatibility 和 blocker-only reopen rules 仍保留为后续候选。
...
#### 候选块 Phase-3：RegularMul selected crit-rate implementation + retained formula pool（current default / same-phase pool retained）

- 候选文件 / 符号：`zsim/sim_progress/ScheduledEvent/Calculator.py` 中的 `Calculator.RegularMul.cal_crit_rate(data)`；reader anchor 为 `CalculatorBuffAttributeReader.read_full_crit_rate(context)`；contrast anchors 为 `Calculator.RegularMul.cal_personal_crit_rate(data)` / `CalculatorBuffAttributeReader.read_personal_crit_rate(context)`；retained pool 仍包括 `Calculator.RegularMul` remaining branches、retained-only sheer、`Calculator.StunMul.get_stun_array()` / `Calculator.cal_stun()` follow-up with named evidence、copied-output retained boundaries 和 P2-A through P2-G guarded buckets。
- 当前已知耦合：full crit 必须继续包含 `crit_rate_received_increase`；personal crit 必须继续排除 received crit。当前 main-loop consistency 输出没有 formula-call count，registered route evidence 也尚未证明 nonzero `crit_rate_received_increase`，所以 live sample 仍是 conditional No-Go。
- 本轮结果：US-003 选择 exact candidate `Calculator.RegularMul.cal_crit_rate(data)`；US-004 关闭 over-one full crit focused oracle；US-005 记录 registered-sample conditional No-Go 与 `莱特火属性队` future seed；US-006 产出 bounded proposal packet；US-007 串行 retained gates 通过；US-008 同步 long-lived handoff docs 与 same-phase candidate pool。
- 当前 verdict：Conditional Go for one later bounded implementation PRD limited to `Calculator.RegularMul.cal_crit_rate(data)`。该 Go 不授权 broad `Calculator.py` / `CalAnomaly.py` rewrite，不授权 RegularMul branch bundling，不授权 retained-only sheer shortcut、registered-team fixture creation、validation-runner rewrite、old-container deletion、layer merge 或 retained compatibility deletion。
- 必须保留：`Calculator.RegularMul.cal_personal_crit_rate(data)` / personal reader contrast boundary、AM/AP/impact helper implementation、`Calculator.AnomalyMul.cal_res_pen()` selector extraction、selected Stun implementation、copied-output constructors、old containers、legacy `buff_add()` / `KickOutBuff()`、dispatch/runtime ports、listener broadcast、dot runtime registration 和 P2-A through P2-G guarded buckets。
- 同阶段候选池：
  ...
  - remaining `Calculator.RegularMul` branches / retained-only sheer follow-up：继续只按 exact branch、deterministic oracle、rollback anchors 和 registered-sample 条件重开；`cal_crit_rate(data)` implementation Go 不授权其他 branches。
...
- 验证入口：next implementation PRD 至少保留 selected crit focused pytest、`uv run python scripts/run_buff_refactor_validation.py --typecheck-profile formula-parity` 和 `uv run python scripts/run_buff_refactor_validation.py --typecheck-profile calculator-reads`。触达 copied-output、event、dispatch、runtime 或 listener 边界时追加 `implicit-events`；触达 lifecycle container / runtime write path 时追加默认 profile；live semantic diff 且 route/count 合格时才追加 main-loop consistency。
- 非目标：不做 broad `Calculator.py` / `CalAnomaly.py` rewrite；不删除 retained compatibility；不把 `cal_crit_expect()`、personal crit、crit damage、damage bonus、defense/resistance/vulnerability、base damage、retained-only sheer、Stun array 或 copied-output 合并进 selected crit-rate implementation PRD。
...
- 当前默认 bounded `Calculator.RegularMul.cal_crit_rate(data)` implementation PRD 必跑：targeted source/docs preflight、focused selected-crit pytest、`formula-parity`、`calculator-reads`、focused docs / JSON / UTF-8 checks、active story scoped typecheck gate，并在 implementation plan 中声明 rollback anchors、registered-sample conditional No-Go / Go 条件、stop conditions 和 retained same-phase pool。不得默认重开已完成的 copied-payload handler/report implementation、`Calculator.AnomalyMul.cal_res_pen()` selector extraction、AM/AP/impact helper implementation、Stun array implementation、remaining RegularMul branches、retained-only sheer No-Go 或 P2 guarded buckets，也不得把下一轮直接扩大为 broad formula rewrite。
