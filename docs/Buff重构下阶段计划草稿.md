...
Phase-3 copied-output handler/report bounded proposal package PRD：当前 copied-output PRD 已给出 conditional Go for later proposal package only，下一默认 PRD 应先把 proposal 写完整，而不是直接实现 production diff。默认 scope 限定为 `NewAnomaly`、`Disorder`、`PolarityDisorder`、`DirgeOfDestinyAnomaly`、`UpdateAnomaly.spawn_output(...)` 以及 anomaly / disorder / polarity / abloom handler report paths 的 bounded proposal contract；PRD 生成器必须先命名 exact touched files / symbols、focused pytest、scoped mypy targets、rollback anchors、registered behavior sample 条件、retained `formula-parity` / `calculator-reads` gates、conditional `implicit-events` gate 和 non-goals。same-phase pool 必须继续保留 registered behavior sample eligibility、`Calculator.RegularMul` remaining branches / retained-only sheer follow-up、`Calculator.StunMul.get_stun_array()` / array output follow-up、P2-A through P2-G guarded maintenance、retained compatibility 与 blocker-only reopen rules；触达 event/runtime 时追加 `implicit-events`，不得扩大为 broad `Calculator.py` / `CalAnomaly.py` rewrite、validation-runner rewrite、registered-team fixture creation 或 retained compatibility 删除。
...
- AM/AP/impact bounded implementation、array-output / RegularMul oracle readiness PRD 与 RegularMul sheer reader-snapshot readiness PRD 均已完成最终 handoff；后续不再把 `cal_am()`、`cal_ap()`、`cal_imp()` helper-family、已完成的 Stun / RegularMul array contract、selected branch matrix 或 retained-only sheer No-Go 当默认 implementation backlog。remaining same-phase pool 应从 copied-output handler/report payload parity、registered-team behavior sample eligibility、`Calculator.RegularMul` remaining branches / retained-only sheer follow-up、`Calculator.StunMul.get_stun_array()` follow-up 与 P2-A through P2-G guarded maintenance 中选择 exact bounded characterization slice。
- Copied-output constructor / listener / handler-report parity 已完成 characterization 和 proposal-readiness decision；剩余工作不是直接 production implementation，而是先把 bounded proposal package 写清楚。该 proposal 完成后，same-phase pool 仍应继续保留 registered behavior sample eligibility、RegularMul / retained-only sheer、Stun array、P2 guarded maintenance 和 retained compatibility 候选，不得只沿 copied-output 生成单一路径。
...
#### 候选块 Phase-3：array outputs / remaining formula oracle closure（已完成 / characterization-only）

- 候选文件 / 符号：`zsim/sim_progress/ScheduledEvent/Calculator.py` 中的 `Calculator.StunMul.get_stun_array()`、`Calculator.StunMul.cal_stun()` array output path、`Calculator.RegularMul` remaining branches（crit expectation、stun vulnerability、special / sheer multiplier、array outputs）以及对应 retained `MultiplierData` / `DynamicStatement` snapshots。
- 当前已知耦合：`get_stun_array()` 仍由 `cal_stun()` 使用，CodeGraph 导航显示它不应因 `cal_imp()` helper extraction 自动获得 production replacement 授权；`RegularMul` branch matrix 已锁定 direct damage、crit、defense/resistance、damage vulnerability、stun vulnerability、special multiplier 和 sheer damage bonus retained values；sheer conversion 还需要 runtime `char_instance.sheer_attack_conversion_rate`，当前 reader-built snapshot 不携带该对象。
- 本轮结果：US-002 至 US-004 已补 Stun array、RegularMul arrays 和 selected RegularMul branch characterization；array / RegularMul US-005 判定 production proposal No-Go；sheer reader-snapshot readiness US-003 / US-005 也判定 production proposal No-Go；US-006 串行验证通过；US-007 handoff 只同步 docs / Ralph evidence，不改 production / validation-runner / event-runtime source。
- 后续工作方向：不要把本块直接升级为 production replacement。若后续仍选择 formula surface，必须命名一个剩余 exact branch 或 follow-up gap，补 deterministic oracle、rollback anchors 和 registered behavior sample 条件后，再重新评估 proposal readiness。
- 必须保留：AM/AP/impact helper implementation、`Calculator.AnomalyMul.cal_res_pen()` selector extraction、`CalAnomaly.py`、copied-output constructors、old containers、legacy `buff_add()` / `KickOutBuff()`、dispatch/runtime ports、listener broadcast、dot runtime registration 和 P2-A through P2-G guarded buckets。
- 同阶段候选池：
  - copied-output handler/report payload parity：保留 `CopyAnomalyForOutput.py`、`UpdateAnomaly.spawn_output(...)`、anomaly handler report payload 和 listener-facing fields；触达 event/runtime 分层时追加 `implicit-events`。
  - registered-team behavior sample eligibility：只在 future production semantic diff 且真实注册队伍可触达目标 route 时运行 main-loop consistency；不创建 validation-only team，样本必须证明 nonzero relevant event count 才能采信。
  - P2-A through P2-G guarded maintenance：继续只由 guardrail / focused test / validation 的 concrete blocker 触发，不作为默认实现 backlog。
- 验证入口：最小 focused pytest 仍从 `tests/simulator/test_buff_attribute_reader.py` 增量扩展；默认 retained gates 为 `uv run python scripts/run_buff_refactor_validation.py --typecheck-profile formula-parity` 与 `uv run python scripts/run_buff_refactor_validation.py --typecheck-profile calculator-reads`。触达 copied-output、event、dispatch、runtime 或 listener 边界时追加 `implicit-events`；触达 lifecycle container / runtime write path 时追加默认 profile。
- 非目标：不做 broad `Calculator.py` / `CalAnomaly.py` rewrite；不删除 retained compatibility；不把 P2-A / P2-B reader migration guardrails、P2-D publish guardrail、P2-E dot runtime guardrail、P2-F forced-write guardrail 或 P2-G direct-context guardrail 重开成默认 backlog。

#### 候选块 Phase-3：copied-output handler/report bounded proposal package（当前默认）
...
- 当前已知耦合：constructor field matrix、listener broadcast vs scheduled publish separation、handler report payload parity、registered-sample eligibility 和 rollback anchors 已由当前 PRD characterization / decision docs 覆盖；US-007 只授权 later proposal package，不授权 immediate production implementation。copied-output payload、listener broadcast、scheduled queue publish、dot runtime registration 与 same-tick runtime writes 仍是分层 retained boundaries。
- Ralph-sized 工作方向：写 proposal-only PRD，列出 exact future production diff candidates、focused tests、scoped mypy targets、serial retained gates、registered-route sample conditions、rollback anchors and reviewer questions；如果 proposal 仍缺真实 registered route 或 nonzero copied-output / anomaly event count，则保持 implementation No-Go。proposal 完成后再由新 PRD 决定是否允许 one bounded production slice。
...
- 同阶段候选池：registered behavior sample eligibility、`Calculator.RegularMul` remaining branches / retained-only sheer follow-up、`Calculator.StunMul.get_stun_array()` / array output follow-up、P2-A through P2-G guarded maintenance、retained compatibility、blocker-only phase-1 reopen rules。若 copied-output proposal 完成，下一轮应从这些候选中重新选择 exact bounded slice，而不是只自动生成 copied-output implementation。
- 验证入口：proposal / docs-only story 至少跑 JSON sanity、focused typecheck 或 active-story typecheck gate、UTF-8 / mojibake scan；future formula / reader changes 跑 `formula-parity` 和 `calculator-reads`；event/runtime/copied-output source changes 加 `implicit-events`；live semantic diff with real registered route and nonzero relevant event count 加 main-loop consistency sample。
- 非目标：不直接实现 production diff，不创建 validation-only registered team，不把 `.codex_worktrees/` 命中当 current source blocker，不重写 validation runner，不删除 retained compatibility，不把 P2-A through P2-G guardrails 或 RegularMul / Stun array follow-ups合并到 copied-output proposal。
...
- 当前默认 Phase-3 copied-output handler/report bounded proposal PRD 必跑：新增或触达的 focused pytest targets、focused typecheck / scoped mypy target、JSON sanity 和 UTF-8 / mojibake scan；proposal 如触达 event/runtime/copied-output source changes，追加 `uv run python scripts/run_buff_refactor_validation.py --typecheck-profile implicit-events`。不得默认重开已完成的 `Calculator.AnomalyMul.cal_res_pen()` selector extraction、AM/AP/impact helper implementation、Stun / RegularMul array contract、selected RegularMul branch matrix 或 retained-only sheer No-Go，也不得把 proposal diff 扩大为 broad formula rewrite。
