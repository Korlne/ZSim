...
Phase-3 AM/AP/impact bounded production proposal PRD：`Calculator.AnomalyMul.cal_res_pen()` 已完成且不作为默认重开目标；AM/AP/impact oracle-gap closure 已把 `Calculator.AnomalyMul.cal_am()`、`Calculator.AnomalyMul.cal_ap()`、`Calculator.StunMul.cal_imp()` 和对应 reader paths 推到 proposal-ready。下一轮默认先写 proposal，不直接实现：选择 exact helper scope，列出 focused pytest、scoped mypy targets、rollback anchors、registered behavior sample 条件、retained `formula-parity` / `calculator-reads` gates、触达 event/runtime 时的 `implicit-events` gate，以及明确 non-goals。
...
#### 候选块 Phase-3：AM/AP/impact bounded production proposal（当前默认）

- 候选文件 / 符号：`zsim/sim_progress/ScheduledEvent/Calculator.py` 中的 `Calculator.AnomalyMul.cal_am()`, `Calculator.AnomalyMul.cal_ap()`, `Calculator.StunMul.cal_imp()`, `_calculate_anomaly_mastery(...)`, `_calculate_anomaly_proficiency(...)`, `CalculatorBuffAttributeReader.read_anomaly_mastery(...)`, `read_anomaly_proficiency(...)`, `read_impact(...)`；focused tests 入口为 `tests/simulator/test_buff_attribute_reader.py` 中 `test_cal_am_retained_multiplier_data_oracle_rows`, `test_cal_ap_retained_multiplier_data_oracle_rows`, `test_cal_imp_retained_multiplier_data_oracle_rows`, `test_calculator_am_ap_impact_formula_boundaries_remain_retained_compatibility`。
...
- 当前默认 Ralph-sized 工作方向：生成 bounded production proposal PRD，而不是立即实现。下一 PRD 第一批 story 必须选择 exact helper scope（单独 `cal_am` / `cal_ap` / `cal_imp`，或明确论证一个 AM/AP/impact helper set 为同一风险面），列出 focused pytest、scoped mypy targets、rollback anchors、registered behavior sample 条件、retained validation gates 和 non-goals；只有 proposal 自身通过后才进入实现 story。
- 后续同阶段候选块：
...
- 验证入口：AM/AP/impact proposal 必跑 relevant focused pytest targets、`uv run python scripts/run_buff_refactor_validation.py --typecheck-profile formula-parity`、`uv run python scripts/run_buff_refactor_validation.py --typecheck-profile calculator-reads`；触达事件 / runtime / dispatch boundary 时追加 `implicit-events`，触达 lifecycle container / runtime write path 时追加默认 profile。若 proposal 需要验证 runner 改动，先跑 `--help` 和 focused runner tests。
...
- 回滚计划：AM/AP/impact proposal 失败时回退对应 helper / profile / formula diff，继续走 retained `Calculator.AnomalyMul.cal_am()`, `Calculator.AnomalyMul.cal_ap()`, `Calculator.StunMul.cal_imp()`, `MultiplierData`, `DynamicStatement`, and `CalculatorBuffAttributeReader` compatibility evidence；不得用回滚删除 `CalAnomaly.py`、copied-output constructors、old containers 或 runtime compatibility paths。
- 非目标：不在 proposal story 中实现 broad `Calculator.py` / `CalAnomaly.py` rewrite；不删除 old containers、legacy `buff_add()` / `KickOutBuff()`、`RuntimeCommandPort`、`LegacyRuntimeCommandAdapter` 或 `LegacyBuffRuntimeFacade`；不把 CLI label 当 live runtime switch；不把 P2-A / P2-B reader migration guardrails 重开成默认 backlog。
...
- 当前默认 Phase-3 AM/AP/impact bounded production proposal PRD 必跑：新增或触达的 focused pytest targets、`uv run python scripts/run_buff_refactor_validation.py --typecheck-profile formula-parity`，以及 retained gate `uv run python scripts/run_buff_refactor_validation.py --typecheck-profile calculator-reads`；不得默认重开已完成的 `Calculator.AnomalyMul.cal_res_pen()` selector extraction，也不得在 proposal story 中直接实现 broad formula rewrite。
...
- US-026 后的 validation decision：`formula-parity`、`calculator-reads` 与 `implicit-events` serial gates 已通过，但当时 production replacement 仍 No-Go。US-013 已把 AM/AP/impact 更新为 bounded production proposal-ready；下一轮必须从本文件的 Phase-3 AM/AP/impact 候选块和 `docs/Buff公式候选与测试目标清单.md` 的 oracle/readiness 证据取材，命名 exact formula helper scope、focused tests、scoped mypy targets、registered behavior sample 条件、rollback plan、retained `formula-parity` / `calculator-reads` / conditional `implicit-events` gate 与 remaining blockers。
- Current US-013 后的 validation / handoff decision：AM/AP/impact final serial `formula-parity` 与 `calculator-reads` 均通过；下一轮默认生成 bounded production proposal PRD，不在 focused regression 或 validation failure 之外重开 `cal_res_pen()`，也不把 AM/AP/impact proposal-ready 外推成 whole-Calculator replacement approval。
