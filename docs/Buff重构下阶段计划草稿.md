...
Phase-3 下一默认 Ralph PRD 形状：implementation batch 或同一 PRD 内 Go / No-Go closure（当前默认 / anti-selection-loop）。当前 enemy dynamic / `AnomalyBar.__get_duration_enemy_buffs()` anomaly-duration read matrix 已完成 runtime-view precedence、legacy fallback、no-buff / nonmatching 和 duration-affecting variants 的 characterization、serial validation、No-Go decision 和 final handoff；下一默认 PRD 应围绕 remaining `Calculator.RegularMul` / retained-only sheer blocker package 做 bounded evidence review 或同 PRD Go / No-Go closure，先证明 `base_attr=4` / `cal_sheer_dmg_bonus(data)` 是否存在新的 reader-contract、registered-route 或 deterministic oracle evidence，而不是继续沿已完成 duration matrix 自动生成 follow-up。

下一轮 PRD generator 应从 `docs/Buff公式候选与测试目标清单.md`、本 checklist、旧耦合审查和当前 focused tests 中选择有 current-root source / oracle / retained-gate 证据的同族实现块；当前默认是 remaining `Calculator.RegularMul` / retained-only sheer blocker package。若该默认只够 No-Go，则在同一 PRD 内写 blocker / No-Go closure 并转向同阶段 sibling block。不得把 registered behavior sample eligibility、future `Calculator.StunMul.get_stun_array()` follow-up、P2 guarded maintenance、retained compatibility 或 blocker-only phase-1 reopen pool 提升为 active default，除非 focused regression、validation failure、root-workspace source evidence、guardrail 或 reviewer-named evidence 明确命名具体 blocker。
...
#### 候选块 Phase-3：RegularMul scalar helper-family bounded implementation PRD（当前默认 / batch implementation / same-phase pool retained）
...
- 当前默认下一 PRD：生成一个 8-12 story 的 batch implementation PRD，最多一个 baseline/preflight story，随后按同类公式组批量实现 / 测试：damage bonus + vulnerability/resistance/special scalar group、defense helper group（如 preflight 通过）、crit expectation group（如 preflight 通过）、non-sheer base damage/base attr group（如 preflight 通过），最后做 serial validation 与 handoff docs。不得再生成单独的 same-phase candidate selection / proposal-readiness PRD；只有 shared oracle / validation / invariant 失败时才整体退回 No-Go。
...
- Same-phase candidate blocks retained：
  ...
  - remaining `Calculator.RegularMul` / retained-only sheer follow-up：只按 exact branch、direct deterministic oracle、rollback anchors 和 snapshot/public-contract边界重开；`cal_base_attr(..., base_attr=4)` / `cal_sheer_dmg_bonus(data)` 仍不得通过 public snapshot expansion 快捷升级。
...
- 当前默认 PRD 改为 Phase-3 remaining `Calculator.RegularMul` / retained-only sheer blocker package：最新 anomaly-duration runtime-view matrix 已完成并验证，下一轮不得再生成 generic same-phase candidate selection / bounded proposal，也不得继续沿已完成 write-path field matrix 或 duration matrix 自动生成 follow-up。PRD 应复用 current-root CodeGraph / focused test / retained gate evidence，围绕 `base_attr=4`、`cal_sheer_dmg_bonus(data)`、reader-contract eligibility、真实 registered-route evidence 和 deterministic oracle gap 做 bounded evidence review 或同 PRD Go / No-Go closure；若需要 public reader-contract expansion、old-container deletion、validation-runner rewrite、event/listener changes、registered-route fabrication 或 retained compatibility deletion，应在同一 PRD 内 No-Go / stop-and-split。不得默认重开已完成的 copied-payload handler/report implementation、`Calculator.AnomalyMul.cal_res_pen()` selector extraction、AM/AP/impact helper implementation、selected Stun implementation、completed RegularMul helper implementations、CalAnomaly helper-family batch、current write-path field matrix、current duration matrix 或 P2 guarded buckets，也不得把下一轮扩大为 broad formula rewrite。
