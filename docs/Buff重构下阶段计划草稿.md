...
Phase-3 下一默认 Ralph PRD 形状：same-phase closure / exact bounded candidate intake（当前默认 / anti-selection-loop）。当前 retained-only sheer blocker closure 已完成 helper seam、oracle / No-Go evidence、serial retained validation 和 final handoff；下一默认 PRD 不再继续沿 retained-only sheer、duration matrix、write-path matrix、non-sheer base、RegularMul crit/helper seams 或其他已完成微表面自动生成实现 follow-up。下一 PRD 应先做 current-root source / docs / focused-test preflight，排除 completed / no-reopen surfaces，然后从下方 same-phase pool 中选择一个由 root-workspace source、focused regression、guardrail、validation failure 或 reviewer-named evidence 明确命名的 exact bounded candidate；若没有同阶段候选具备证据，应在同一 PRD 内记录 Phase-3 closure / No-Go 并给出进入下一阶段的证据。

下一轮 PRD generator 应从 `docs/Buff公式候选与测试目标清单.md`、本 checklist、旧耦合审查和当前 focused tests 中选择有 current-root source / oracle / retained-gate 证据的同族实现块；当前默认是 Phase-3 same-phase candidate-pool closure / exact bounded candidate selection，不是某个单一已完成 formula branch 的 follow-up。不得把 registered behavior sample eligibility、future `Calculator.StunMul.get_stun_array()` follow-up、P2 guarded maintenance、retained compatibility 或 blocker-only phase-1 reopen pool 提升为 active default，除非 focused regression、validation failure、root-workspace source evidence、guardrail 或 reviewer-named evidence 明确命名具体 blocker。
