...
Phase-3 RegularMul personal crit damage bounded implementation PRD（当前默认）：当前 proposal-readiness PRD 已给出 Conditional Go，但只允许 later implementation PRD 处理 `Calculator.RegularMul.cal_personal_crit_dmg(data)` / `CalculatorBuffAttributeReader.read_personal_crit_damage(context)` 的等价 helper seam；不得把 full crit damage、`cal_crit_rate(data)`、`cal_crit_expect()`、damage bonus / defense / resistance / vulnerability / base damage、retained-only sheer、array output、CalAnomaly、copied-output、old-container、validation-runner 或 event/runtime/listener layer 合并进该默认 PRD。same-phase pool 必须继续保留 registered behavior sample eligibility、remaining `Calculator.RegularMul` branches / retained-only sheer follow-up、future `Calculator.StunMul.get_stun_array()` follow-up if new evidence names one、P2-A through P2-G guarded maintenance、retained compatibility 与 blocker-only reopen rules。
...
#### 候选块 Phase-3：RegularMul selected personal crit damage proposal + retained formula pool（proposal-ready / same-phase pool retained）
...
- 本轮结果：US-003 选择 exact candidate `Calculator.RegularMul.cal_personal_crit_dmg(data)`，US-004 关闭 focused oracle gap，US-005 固化 proposal boundary / rollback anchors，US-006 串行通过 focused reader pytest、`formula-parity` 和 `calculator-reads`，US-007 reviewer / invariant gate 给出 PASS 与 Conditional Go，US-008 handoff sync 把该结论写入 long-lived docs。
- 当前默认下一 PRD：later bounded implementation PRD 只能为 `cal_personal_crit_dmg(data)` 引入等价 module-local scalar helper seam（若实现确实需要），并保留 public formula expression、reader anchor、focused crit-family tests、`formula-parity` / `calculator-reads` gates 和 rollback anchors。
- 当前 verdict：Conditional Go for one later bounded implementation PRD only。Registered main-loop sample 仍是 conditional No-Go，除非 future live semantic diff 且真实 registered direct crit route 在 explicit stop tick 内证明 nonzero selected-branch relevance；不得创建 validation-only team、fake APL、fixture-only route 或 retained-vs-retained sample。
