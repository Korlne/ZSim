...
- 2026-06-18 route override result：当前默认下一 PRD 是 broad same-family `BuffXLogic` owner-family state-sync / read-gate bounded slice；必须先命名 exact owner family、files / symbols、focused state-sync / read-gate pytest、rollback anchors、`calculator-reads` / conditional `formula-parity` / conditional `implicit-events` gates 和 non-goals。不得继续生成 Phase 5 validation PRD，也不得把 Phase 5 sample 当作全面替换完成证明。
...
Phase 1-4 当前默认路由：`BuffXLogic` owner-family state-sync / read-gate bounded proposal and implementation。

- 决策基线：Phase 5 暂停；US-008 decision matrix 已覆盖 old containers、lifecycle entrypoints、`BuffXLogic`、`JudgeTools`、unified event-driven trigger chain、formula / effect-system replacement、old residual deletion 和 Phase 5 scope correction。下一轮默认 PRD 不再重复 gap-closure matrix，而是选择一个 broad same-family `BuffXLogic` owner family，围绕 read-only gates 与 read-then-writeback state-sync 形成 bounded implementation slice。
- 默认候选形状：从 US-005 的 `BuffXLogic` owner-family evidence 中选择一个共享 old template identity、`history.record`、`check_preparation(...)`、attribute/read inputs 和 `update_to_buff_0(...)` ordering 的 coherent family；允许同 PRD 内包含 source audit、focused characterization, implementation, serial validation 和 final handoff，但不得把 scheduled publish、runtime immediate write、formula semantic rewrite 或 old-container deletion 混入同一实现故事。
- 同阶段候选池仍保留：old containers / lifecycle compatibility、`JudgeTools` helper responsibility slices、planned-event producer / handler-requeue owner families、exact formula-domain oracle PRDs、registered behavior/performance gates、completed guarded-maintenance surfaces、retained compatibility 和 blocker-only reopen rules。候选池是 reviewer context，不是自动 backlog；若默认 `BuffXLogic` owner-family preflight 失败，应转到下一个 exact candidate 或 stop for human review，而不是生成 generic investigation loop。
