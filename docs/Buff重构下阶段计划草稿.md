...
- 2026-06-18 impact / crit / stun owner-family state-sync/read-gate PRD 已完成：selected family `LighterAdditionalAbility_IceFireBonus.py`、`QingYiAdditionalAbilityStunConvertToATK.py`、`TriggerAdditionalAbilityStunBonus.py`、`Soldier0AnbyCoreSkillCritDMGBonus.py` 已有 source preflight、focused characterization、state-sync/read-gate guardrails、serial `calculator-reads` / `implicit-events` verifier evidence 和 final handoff；current-root source did not require production edits. 当前默认下一 PRD 仍应使用 broad same-phase pool 选择下一个 exact owner/layer family，不得继续沿这四个 selected files 生成 narrow follow-up，也不得恢复 Phase 5 validation PRD。
...
Phase 1-4 当前默认路由：next exact `BuffXLogic` owner/layer-family bounded implementation or Go / No-Go closure, selected from the preserved same-phase pool after the completed impact / crit / stun family.

- 决策基线：Phase 5 暂停；US-008 decision matrix 已覆盖 old containers、lifecycle entrypoints、`BuffXLogic`、`JudgeTools`、unified event-driven trigger chain、formula / effect-system replacement、old residual deletion 和 Phase 5 scope correction。本 PRD 已把 selected impact / crit / stun owner-family 收口为 completed / guarded-maintenance evidence；下一轮默认 PRD 不再重复 gap-closure matrix，也不继续跟随 `LighterAdditionalAbility_IceFireBonus.py`、`QingYiAdditionalAbilityStunConvertToATK.py`、`TriggerAdditionalAbilityStunBonus.py`、`Soldier0AnbyCoreSkillCritDMGBonus.py`。
- 当前默认下一 PRD：从同阶段池中选择下一个 exact owner/layer family，优先形状是 implementation batch 或同 PRD Go / No-Go closure；候选必须命名 files / symbols / behavior, focused verifier, rollback anchors, retained boundaries and non-goals. If no exact candidate can be named from current-root source, focused tests, guardrails, validation failure or reviewer evidence, stop for human route review instead of generating a generic selection loop.
- Same-phase candidate pool for future generation remains broad and explicit:
  - old containers / lifecycle compatibility：`exist_buff_dict`、`DYNAMIC_BUFF_DICT`、`LOADING_BUFF_DICT`、pending activation、active removal、legacy `buff_add()` / `KickOutBuff()` and old template identity remain retained boundaries; deletion requires exact unused or behavior-preserving proof.
  - `JudgeTools` helper responsibility slices：template identity lookup, lazy record initialization, trigger-state read helpers, preload/action context, enemy context and tick / RNG-adjacent reads must stay owner-family scoped; do not delete helper responsibilities as a side effect of reader/state-sync work.
  - planned-event producer / handler-requeue owner families：future producer work must preserve `ScheduleDispatchPort` publish semantics, not-yet-executable handler requeue, listener broadcast, dot runtime registration, local event groups, runtime reads and same-tick writes as separate layers.
  - exact formula-domain oracle PRDs：remaining formula/effect candidates must name formula method, oracle rows, scoped mypy targets, `formula-parity` / `calculator-reads` gates and rollback anchors; selected impact / crit / stun owner-family validation does not authorize formula semantic rewrites.
  - registered behavior / performance gates：run main-loop consistency or runtime benchmarks only for live semantic or performance-sensitive diffs with real registered route/APL, explicit stop tick, nonzero relevant counts and rollback anchor; no validation-only teams or fake routes.
  - completed guarded-maintenance surfaces：P2-A through P2-G, AM/AP/impact scalar helpers, selected Stun, RegularMul helper families, CalAnomaly helper family, copied-output handler/report and current selected impact / crit / stun family stay no-reopen unless new evidence names a concrete blocker.
  - retained compatibility and blocker-only reopen rules：retained compatibility paths are reviewer context, not automatic backlog; old coupling review changes only when a new coupling or changed classification is discovered.
- 验证入口：docs/control-plane handoff 继续使用 `git diff --check -- docs scripts/ralph tasks`、Ralph JSON sanity、UTF-8 / U+FFFD / mojibake scans 和 scoped Ralph tooling typecheck。`BuffXLogic` source work must add focused state-sync / read-gate pytest; add `calculator-reads` when attribute readers are touched, `formula-parity` only for formula output changes, and `implicit-events` for event-adjacent branches. Run profiles serially.
