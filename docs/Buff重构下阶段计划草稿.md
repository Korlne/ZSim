...
Phase-3 same-phase candidate selection / bounded proposal PRD（当前默认）：当前 RegularMul crit-rate implementation 已完成，`Calculator.RegularMul.cal_crit_rate(data)` 不再是默认实现 backlog，除非 focused regression、validation failure 或 root-workspace source evidence 命名该 exact surface。下一轮默认应从 same-phase pool 重新选择一个 exact bounded candidate，并先记录 scope、rollback anchors、focused tests、scoped mypy、registered-sample 条件、retained gates、non-goals 和 reviewer questions；不得在同一步直接改 `scripts/ralph/prd.json`，Ralph JSON conversion 留给后续显式转换步骤。same-phase pool 必须继续保留 registered behavior sample eligibility、remaining `Calculator.RegularMul` branches / retained-only sheer follow-up、future `Calculator.StunMul.get_stun_array()` follow-up if new evidence names one、P2-A through P2-G guarded maintenance、retained compatibility 与 blocker-only reopen rules。
...
#### 候选块 Phase-3：RegularMul selected crit-rate implementation + retained formula pool（implemented / same-phase pool retained）
...
- 本轮结果：previous proposal PRD 选择 exact candidate `Calculator.RegularMul.cal_crit_rate(data)` 并完成 handoff；current implementation PRD 已把 `cal_crit_rate(data)` 收敛到 behavior-preserving `_calculate_full_crit_rate(...)` helper seam，并在 US-007 handoff 中标记 implemented / no-op verified。
- 当前 verdict：Implemented / no-op verified at handoff。`cal_crit_rate(data)` 不再作为下一默认 implementation backlog；只有 focused regression、validation failure、root-workspace source evidence 或 reviewer-named follow-up 才能重开。该结论不授权 broad `Calculator.py` / `CalAnomaly.py` rewrite，不授权 RegularMul branch bundling，不授权 retained-only sheer shortcut、registered-team fixture creation、validation-runner rewrite、old-container deletion、layer merge 或 retained compatibility deletion。
