...
Phase-3 `Calculator.RegularMul.cal_dmg_bonus(data)` bounded implementation PRD（当前默认）：本轮 same-phase candidate selection PRD 的 US-010 proposal packet 已给出 Conditional Go / No-Go，目标只限 `Calculator.RegularMul.cal_dmg_bonus(data)` 的 `regular-dmg-bonus-character-field-stack`（fire / normal attack / `aftershock_attack` / all-damage）等价 helper seam；后续实现 PRD 必须先复用 focused oracle nodeids、scoped mypy、rollback anchors、retained `formula-parity` / `calculator-reads` gates 与 stop conditions。Registered behavior sample 仍为 Conditional No-Go，只有 future live production semantic diff 且真实 registered route 在 explicit stop tick 内证明 nonzero selected `cal_dmg_bonus()` relevance 时才运行 main-loop consistency；不得创建 validation-only team、fake APL、fixture-only route 或 retained-vs-retained sample。当前默认候选池继续保留 registered behavior sample eligibility、remaining `Calculator.RegularMul` branches / retained-only sheer follow-up、future `Calculator.StunMul.get_stun_array()` follow-up if named evidence appears、P2-A through P2-G guarded maintenance、retained compatibility 与 blocker-only reopen rules；不得把后续 PRD 生成折叠成 only-`cal_dmg_bonus()` automation，也不得重开 completed copied-output、`cal_res_pen()`、AM/AP/impact、selected Stun、full crit rate、personal crit rate、personal crit damage 或 full crit damage surfaces，除非 focused regression、validation failure、root-workspace source evidence、guardrail 或 reviewer-named evidence 明确命名。

US-012 final evidence packaging 已确认：当前 Markdown PRD 仍是后续显式 Ralph conversion 的输入，不在本 story 内直接改写 / 生成 `scripts/ralph/prd.json`；下一 likely PRD 类型是 bounded implementation。若后续 reviewer 或 validation evidence 推翻 `cal_dmg_bonus()` 条件 Go，则退回 same-phase candidate pool 中的 proposal-readiness、guarded maintenance 或 No-Go/blocker closure 分支，而不是在本轮扩大实现范围。
...
#### 候选块 Phase-3：RegularMul cal_dmg_bonus bounded implementation PRD（当前默认 / same-phase pool retained）

- 候选文件 / 符号：`zsim/sim_progress/ScheduledEvent/Calculator.py` 中的 `Calculator.RegularMul.cal_dmg_bonus(data)`，可选 module-local behavior-preserving helper；focused oracle anchor 为 `tests/simulator/test_buff_attribute_reader.py::test_formula_oracle_table_cases_drive_expected_fields_and_reader_parity[regular-dmg-bonus-character-field-stack]`，supporting branch-matrix anchor 为 `test_calculator_regular_mul_branch_matrix_characterizes_selected_methods[fire-stunned-reader-snapshot]`。
- 当前 proposal outcome：Conditional Go / No-Go for one later bounded implementation PRD only。允许的 diff 只可保持 fire / normal attack / `aftershock_attack` / all-damage stack 的等价语义，并保留 public method signature、current `SkillNode` assumption、`element_type` dispatch、`trigger_buff_level` dispatch、label branch、all-damage addition、focused oracle rows 与 rollback anchors。
- 当前默认下一 PRD：生成 one bounded implementation PRD for exactly `Calculator.RegularMul.cal_dmg_bonus(data)`；若实现需要 all-element cross-product migration、RegularMul bundling、public reader/snapshot expansion、registered fixture creation、validation-runner rewrite、old-container deletion、layer merge 或 retained compatibility cleanup，则 stop-and-split / No-Go。
- Retained gates：先跑 selected focused pytest nodeids 与 scoped mypy；formula / reader work 继续串行保留 `formula-parity` 与 `calculator-reads`；只有触达 copied-output、event、dispatch/runtime、listener、dot-runtime 或 same-tick runtime write surface 时追加 `implicit-events`；触达 lifecycle container / runtime write path 时追加默认 profile。
...
- 非目标：不做 broad `Calculator.py` / `CalAnomaly.py` rewrite；不新增 public `CalculatorBuffAttributeReader` damage-bonus reader；不扩展 `_CalculatorReadSnapshot`；不重开 completed copied-output / `cal_res_pen()` / AM/AP/impact / selected Stun / crit helper seams；不创建 validation-only registered team；不删除 old containers、legacy `buff_add()` / `KickOutBuff()` 或 retained compatibility。
...
- 当前默认 PRD 改为 Phase-3 same-phase candidate selection / bounded proposal：最新 full-crit-damage implementation 已完成并验证，下一轮应从 retained same-phase pool 中选择一个 exact bounded surface，先记录 source audit、deterministic oracle / gap review、registered-sample 条件、rollback anchors、focused tests、scoped mypy、retained `formula-parity` / conditional `calculator-reads` / conditional `implicit-events` gate、reviewer verdict 与 handoff docs，再决定 Go / No-Go 或 bounded implementation。不得默认重开已完成的 copied-payload handler/report implementation、`Calculator.AnomalyMul.cal_res_pen()` selector extraction、AM/AP/impact helper implementation、selected Stun implementation、current `Calculator.RegularMul.cal_crit_rate(data)` implementation、current `Calculator.RegularMul.cal_personal_crit_rate(data)` implementation、current `Calculator.RegularMul.cal_personal_crit_dmg(data)` implementation、current `Calculator.RegularMul.cal_crit_dmg(data)` implementation、retained-only sheer No-Go 或 P2 guarded buckets，也不得把下一轮扩大为 broad formula rewrite。
