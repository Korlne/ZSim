...
| Immediate phase-2 default | Trigger-state read-only gate package | `AstralVoice.py`, `FlamemakerShakerApBonus.py`, `CordisGerminaSNAAndQIgnoreDefense.py`, `SpectralGazeImpactBonus.py`, `SharpenedStingerAnomalyBuildupBonus.py`, files with `trigger_buff_0=` | Old template Buff state reads through `exist_buff_dict` / `history.record`; mostly no direct writeback. | Build read-only trigger-state access design and tests for pure gates before touching read-then-writeback variants. | `BuffRuntimeReadPort` remains read-only; old template identity retained. | `implicit-events` plus focused no-write gate tests. | No write API on `BuffRuntimeReadPort`, no old container deletion. |
...
- Next pool state: P2-B crit / impact reader family has completed after the follow-up PRD. P2-C trigger-state gates are now the default same-phase PRD candidate; P2-D scheduled publish ordering, P2-E dot runtime-state, P2-F BuffAddStrategy facade-write design, phase-3 formula snapshot replacement, retained compatibility, and blocker-only phase-1 reopen rows remain available.
...
- Next pool state: P2-C trigger-state read-only gates are now the default same-phase PRD candidate. P2-D scheduled publish ordering, P2-E dot runtime-state, P2-F BuffAddStrategy facade-write design, phase-3 formula snapshot replacement, retained compatibility, and blocker-only phase-1 reopen rows remain available.

### US-008 结论

- 第一推荐 follow-up 已从完成的 P2-B 转为 P2-C trigger-state read-only gates；它应覆盖 old template Buff state、`trigger_buff_0=`、`history.record` lazy init、active / inactive no-write gates 和 `BuffRuntimeReadPort` 只读边界，而不是单文件薄 PRD。
