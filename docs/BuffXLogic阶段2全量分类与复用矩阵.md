...
- Next pool state: P2-B crit / impact reader family, P2-C trigger-state gates and P2-D scheduled publish ordering / adapter parity have completed after follow-up PRDs. P2-E dot runtime-state / initialization is now the default same-phase PRD candidate; P2-F BuffAddStrategy facade-write design, direct simulator context extraction, phase-3 formula snapshot replacement, retained compatibility, and blocker-only phase-1 reopen rows remain available.
...
- Next pool state: P2-C trigger-state read-only gates and P2-D scheduled publish ordering / adapter parity have completed after follow-up PRDs. P2-E dot runtime-state / initialization is now the default same-phase PRD candidate. P2-F BuffAddStrategy facade-write design, direct simulator context extraction, phase-3 formula snapshot replacement, retained compatibility, and blocker-only phase-1 reopen rows remain available.
...
- Next pool state: P2-D scheduled publish ordering / adapter parity has completed after the follow-up PRD. P2-E dot runtime-state / initialization is now the default same-phase PRD candidate. P2-F BuffAddStrategy facade-write design, direct simulator context extraction, phase-3 formula snapshot replacement, retained compatibility, and blocker-only phase-1 reopen rows remain available.
...
- Next pool state: P2-E dot runtime-state / initialization is now the default same-phase PRD candidate. P2-F BuffAddStrategy facade-write design, direct simulator context extraction, phase-3 formula snapshot replacement, retained compatibility and blocker-only phase-1 reopen rows remain available.

### US-008 结论

- 第一推荐 follow-up 已从完成的 P2-D 转为 P2-E dot runtime-state / initialization；它应覆盖 dot duration、registration、replacement、removal、scheduled follow-up 分层和 no-op branches，而不是把 dot runtime registration 误归入 planned-event backlog 或继续围绕已完成 P2-D 做单文件薄 PRD。
- Immediate phase-2 follow-ups should group by dot runtime-state / initialization semantics, helper family, direct context service or validation profile. Later phase-2 work should handle direct simulator services and facade-backed forced writes after focused tests exist.
