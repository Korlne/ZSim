...
| Default next PRD / phase-3 design only | Formula parity suite design / characterization | `Calculator.py`, `CalAnomaly.py`, `AnomalyBarClass.py`, `CopyAnomalyForOutput.py`, `UpdateAnomaly.py`, Vivian / Yanagi / Alice copied-output paths, enemy anomaly-state read helpers, and existing reader seam tests. | Formula internals, settled anomaly snapshots, copied-output ratios, and enemy-state gates define or gate damage behavior; current reader seams are evidence, not replacement permission. | Name formula parity contract, focused pytest targets, scoped mypy targets, behavior-sample conditions, rollback plan, and non-goals before any production formula change. | `MultiplierData`, `MulData`, `DynamicStatement`, `AnomalyBar.current_ndarray`, Calculator / CalAnomaly formulas, old containers, dispatch/runtime/listener boundaries retained. | Start from `calculator-reads`, existing formula characterization tests, and `implicit-events` when event/runtime layers are touched; do not wire a new profile until tests and mypy targets are named. | No production formula rewrite, no old-container deletion, no dispatch/runtime/listener merge. |
...
### 2026-06-09 US-009 Copied Anomaly / Disorder Output Formula-Read Classification

本节只分类 copied anomaly / disorder output 相关读取与发布边界，不改 `VivianCorePassiveTrigger.py`、`VivianCinema6Trigger.py`、`YanagiPolarityDisorderTrigger.py`、`AlicePolarizedAssaultTrigger.py`、`UpdateAnomaly.py`、`CopyAnomalyForOutput.py`、handler、dispatch adapter、listener、dot runtime 或 runtime command wiring。

#### Source / navigation evidence

- `rg` preflight covered the four named BuffXLogic trigger files, `CopyAnomalyForOutput.py`, `UpdateAnomaly.py`, dispatch tests, this matrix, replacement notes, and Ralph artifacts for `DirgeOfDestinyAnomaly`、`PolarityDisorder`、`PolarizedAssault`、copied anomaly、`ScheduleDispatchPort`、公式快照、异常输出、紊乱 terms.
- CodeGraph pinned root `UpdateAnomaly.spawn_output(...)` / `_publish_scheduled_event(...)`, root `YanagiPolarityDisorderTrigger.special_effect_logic(...)`, root `AlicePolarizedAssaultTrigger.special_effect_logic(...)`, and the Vivian copied-output special-effect bodies; `.codex_worktrees/` duplicate hits remain historical navigation evidence only.
- Validation wiring already carries `tests/simulator/test_alice_polarized_assault_trigger_dispatch.py`, `tests/simulator/test_yanagi_polarity_disorder_dispatch.py`, `tests/simulator/test_vivian_core_passive_trigger_dispatch.py`, `tests/simulator/test_vivian_cinema6_trigger_dispatch.py`, `test_migrated_p2d_scheduled_publish_guardrail.py`, and anomaly handler/runtime-view coverage under `implicit-events`.

| path / output family | copied formula or payload data | scheduled publish boundary | listener / runtime boundary classification | validation / non-goal |
| --- | --- | --- | --- | --- |
| `VivianCorePassiveTrigger.py` -> `DirgeOfDestinyAnomaly` | Copies active `AnomalyBar` with `AnomalyBar.create_new_from_existing(...)`, settles if needed, builds `MultiplierData` through `Mul(...)`, reads AP via `Calculator.AnomalyMul.cal_ap(...)`, and writes `anomaly_dmg_ratio = ap / 10 * element_ratio * cinema_ratio`. | `_create_dispatch_port().publish_scheduled(dirge_of_destiny_anomaly)` remains the only planned-payload publish in the effect path and is created on demand from the current `sim_instance`. | No listener broadcast or dot runtime registration is introduced here; formula data remains a retained Calculator/AP snapshot, not a dispatch adapter responsibility. | Keep `Calculator.py`, `CopyAnomalyForOutput.py`, copied payload construction, and publish ordering unchanged. Validate with Vivian core dispatch focused test plus `implicit-events`; no formula rewrite in this story. |
| `VivianCinema6Trigger.py` -> `DirgeOfDestinyAnomaly` | Same copied anomaly/AP formula family as core passive, with `math.floor(ap / 10) * element_ratio * cinema_ratio * c6_ratio`; feather/resource mutations occur before and after the copied-output branch per current source. | `_create_dispatch_port().publish_scheduled(dirge_of_destiny_anomaly)` stays after copied payload construction and before the report branch / final guard-feather reset. | Report `change_process_state()` remains debug/report state, not listener broadcast, runtime write, or planned-payload construction. | Preserve existing file-specific order and dispatch test evidence; future phase-3 parity must decide ratio/floor behavior before changing formulas. |
| `YanagiPolarityDisorderTrigger.py` -> `PolarityDisorder` | Computes `final_ratio` from retained record state (`polarity_disorder_basic_dmg_ratio` plus `e_counter`), deep-copies the active anomaly bar, settles it, and calls `spawn_output(..., mode_number=2, polarity_ratio=final_ratio, skill_node=..., sim_instance=...)`. | The returned `PolarityDisorder` is published through `_create_dispatch_port().publish_scheduled(...)`; the record counter and update signal reset after publish. | `spawn_output(..., mode_number=2)` constructs `PolarityDisorder` and synchronously broadcasts `LBS.DISORDER_SPAWN` before returning. That listener broadcast is inside output construction, not the scheduled queue boundary. | Keep listener broadcast and publish as separate layers; validate with Yanagi dispatch focused test and `implicit-events`. Do not move `spawn_output()` or broadcast in this readiness story. |
| `AlicePolarizedAssaultTrigger.py` -> `PolarizedAssaultEvent` | Deep-copies `enemy.anomaly_bars_dict[0]`, stamps `activated_by`, and wraps it in `PolarizedAssaultEvent(execute_tick=tick, anomlay_bar=..., char_instance=..., skill_node=...)`; no Calculator/AP formula read is performed in this XLogic path. | Publishes the event through a fresh dispatch port, then clears `record.trigger_origin`; current event priority / fan-out remain owned by the `PolarizedAssaultEvent` / handler layer. | No `PolarityDisorder`, `DISORDER_SPAWN` listener broadcast, dot runtime registration, or same-tick runtime command write occurs in this trigger effect. | Validate with Alice dispatch focused test and `implicit-events`; do not classify Alice as formula-read replacement work just because it copies an anomaly bar. |
| `UpdateAnomaly.spawn_output(...)` / `CopyAnomalyForOutput.py` | `NewAnomaly`、`Disorder`、`PolarityDisorder` and `DirgeOfDestinyAnomaly` deep-copy an `AnomalyBar`, retain copied snapshot metadata, normalize `active_by`, and add disorder / polarity fields where needed. | `spawn_output()` returns payloads; publish is caller-owned (`UpdateAnomaly._publish_scheduled_event(...)` or XLogic dispatch ports). | `spawn_output()` broadcasts `LBS.DISORDER_SPAWN` only for mode `1` / `2`; `remove_dots_cause_disorder(...)` dot removals / freeze follow-up and `anomaly_effect_active(...)` runtime writes remain separate retained boundaries. | Existing `test_update_anomaly_dispatch.py`, anomaly handler runtime-view tests, dot runtime tests, and `implicit-events` are compatibility evidence. No new output publisher, listener gateway, dot facade, or runtime write facade is added. |

US-009 结论：copied anomaly / disorder output work must be split into at least four axes before phase-3 work: formula / copied snapshot data (`AnomalyBar.current_ndarray`, `MulData` / AP ratios, `anomaly_dmg_ratio`, polarity ratio), scheduled payload publication (`ScheduleDispatchPort`), listener broadcast (`LBS.DISORDER_SPAWN` inside `spawn_output()`), and runtime state / same-tick writes (dot runtime removal / registration and existing `RuntimeCommandPort`-backed paths). The four named BuffXLogic paths are already dispatch-port-backed and remain guarded-maintenance scope; this story adds classification evidence only.

No new old-Buff coupling was discovered; `docs/旧Buff系统耦合审查结果.md` does not need a new entry for this characterization.

### 2026-06-09 US-010 Remaining XLogic Formula-Adjacent Candidate Classification

本节只把剩余 formula-adjacent `XLogic` 候选按 helper family 和风险分组；不改 `BuffXLogic`、`Calculator.py`、`CalAnomaly.py`、dispatch adapter、runtime port、guardrail 或验证脚本。P2-A through P2-G 已迁移文件默认进入 guarded maintenance，不再作为下一轮默认 implementation backlog。

#### Source / navigation evidence

- `rg` preflight used `cal_am|cal_ap|cal_imp|cal_crit_rate|cal_personal_crit_rate|cal_personal_crit_dmg|CalAnomaly|Calculator|MultiplierData|DynamicStatement|copyed_anomaly|copied anomaly|anomaly_bar|impact|crit|异常|公式快照|公式一致` across `BuffXLogic`, `Calculator.py`, `anomaly_bar`, `UpdateAnomaly`, `tests/simulator`, this matrix, replacement notes, and Ralph artifacts; doc inventory used `rg --files docs`.
- Exact reader-symbol scan for `read_anomaly_mastery|read_anomaly_proficiency|read_impact|read_full_crit_rate|read_personal_crit_rate|read_personal_crit_damage|create_anomaly_attribute_read_context` found 24 files / 178 lines across migrated P2-A / P2-B root files, reader / state-sync / guardrail tests, docs, and retained Calculator helper definitions; these are guarded-maintenance evidence, not default backlog.
- Root source scans found `enemy.dynamic.*` BuffXLogic reads in 23 files / 41 lines, with the anomaly / freeze / dot / stun subset in 21 files / 36 lines; formula snapshot source terms in 11 files / 88 lines; and copied-output source terms in 7 files / 41 lines for the narrowed BuffXLogic / UpdateAnomaly / CopyAnomaly set. These counts exclude `.codex_worktrees/` blocker conclusions and must still be narrowed by source, focused tests, and validation risk before implementation.
- CodeGraph query terms used: the generated US-010 seed from `scripts/ralph/context/codegraph-guide.md`, plus `BuffXLogic Calculator CalAnomaly MultiplierData DynamicStatement cal_am cal_ap cal_imp cal_crit_rate cal_personal_crit_rate cal_personal_crit_dmg anomaly_bar copyed_anomaly impact crit anomaly ratio remaining XLogic Formula Adjacent Candidates`, and `remaining formula-adjacent XLogic candidates enemy.dynamic anomaly-state reads ElectroLipGlossAtkAndDmgBonus JaneAdditionalAbilityPhyBuildupBonus MarcatoDesireAtkBonus TimeweaverApBonus WeepingGeminiApBonus HailstormShrineIceBonus MiyabiAdditionalAbility_IgnoreIceRes BranchBladeSongCritRateBonus VivianCorePassiveTrigger VivianCinema6Trigger YanagiPolarityDisorderTrigger YuzuhaCinema2Trigger`.
- CodeGraph cross-checks surfaced `VivianCinema6Trigger.special_effect_logic(...)` AP / `anomaly_dmg_ratio` / dispatch ordering, `AlicePolarizedAssaultTrigger.special_effect_logic(...)` copied anomaly event construction, and reader helper callers for impact / crit families. Because CodeGraph still surfaces historical `.codex_worktrees/` duplicates, final candidate decisions below use root source scans and current focused validation evidence.

| helper / risk family | root-workspace evidence | current classification | guarded maintenance / unguarded decision | validation entrypoint / non-goal |
| --- | --- | --- | --- | --- |
| AM / AP reader + computed count state-sync | P2-A migrated files `AliceAdditionalAbilityApBonus.py`, `YuzuhaAdditionalAbilityAnomalyBuildupBonus.py`, `YuzuhaAdditionalAbilityAnomalyDmgBonus.py`, `JaneCinema1APTransToDmgBonus.py`, `JaneCoreSkillStrikeCritRateBonus.py`, `JanePassionStateAPTransToATK.py`; older reader-backed samples `BranchBladeSongCritDamageBonus.py` and `TimeweaverDisorderDmgMul.py`; copied-output AP readers in Vivian files remain separate. | Completed reader/state-sync bucket for migrated files; Vivian copied-output AP ratios are formula / copied-output parity candidates, not P2-A backlog. | Migrated files are guarded maintenance only. Reopen only on P2-A source guardrail, reader parity, state-sync order, or validation failure. No true unguarded AM/AP implementation candidate is named by this story. | `calculator-reads`; add `implicit-events` only when a future story changes event-adjacent output / publish behavior. Do not delete `MultiplierData` or rewrite AP / AM formulas. |
| Impact / full-crit / personal-crit reader families | P2-B migrated files `LighterAdditionalAbility_IceFireBonus.py`, `QingYiAdditionalAbilityStunConvertToATK.py`, `CannonRotor.py`, `MiyabiCoreSkill_IceFire.py`, `WoodpeckerElectroSet4_*`, `TriggerAdditionalAbilityStunBonus.py`, `Soldier0AnbyCoreSkillCritDMGBonus.py`; exact reader scan and CodeGraph callers show current helper usage and focused tests. | Completed P2-B guarded bucket. Full crit includes received crit; personal crit rate / damage exclude received fields; event-adjacent full-crit paths keep their existing dispatch / RNG boundaries. | Migrated files are guarded maintenance only. No new impact / crit default backlog is named unless root source or guardrail evidence finds a non-migrated direct helper path with matching validation risk. | `calculator-reads` plus P2-B focused tests / guardrail; `implicit-events` for event-adjacent maintenance. Do not merge full / personal crit semantics or widen P2-B guardrail to unrelated buckets. |
| Anomaly ratio / copied-output formula family | `VivianCorePassiveTrigger.py`, `VivianCinema6Trigger.py`, `YanagiPolarityDisorderTrigger.py`, `AlicePolarizedAssaultTrigger.py`, `UpdateAnomaly.py`, `CopyAnomalyForOutput.py`, `CalAnomaly.py`; source terms include `anomaly_dmg_ratio`, `PolarityDisorder`, `DirgeOfDestinyAnomaly`, `PolarizedAssaultEvent`, `spawn_output(...)`, `current_ndarray`, and AP ratio reads. | Formula / copied-output parity candidate, not a phase-2 dispatch backlog. Scheduled publish paths are already P2-D guarded; listener broadcast and dot/runtime writes stay separate retained layers. | Guarded maintenance for existing dispatch publishers. A future phase-3 formula-output PRD may name this family only after parity suite / behavior sample requirements are explicit. No US-010 live implementation target. | `calculator-reads` for ratio / formula characterization; `implicit-events` when publish, listener, dot, or runtime layering changes. Do not move `spawn_output()` broadcast, add a second write facade, or rewrite copied-output formulas here. |
| Enemy anomaly-state read helpers | Root scan found 23 files / 41 lines of `enemy.dynamic.*` BuffXLogic reads, including 21 files / 36 lines in the anomaly / freeze / dot / stun subset. Representative files include `ElectroLipGlossAtkAndDmgBonus.py`, `JaneAdditionalAbilityPhyBuildupBonus.py`, `MarcatoDesireAtkBonus.py`, `TimeweaverApBonus.py`, `WeepingGeminiApBonus.py`, `MiyabiAdditionalAbility_IgnoreIceRes.py`, `BranchBladeSongCritRateBonus.py`, `VivianCorePassiveTrigger.py`, `VivianCinema6Trigger.py`, `YanagiPolarityDisorderTrigger.py`, and selected P2-G / P2-E representatives. | Read-only enemy-state / edge-detection candidate block. This is formula-adjacent because it gates anomaly / disorder / ratio-producing behavior, but it is not Calculator formula replacement. | This is the only true unguarded same-phase candidate block named by US-010: start with focused branch characterization and read-only helper design if a future PRD needs it. Keep P2-G selected direct-context guardrail scope separate from the wider enemy-state pool. | `implicit-events`; add file-specific no-op / positive / edge-detection tests before extraction. Do not classify enemy-state gates as scheduled publishers, old-container deletion, `LegacyBuffRuntimeFacade` replacement, or phase-3 formula rewrite. |
| Retained formula snapshots and compatibility surfaces | `Calculator.py`, `CalAnomaly.py`, `MultiplierData`, `MulData`, `DynamicStatement`, `AnomalyBar.current_ndarray`, old containers, legacy `buff_add()` / `KickOutBuff()`, deleted raw event-list discovery surfaces, and planning/docs/script hits from the root census. | Retained compatibility / phase-3 readiness only. Reader helper parity and copied-output characterization are evidence for a future formula parity suite, not proof that formula snapshots can be removed. | Not an unguarded phase-2 implementation candidate. Move only through a later phase-3 go / no-go decision with explicit formula parity, registered behavior sample conditions, rollback plan, and scoped validation wiring. | `calculator-reads` today; future formula profile only after tests exist. Do not delete old containers, raw compatibility adapters, `MultiplierData`, `MulData`, `CalAnomaly`, or legacy lifecycle helpers in phase-2 closure work. |

#### US-010 conclusion

- P2-A through P2-G migrated files are now explicitly classified as completed guarded-maintenance scope rather than default implementation backlog.
- The only source-backed unguarded same-phase candidate block is enemy anomaly-state read helpers, because root `enemy.dynamic.*` reads remain wider than the selected P2-G / P2-E guarded representatives and carry `implicit-events` branch-risk. It should start with focused characterization, not extraction.
- Formula snapshots, copied anomaly output ratios, Calculator / CalAnomaly internals, old containers, legacy `buff_add()` / `KickOutBuff()`, and deleted raw event-list discovery surfaces stay retained compatibility or phase-3 readiness items.
- No new old-Buff coupling was discovered; `docs/旧Buff系统耦合审查结果.md` does not need a new entry for this classification.

### 2026-06-09 US-011 Behavior Sample Matrix And Registered-Team Requirements

本节只记录 behavior sample 触发条件和注册队伍覆盖状态；不新增注册队伍、不运行 main-loop sample、不改 `scripts/run_buff_main_loop_consistency.py`、不改验证 profile，也不替换任何 live formula / event / runtime path。

#### Source / navigation evidence

- `rg` preflight used `run_buff_main_loop_consistency|莱特火属性队|席德大安比队|青衣雷属性队|Alice|Yuzuha|Jane|Vivian|Yanagi|注册队伍|registered|行为样本|formula snapshot|公式快照|Phase-3|阶段 3|阶段3` across docs, scripts, tests, `zsim/sim_progress/Character`, and `zsim/utils/main_loop_consistency.py`.
- CodeGraph query terms used: `US-011 Decide Behavior Sample Matrix And Registered-Team Requirements scripts/run_buff_main_loop_consistency.py zsim/utils/main_loop_consistency.py zsim/run.py run_buff_refactor_validation Character Alice Jane Yuzuha Vivian Yanagi team registry registered team isolated teams main_loop consistency`; narrowed symbols included `_load_team_config`, `auto_register_teams`, `TeamRegistry`, and the root `tests/teams/*_teams.py` registered-team classes.
- Root `zsim/utils/main_loop_consistency.py::_load_team_config(...)` loads `tests.teams.auto_register_teams()` and raises for unknown teams, so future samples must use actual registered `TeamRegistry` entries rather than ad hoc validation-only names.
- Root registered-team source currently names `青衣雷属性队`, `席德大安比队`, `莱特火属性队`, and `薇薇安物理队`; `示例冰属性队` is not auto-registered in `tests/teams/ice_teams.py`.

| sample / candidate family | current registered-team state | what existing evidence proves | US-011 decision |
| --- | --- | --- | --- |
| P2-B crit / impact migrated files | `莱特火属性队` is registered and has a passing stop-tick 600 consistency sample. | Proves the Lighter / Trigger / Hugo representative route kept total damage, event count, and buff timeline stable for P2-B reader work. | Reuse as behavior evidence only for future stories that actually touch the same live route; it is not Calculator / CalAnomaly deletion proof. |
| P2-C trigger-state read-only gates | `席德大安比队` is registered and has a passing stop-tick 1000 consistency sample. | Proves the Seed / Zero Anby / Trigger route kept total damage, event count, and buff timeline stable for P2-C helper work. | Reuse as behavior evidence only for matching live trigger-state maintenance; it does not cover AM/AP, copied-output ratios, dot runtime, forced writes, or direct-context helpers. |
| P2-A AM/AP reader + state-sync (`Alice*`, `Yuzuha*`, `Jane*`) | No registered Alice / 爱丽丝, Yuzuha / 柚叶, or Jane / 简 representative team is present. | Existing evidence is focused parity / order / guardrail tests plus `calculator-reads` and `implicit-events`, not main-loop sample evidence. | Do not invent a sample team. Future live semantic changes need a real registered team first; doc-only / test-only / guardrail work remains covered by focused tests and validation profiles. |
| Vivian / Yanagi copied-output, dot, and scheduled-publish candidates | `薇薇安物理队` is registered and includes 薇薇安 / 柳 / 耀嘉音, but no passing main-loop consistency sample is documented for this PRD. | Current evidence is file-specific dispatch / dot / copied-output characterization and `implicit-events`; it proves layer separation, not live output parity for the physical team. | A future formula-output story may run `薇薇安物理队` only after confirming the APL reaches the target Vivian / Yanagi route; until then use focused characterization plus validation profiles. |
| Enemy anomaly-state read helpers | Candidate files span Jane, Miyabi, Vivian, Yanagi, Timeweaver / Weeping Gemini, and other formula-adjacent gates; no single registered team covers the whole pool. | US-010 source scan proves an unguarded same-phase candidate block exists, but not that a behavior sample can cover every branch. | Start with focused no-op / positive / edge-detection characterization and `implicit-events`; add a registered-team sample only for the concrete live route changed by a future PRD. |
| Phase-3 formula snapshot replacement | No phase-3 implementation is active yet. | `calculator-reads` provides retained snapshot characterization, not full formula replacement parity. | Main-loop consistency is required only after focused formula parity exists and the story changes live formula semantics on a registered route; readiness / go-no-go docs alone skip it. |

US-011 conclusion：behavior samples are live-route evidence, not a generic validation checkbox. Future PRDs must record the registered team, target route, stop tick, labels, and what the sample proves; when the team gap is real, keep the story to focused characterization / validation profiles and document the gap instead of creating a validation-only team.

### 2026-06-09 US-013 Handoff Docs And Next Pool State

本节只同步 handoff docs 和下一轮候选池；不改 `BuffXLogic`、`Calculator.py`、`CalAnomaly.py`、dispatch adapter、runtime port、guardrail、validation profile 或 live formula / event / runtime behavior。未来 PRD 仍沿 `docs/Buff重构方案.md` 的阶段顺序推进：phase-2 implementation buckets 已转入 guarded maintenance，下一步只进入 phase-3 formula parity design / characterization。

#### Source / navigation evidence

- `rg` preflight used `当前默认|下一轮 PRD|Phase-3|阶段3|阶段 3|公式快照|公式一致|formula|closure|阶段2|阶段 2|候选|验证入口|No-Go|Go|兼容|保留边界|US-012|US-013` across next-stage plan, this matrix, checklist, replacement notes, old-coupling review, architecture plan, Ralph artifacts, and the validation runner; doc inventory used `rg --files docs`.
- CodeGraph query terms used: `US-013 Update Handoff Docs And Next Candidate Pool`, `scripts/run_buff_refactor_validation.py`, `TYPECHECK_PROFILES`, `FOCUSED_PYTEST_PROFILES`, `FOCUSED_MYPY_PROFILES`, `BuffRefactorValidationRunner.build_steps`, `parse_args`, `implicit-events`, `calculator-reads`, and phase-3 formula readiness terms.
- CodeGraph confirmed current root validation wiring already exposes `implicit-events` and `calculator-reads`; this story does not add a validation profile or modify `run_buff_refactor_validation.py`.

#### Phase-2 closure evidence

| bucket | current state | guarded maintenance evidence | reopen rule |
| --- | --- | --- | --- |
| P2-A AM/AP reader + computed count state-sync | Completed guarded bucket. | Migrated-source guardrail, reader parity, state-sync order tests, `calculator-reads`, and `implicit-events`. | Reopen only when focused tests, source guardrail, or validation names a concrete migrated-file regression. |
| P2-B impact / crit reader families | Completed guarded bucket. | P2-B reader guardrail, full-crit / personal-crit tests, dispatch-adjacent tests, `calculator-reads`, `implicit-events`, and route-specific `莱特火属性队` evidence. | Reopen only on concrete reader semantics, event-order, source guardrail, or validation failure. |
| P2-C trigger-state read-only gates | Completed guarded bucket. | `test_trigger_state_read_only_gates.py`, P2-C exact-file guardrail, `implicit-events`, and route-specific `席德大安比队` evidence. | Reopen only if migrated files regain direct writable old-template access or validation names a read-only gate/order regression. |
| P2-D scheduled publish ordering / adapter parity | Completed guarded bucket. | File-specific dispatch tests, adapter rebinding tests, exact-file source guardrail, and `implicit-events`. | Reopen only on payload/order/fan-out/stale-adapter/raw-queue regression. |
| P2-E dot runtime-state / initialization | Completed guarded bucket. | Dot runtime-state / initialization tests, Vivian / UpdateAnomaly focused tests, P2-E exact-file guardrail, and `implicit-events`. | Reopen only on runtime-dot layering, Shock direct-read, invalid scheduled payload, second write facade, or validation regression. |
| P2-F BuffAddStrategy caller / facade-write design | Completed guarded bucket. | Facade contract tests, caller-family tests, bypass-layer tests, raw-container guardrail, P2-F exact-file guardrail, and `implicit-events`. | Reopen only on same-tick write boundary, active/mirror/target fan-out, pending queue, or validation regression. |
| P2-G direct simulator context helpers | Completed guarded bucket. | Service-family tests, exact-file / selected-symbol guardrail, `implicit-events`, and `calculator-reads` contrast evidence for crit-adjacent reader seams. | Reopen only on selected representative service regression; generic `sim_instance` text or `.codex_worktrees/` hits are not blockers. |

#### Formula-readiness findings and next pool

| next pool block | candidate files / symbols | Ralph-sized direction | validation entrypoints | non-goals |
| --- | --- | --- | --- | --- |
| Current default: formula parity suite design / characterization | `zsim/sim_progress/ScheduledEvent/Calculator.py`, `CalAnomaly.py`, `MultiplierData`, `MulData`, `DynamicStatement`, `AnomalyBarClass.py`, `AnomalyBar.current_ndarray`, `CopyAnomalyForOutput.py`, copied anomaly / disorder output paths, and existing reader seam tests. | Name focused formula parity tests, scoped mypy targets, behavior-sample conditions, rollback plan, and retained formula responsibilities before any production formula replacement. | Start with `calculator-reads`, existing `test_buff_attribute_reader.py` characterization, and add named focused tests only after the PRD defines them; add `implicit-events` if event/runtime layering is touched. | No production formula rewrite, no `formula-parity` profile before targets are named, no `MultiplierData` / `MulData` deletion, no old-container deletion. |
| Formula-output / copied anomaly parity | `VivianCorePassiveTrigger.py`, `VivianCinema6Trigger.py`, `YanagiPolarityDisorderTrigger.py`, `AlicePolarizedAssaultTrigger.py`, `UpdateAnomaly.py`, `CopyAnomalyForOutput.py`. | Characterize ratio / copied-output numeric parity and keep payload construction separate from scheduled publish and listener broadcast. | `calculator-reads`; `implicit-events` if payload publish, listener, dot runtime, or same-tick runtime layers change. | Do not move `spawn_output()` broadcast, add a second write facade, or rewrite copied-output formulas without parity coverage. |
| Enemy anomaly-state read helper design | `ElectroLipGlossAtkAndDmgBonus.py`, `JaneAdditionalAbilityPhyBuildupBonus.py`, `MarcatoDesireAtkBonus.py`, `TimeweaverApBonus.py`, `WeepingGeminiApBonus.py`, `MiyabiAdditionalAbility_IgnoreIceRes.py`, `BranchBladeSongCritRateBonus.py`, Vivian / Yanagi representatives, and selected P2-G / P2-E overlap files. | If selected, start with no-op / positive / edge-detection characterization and read-only helper design; keep it separate from Calculator formula replacement. | `implicit-events` plus file-specific focused tests before extraction; behavior sample only for a concrete registered live route. | Do not classify enemy-state gates as scheduled publishers, `LegacyBuffRuntimeFacade` replacement, old-container deletion, or phase-3 production formula rewrite. |
...
US-015 conclusion：handoff docs and the next candidate pool were synchronized at that handoff. The then-current default was US-016 final serial validation / Go-No-Go; the US-016 section below supersedes that default with the final No-Go and next candidate pool.

### 2026-06-10 US-016 Final Serial Validation And Formula Go-No-Go

本节记录 phase-3 formula parity characterization PRD 的最终验证和下一候选池状态；不改 `BuffXLogic`、`Calculator.py`、`CalAnomaly.py`、dispatch adapter、runtime port、guardrail、validation profile、focused tests 或 live formula / event / runtime behavior。

#### Validation evidence

- Touched focused pytest passed: `tests/simulator/test_buff_attribute_reader.py -q` -> `60 passed`; `tests/simulator/test_vivian_core_passive_trigger_dispatch.py tests/simulator/test_vivian_cinema6_trigger_dispatch.py -q` -> `3 passed`.
- `formula-parity` profile passed: base `2 passed`, isolated teams `3 passed`, focused `60 passed`, mypy `Success: no issues found in 9 source files`.
- `calculator-reads` profile passed: base `2 passed`, isolated teams `3 passed`, focused `160 passed`, mypy `Success: no issues found in 22 source files`.
- `implicit-events` profile passed because this PRD touched event-adjacent copied-output characterization and validation wiring: base `2 passed`, isolated teams `3 passed`, focused `238 passed`, mypy `Success: no issues found in 88 source files`.
- Default lifecycle profile passed because this PRD touched validation runner behavior: base `2 passed`, isolated teams `3 passed`, focused `18 passed`, mypy `Success: no issues found in 9 source files`.

#### Final decision

| candidate block | status after US-016 | next Ralph-sized direction | validation entrypoints | non-goals |
| --- | --- | --- | --- | --- |
| Production formula replacement | No-Go | Do not generate a production replacement PRD yet. Keep `Calculator.py`, `CalAnomaly.py`, copied-output formulas, `MultiplierData`, `MulData`, `DynamicStatement`, and `AnomalyBar.current_ndarray` as retained compatibility snapshots. | Keep `formula-parity`, `calculator-reads`, and event/runtime-specific `implicit-events` gates available. | No broad formula rewrite, no old-container deletion, no dispatch/runtime/listener merge. |
| Phase-3 formula oracle gap closure | Current default | Add deterministic table-driven oracles for `RegularMul` / `AnomalyMul` / `StunMul`, `CalDisorder` / `CalPolarityDisorder` / `CalAbloom`, copied-output report payload parity, and `AnomalyBar.current_ndarray` field matrices before any production formula edit. | New focused pytest targets plus `formula-parity`; retain `calculator-reads`; add `implicit-events` only for event/runtime layering and default lifecycle only for lifecycle / validation-runner behavior. | Do not treat reader seam parity or `formula-parity` alone as deletion permission. |
...
### 2026-06-11 US-007 Final Proposal Go / No-Go

This section records the final handoff for the bounded `Calculator.AnomalyMul.cal_res_pen()` proposal PRD. It does not change `BuffXLogic`, `Calculator.py`, `CalAnomaly.py`, dispatch adapters, runtime ports, guardrails, validation profiles, focused tests, copied-output constructors, old containers, or live formula / event / runtime behavior.

#### Validation evidence carried forward

- `US-006` serial gate passed `formula-parity`: base `2 passed`, isolated teams `3 passed`, focused `103 passed`, mypy `Success: no issues found in 9 source files`.
- `US-006` serial gate passed `calculator-reads`: base `2 passed`, isolated teams `3 passed`, focused `203 passed`, mypy `Success: no issues found in 22 source files`.
- `US-007` handoff rerun kept both required profiles green with the same focused counts and scoped mypy success.
- `US-007` is docs-only and does not add production formula, runtime, dispatch, listener, validation-runner, old-container, copied-output constructor, or registered-team fixture changes beyond the final handoff decision.

#### Final Go / No-Go

| candidate block | status after US-007 | next Ralph-sized direction | validation entrypoints | non-goals |
| --- | --- | --- | --- | --- |
| `Calculator.AnomalyMul.cal_res_pen()` | Implementation-eligible bounded domain | Generate one later implementation PRD for exactly this selector / extraction diff. | `formula-parity`; `calculator-reads`; registered main-loop sample only if the future diff changes live semantics and `薇薇安物理队` produces nonzero anomaly / disorder event counts. | No broad `Calculator.py` / `CalAnomaly.py` rewrite, no adjacent formula replacement, no old-container deletion. |
| `Calculator.AnomalyMul.anomaly_snapshot` | Supporting retained vector-order evidence | Keep as guardrail / oracle evidence for the `cal_res_pen()` implementation. | `formula-parity`; focused vector cases. | Do not promote into a second production replacement domain without new PRD evidence. |
| `CalAnomaly.cal_k_level()` clamp | Supporting retained lookup evidence | Preserve below / normal / above-boundary clamp and log side effects. | `formula-parity`; focused level-boundary cases. | Do not rewrite `CalAnomaly.py` as part of `cal_res_pen()` implementation. |
| Copied-output handler/report payload parity | Supporting event-adjacent compatibility evidence | Preserve copied-output constructors, handler report payloads, listener-facing fields, and layer separation. | Focused copied-output tests; `implicit-events` when event/runtime layering is touched. | Do not merge scheduled publish, listener broadcast, dot runtime registration, or same-tick runtime writes. |
| P2-A through P2-G guarded maintenance | Completed guarded buckets | Open only blocker-driven PRDs when focused tests, source guardrails, or validation names a concrete regression. | Existing P2 focused tests plus `calculator-reads`, `implicit-events`, or default profile depending on touched layer. | Do not manufacture implementation backlog from completed buckets, retained compatibility names, or `.codex_worktrees/` hits. |
