...
| --- | --- | --- | --- | --- | --- | --- | --- |
| Immediate phase-2 default | AM/AP reader + computed count state-sync family | `AliceAdditionalAbilityApBonus.py`, `YuzuhaAdditionalAbilityAnomalyBuildupBonus.py`, `YuzuhaAdditionalAbilityAnomalyDmgBonus.py`, `JaneCinema1APTransToDmgBonus.py`, `JaneCoreSkillStrikeCritRateBonus.py`, `JanePassionStateAPTransToATK.py`, existing AM/AP reader samples | Direct `MultiplierData` / alias reads, `dynamic_buff_list=1`, computed count writeback, `simple_start(..., no_count=1)`, `dy.count`, `update_to_buff_0(...)` | Design and test a reusable reader + state-sync harness for one helper family group, then migrate only when parity and ordering tests are green. | `MultiplierData` snapshots, old `buff_0` identity, record fields, state-sync order. | `calculator-reads` plus new focused state-sync pytest. | No scheduled publish migration, no Calculator formula rewrite, no old container deletion. |
| Immediate phase-2 | Crit / impact reader family package | `LighterAdditionalAbility_IceFireBonus.py`, `QingYiAdditionalAbilityStunConvertToATK.py`, `CannonRotor.py`, `MiyabiCoreSkill_IceFire.py`, `WoodpeckerElectroSet4_*`, `TriggerAdditionalAbilityStunBonus.py`, `Soldier0AnbyCoreSkillCritDMGBonus.py` | Impact and crit helpers mix read-only gates, RNG / event-adjacent reads and count writeback. | Design `read_impact(...)`, `read_full_crit_rate(...)`, `read_personal_crit_rate(...)`, `read_personal_crit_damage(...)` candidates with focused parity tests before production replacement. | Full crit includes received crit; personal crit helpers must not include received crit; existing event publishers keep `ScheduleDispatchPort`. | `calculator-reads`; event-adjacent paths add `implicit-events`; focused state-sync tests. | Do not combine full / personal crit semantics or make a one-file Trigger / Soldier PRD. |
...
- 第一推荐 follow-up 是 AM/AP reader + computed count state-sync family，因为它同时覆盖已有 reader seam、多个文件、公式 parity 和写回顺序；它不是单文件薄 PRD。
