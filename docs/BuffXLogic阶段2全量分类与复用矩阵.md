...
| Immediate phase-2 default | BuffAddStrategy caller and facade-write design package | `HugoCorePassiveTotalizeTrigger.py`, `RoaringRideBuffTrigger.py`, `Seed*Trigger.py`, `UpdateAnomaly.anomaly_effect_active(...)`, BattleEventListener callers | Forced same-tick Buff / Debuff writes route through `buff_add_strategy(...)` and `LegacyBuffRuntimeFacade`; target fan-out and enemy mirror sync are behavior-critical. | Classify caller shapes and add focused tests for active replacement, enemy mirror sync, selected target fan-out and no pending queue write before any replacement. | Existing `LegacyBuffRuntimeFacade`, old registry/template identity, one `RuntimeCommandPort` / facade write boundary. | `implicit-events`, `test_buff_add_strategy_runtime_facade.py`, raw-container guardrail; default lifecycle only for lifecycle edits or live write semantic changes. | No second write facade, no scheduled-publish conversion, no legacy `buff_add()` / `KickOutBuff()` deletion. |
| Later phase-2 | Direct simulator context extraction | `YuzuhaHardCandyShotTrigger.py`, `YuzuhaCinema4QuickAssistTrigger.py`, `YuzuhaCinema6SheelTrigger.py`, RNG trigger files, report-only `change_process_state()` files | XLogic reads local tick, preload, char-data, enemy, listener or RNG services directly through `sim_instance`; these are service-specific dependencies, not one Buff runtime facade target. | Group by concrete service and add branch-focused tests before extracting explicit context helpers. | Local preload, Character action/resource, listener/report/RNG and scheduled publish/runtime write boundaries stay separate. | `implicit-events` plus file-specific branch tests; real registered consistency sample only when live behavior warrants it. | Not raw queue backlog, not `LegacyBuffRuntimeFacade` replacement, no Calculator formula rewrite. |
...
- Next pool state: P2-B crit / impact reader family, P2-C trigger-state gates, P2-D scheduled publish ordering / adapter parity and P2-E dot runtime-state / initialization have completed after follow-up PRDs. P2-F BuffAddStrategy facade-write design is now the default same-phase PRD candidate; direct simulator context extraction, phase-3 formula snapshot replacement, retained compatibility, and blocker-only phase-1 reopen rows remain available.
...
- Next pool state: P2-C trigger-state read-only gates, P2-D scheduled publish ordering / adapter parity and P2-E dot runtime-state / initialization have completed after follow-up PRDs. P2-F BuffAddStrategy facade-write design is now the default same-phase PRD candidate. Direct simulator context extraction, phase-3 formula snapshot replacement, retained compatibility, and blocker-only phase-1 reopen rows remain available.
...
- Next pool state: P2-D scheduled publish ordering / adapter parity and P2-E dot runtime-state / initialization have completed after follow-up PRDs. P2-F BuffAddStrategy facade-write design is now the default same-phase PRD candidate. Direct simulator context extraction, phase-3 formula snapshot replacement, retained compatibility, and blocker-only phase-1 reopen rows remain available.
...
- Next pool state: P2-E dot runtime-state / initialization has completed guarded scope after the follow-up PRD. P2-F BuffAddStrategy facade-write design is now the default same-phase PRD candidate. Direct simulator context extraction, phase-3 formula snapshot replacement, retained compatibility and blocker-only phase-1 reopen rows remain available.
...
- Next pool state: P2-F BuffAddStrategy caller / facade-write design is now the default same-phase PRD candidate. Direct simulator context extraction, P2-D / P2-E guarded maintenance, phase-3 formula snapshot replacement, retained compatibility and blocker-only phase-1 reopen rows remain available.

### US-008 结论

- 第一推荐 follow-up 已从完成的 P2-E 转为 P2-F BuffAddStrategy caller / facade-write design；它应覆盖 caller taxonomy、active-store replacement、enemy debuff mirror sync、selected-target fan-out 与 no pending queue write focused tests，而不是把 forced Buff / Debuff write 转成 planned-event backlog 或新增第二套 same-tick write facade。
- Immediate phase-2 follow-ups should group by same-tick write boundary, direct context service, guarded maintenance evidence or validation profile. Later phase-2 work should handle direct simulator services and formula snapshot replacement after focused tests exist.
