...
| Immediate phase-2 default | Direct simulator context extraction | `YuzuhaHardCandyShotTrigger.py`, `YuzuhaCinema4QuickAssistTrigger.py`, `YuzuhaCinema6SheelTrigger.py`, RNG trigger files, report-only `change_process_state()` files | XLogic reads local tick, preload, char-data, enemy, listener, RNG or report-state services directly through `sim_instance`; these are service-specific dependencies, not one Buff runtime facade target. | Group by concrete service and add branch-focused tests before extracting explicit context helpers; start with Yuzuha tick / preload / next-char / report-state representatives and keep RNG/report-only representatives separate. | Local preload, Character action/resource, listener/report/RNG and scheduled publish/runtime write boundaries stay separate. | `implicit-events` plus file-specific branch tests; real registered consistency sample only when live behavior warrants it. | Not raw queue backlog, not `LegacyBuffRuntimeFacade` replacement, no Calculator formula rewrite, no forced Buff write rewrite. |
...
- Next pool state: P2-B crit / impact reader family, P2-C trigger-state gates, P2-D scheduled publish ordering / adapter parity, P2-E dot runtime-state / initialization and P2-F BuffAddStrategy facade-write design have completed after follow-up PRDs. P2-G direct simulator context extraction is now the default same-phase PRD candidate; phase-3 formula snapshot replacement, retained compatibility, P2-F guarded maintenance and blocker-only phase-1 reopen rows remain available.
...
- Next pool state: P2-C trigger-state read-only gates, P2-D scheduled publish ordering / adapter parity, P2-E dot runtime-state / initialization and P2-F BuffAddStrategy facade-write design have completed after follow-up PRDs. P2-G direct simulator context extraction is now the default same-phase PRD candidate. Phase-3 formula snapshot replacement, retained compatibility, P2-F guarded maintenance and blocker-only phase-1 reopen rows remain available.
...
- Next pool state: P2-D scheduled publish ordering / adapter parity, P2-E dot runtime-state / initialization and P2-F BuffAddStrategy facade-write design have completed after follow-up PRDs. P2-G direct simulator context extraction is now the default same-phase PRD candidate. Phase-3 formula snapshot replacement, retained compatibility, P2-F guarded maintenance and blocker-only phase-1 reopen rows remain available.
...
- Next pool state: P2-E dot runtime-state / initialization and P2-F BuffAddStrategy facade-write design have completed guarded scope after follow-up PRDs. P2-G direct simulator context extraction is now the default same-phase PRD candidate. Phase-3 formula snapshot replacement, retained compatibility, P2-F guarded maintenance and blocker-only phase-1 reopen rows remain available.
...
- Next pool state: P2-F BuffAddStrategy caller / facade-write design has completed guarded scope after the follow-up PRD. P2-G direct simulator context extraction is now the default same-phase PRD candidate. P2-D / P2-E / P2-F guarded maintenance, phase-3 formula snapshot replacement, retained compatibility and blocker-only phase-1 reopen rows remain available.
...
- Next pool state: P2-G direct simulator context extraction is now the default same-phase PRD candidate. P2-D / P2-E / P2-F guarded maintenance, phase-3 formula snapshot replacement, retained compatibility and blocker-only phase-1 reopen rows remain available.

### US-008 结论

- 第一推荐 follow-up 已从完成的 P2-F 转为 P2-G direct simulator context extraction；它应按服务分类 `sim_instance.tick`、`preload.preload_data`、`char_data.find_next_char_obj(...)`、`schedule_data.enemy`、`listener_manager`、`rng_instance` 与 report-state / `change_process_state()` usage，先补 branch-focused tests，再设计 explicit context helper。
- Immediate phase-2 follow-ups should group by direct context service, guarded maintenance evidence or validation profile. Later phase-2 work should handle formula snapshot replacement after focused tests exist.
