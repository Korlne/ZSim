...
| Immediate phase-2 default | Scheduled publish ordering and adapter parity package | `CannonRotor.py`, `HugoCorePassiveTotalizeTrigger.py`, `YixuanCinema1Trigger.py`, `VivianDotTrigger.py`, `YanagiPolarityDisorderTrigger.py`, `ElegantVanitySpRecover.py`, `SliceofTimeExtraResources.py`, `UpdateAnomaly.update_anomaly(...)` | Already-migrated `ScheduleDispatchPort` producers still carry source-specific payload / target / priority / order constraints, including `mission_start(...)`, publish-before/after-reset and adapter event-list rebinding risks. | Extend focused parity coverage by payload family before any further event helper consolidation. | `ScheduleDispatchPort` queue-only boundary; listener broadcast, dot runtime registration and runtime writes stay separate; create adapters on demand from current `sim_instance` / `schedule_data`. | `implicit-events` and file-specific dispatch tests. | No raw queue reopen, no dot runtime registration rewrite, no same-tick runtime write facade. |
...
- Next pool state: P2-B crit / impact reader family and P2-C trigger-state gates have completed after follow-up PRDs. P2-D scheduled publish ordering is now the default same-phase PRD candidate; P2-E dot runtime-state, P2-F BuffAddStrategy facade-write design, direct simulator context extraction, phase-3 formula snapshot replacement, retained compatibility, and blocker-only phase-1 reopen rows remain available.
...
- Next pool state: P2-C trigger-state read-only gates have completed after the follow-up PRD. P2-D scheduled publish ordering is now the default same-phase PRD candidate. P2-E dot runtime-state, P2-F BuffAddStrategy facade-write design, direct simulator context extraction, phase-3 formula snapshot replacement, retained compatibility, and blocker-only phase-1 reopen rows remain available.
...
- Next pool state: P2-D scheduled publish ordering / adapter parity is now the default same-phase PRD candidate. P2-E dot runtime-state, P2-F BuffAddStrategy facade-write design, direct simulator context extraction, phase-3 formula snapshot replacement, retained compatibility, and blocker-only phase-1 reopen rows remain available.

### US-008 结论

- 第一推荐 follow-up 已从完成的 P2-C 转为 P2-D scheduled publish ordering / adapter parity；它应覆盖 payload fields、target fan-out、priority、source-specific publish ordering、adapter rebinding 和 `.codex_worktrees/` 排除，而不是单文件薄 PRD。
- Immediate phase-2 follow-ups should group by scheduled payload/order semantics, helper family, direct context service or validation profile. Later phase-2 work should handle dot runtime state, direct simulator services and facade-backed forced writes after focused tests exist.
